...
def parse_args(project_root: Path) -> argparse.Namespace:
    ...
    p.add_argument(
        "--failure_summary_independent_pool", action="store_true",
        help="Select failure summaries from the full ranked candidate pool instead of only final top-k.",
    )
    p.add_argument(
        "--failure_summary_min_success", type=int, default=0,
        help="Minimum number of successful memories preserved when injecting failure summaries.",
    )
    p.add_argument(
        "--failure_summary_min_similarity", type=float, default=0.0,
        help="Minimum similarity for independent failure-summary candidates.",
    )
    p.add_argument(
        "--failure_summary_min_evidence", type=int, default=1,
        help="Minimum compatible Region x SQL-signature failure memories required for Structured FS.",
    )
    p.add_argument(
        "--failure_summary_db_structured", action="store_true",
        help="For LLB-DB, aggregate Region failures by SQL signature into schema-agnostic guardrails.",
    )
    p.add_argument(
        "--failure_summary_preserve_selection", action="store_true",
        help="Preserve corrected top-k IDs; only replace naturally selected failure content.",
    )
    p.add_argument(
    ...
    try:
        ...
        if args.mem0:
            ...
            region_manager = RegionManager(
                ...
                region_split_range_fraction=float(os.environ.get("MEMRL_REGION_SPLIT_RANGE_FRACTION", "0.15")),
                region_max_variance_splits_per_epoch=int(os.environ.get("MEMRL_REGION_MAX_VARIANCE_SPLITS_PER_EPOCH", "0")),
                region_split_min_effective_evidence=float(os.environ.get("MEMRL_REGION_SPLIT_MIN_EFFECTIVE_EVIDENCE", "0")),
                region_progressive_best_split=os.environ.get("MEMRL_REGION_PROGRESSIVE_BEST_SPLIT", "0").lower() in {"1", "true", "yes"},
                region_max_merges_per_epoch=int(os.environ.get("MEMRL_REGION_MAX_MERGES_PER_EPOCH", "0")),
                region_split_min_child_size=int(os.environ.get("MEMRL_REGION_SPLIT_MIN_CHILD_SIZE", "1")),
                region_protect_new_split_children=os.environ.get("MEMRL_REGION_PROTECT_NEW_SPLIT_CHILDREN", "0").lower() in {"1", "true", "yes"},
            )
        ...
        # Optional one-shot topology maintenance immediately after a mid-section
        # restore. This lets an adaptive-split continuation use multiple Regions
        # for the remaining batches instead of waiting until section end. It is
        # deliberately gated and only runs for a single-region restored state, so
        # retries from an already-split checkpoint do not repeatedly force splits.
        _split_on_resume = os.environ.get("MEMRL_REGION_SPLIT_ON_RESUME", "0") == "1"
        _resume_rm = getattr(memory_service, "region_manager", None)
        if (
            _split_on_resume
            and resumed_batch > 0
            and _resume_rm is not None
            and getattr(_resume_rm, "_is_clustered", False)
            and len(getattr(_resume_rm, "regions", [])) == 1
        ):
            _before_sizes = [len(r.member_ids) for r in _resume_rm.regions]
            _changed = _resume_rm.maybe_split_merge()
            logger.info(
                "[Region] Resume-time adaptive split/merge: changed=%s, before=%s, after=%s",
                _changed, _before_sizes, [len(r.member_ids) for r in _resume_rm.regions],
            )
        ...
        if args.failure_summary_n_slots > 0:
            runner.configure_failure_summary(
                ...
                independent_pool=args.failure_summary_independent_pool,
                min_success=args.failure_summary_min_success,
                min_similarity=args.failure_summary_min_similarity,
                min_evidence=args.failure_summary_min_evidence,
                db_structured=args.failure_summary_db_structured,
                preserve_selection=args.failure_summary_preserve_selection,
        ...
        # Propagate business failures to the launcher so AIStudio does not mark a
        # crashed experiment as success merely because cleanup completed.
        raise
    finally:
        # --- 3. CLEANUP ---
