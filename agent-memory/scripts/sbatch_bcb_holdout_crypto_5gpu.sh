#!/bin/bash
#SBATCH --job-name=yl-bcb-holdout-crypto
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/bcb_ds_holdout_crypto_5gpu_%j.log
#SBATCH --error=logs/bcb_ds_holdout_crypto_5gpu_%j.log
...
echo "=========================================="
echo "BCB Holdout=Cryptography (5 GPU layout)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "GPU Config: Embed on GPU 4, LLM TP=4 on GPUs 0-3"
echo "=========================================="
...
TEMP_CONFIG=/tmp/holdout_crypto_config_\$\$.yaml
...
echo ''
echo '=========================================='
echo '[HOLDOUT-CRYPTO] Region + Zero-Shot Transfer to Cryptography'
echo 'Config: tau=0.18, trajectory, avefact, additive gating'
echo 'Holdout: bcb/Cryptography (excluded from train)'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --strip_think \
    --holdout_subtask bcb/Cryptography \
    --task_cluster_k 0 \
    ...
    --output_dir ./results/deepseek_holdout_crypto_5gpu

HOLDOUT_EXIT=\$?
echo \"[INFO] Holdout-Crypto exited with code: \$HOLDOUT_EXIT\"
