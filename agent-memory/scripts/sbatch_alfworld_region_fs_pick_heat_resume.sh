#!/bin/bash
#SBATCH --job-name=yl-alf-rfs-heat-resume
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_rfs_pick_heat_resume_%j.log
#SBATCH --error=logs/alf_rfs_pick_heat_resume_%j.log

# Region failure summary: pick_two_obj_and_place ONLY (resume from S4 checkpoint)
# With thread-local Q fix, eval should take ~1h instead of 16h
...
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    "configs/rl_alf_config.qwen72b_holdout_heat_region_failure_summary_4section_resume.yaml" > "/tmp/alf_heat_rfs_$$.yaml"
...
echo "=========================================="
echo "pick_heat_then_place_in_recep — Region FS RESUME from S2 (run S3+S4)"
echo "Start: $(date)"
echo "=========================================="
