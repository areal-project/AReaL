...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_highconf_corrected_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"
echo '=========================================='
echo 'LLB DB corrected-topology high-confidence structured FS 10-epoch training (gpt-4.1-mini)'
...
export MEMRL_RUN_ID=region-fs-db-gpt41mini-highconf-corrected-20260804
...
cfg['memory']['user_id']='llb_db_region_fs_highconf_corrected_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_highconf_corrected'
...
printf '%s\n' '[INFO] Fresh corrected topology with high-confidence Structured FS, no dedup, sim/Q=0.5/0.5.'
...
python3 scripts/run_llb_with_rotated_matrix_credentials.py \
  ...
  --failure_summary_min_similarity 0.60 \
  --failure_summary_min_evidence 3 \
