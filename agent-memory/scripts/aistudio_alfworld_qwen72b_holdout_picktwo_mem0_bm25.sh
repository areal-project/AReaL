#!/bin/bash
# AIStudio: Qwen2.5-72B ALFWorld pick-two holdout Mem0 BM25 baseline (SGLang, 3x H200).
...
LOGFILE="${MEMRL_DIR}/logs/aistudio_qwen72b_holdout_picktwo_mem0_bm25_${RUN_TAG}.log"
if [[ -n "${EMBED_PORT:-}" && -n "${LLM_PORT:-}" && -n "${NCCL_PORT:-}" ]]; then
    : # explicit override for diagnostics
else
    read -r EMBED_PORT LLM_PORT NCCL_PORT < <(python3 - <<'PYPORTS'
import socket
ports=[]
for _ in range(3):
...
export EMBED_PORT LLM_PORT NCCL_PORT
export MEMRL_RUN_ID="${MEMRL_RUN_ID:-qwen72b_picktwo_mem0_bm25_${RUN_TAG//_/-}}"
MEM0_COLLECTION="${MEM0_COLLECTION:-memrl_mem0_alf_qwen72b_picktwo_${RUN_TAG}}"
...
EXPERIMENT_NAME="alfworld_holdout_picktwo_mem0_bm25_qwen72b"
OUTPUT_DIR="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_holdout_qwen72b_picktwo_mem0_bm25_20260806"
