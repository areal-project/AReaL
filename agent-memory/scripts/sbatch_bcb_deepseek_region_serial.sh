#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-region-b32
...
#SBATCH --output=logs/bcb_ds_region_serial_%j.log
#SBATCH --error=logs/bcb_ds_region_serial_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Region, batch_size=1)"
...
# --- Run Region experiment (batch_size=1) ---
echo ''
echo '=========================================='
echo '[REGION] Starting BCB Region Memory Transfer with local embedding (batch_size=1, 10 epochs)'
echo '=========================================='

python run/run_bcb_region.py \
    --config configs/rl_bcb_config.deepseek_local_embed_region_serial.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --k_global 30 \
    --k_local 10 \
    --output_dir ./results/deepseek_region_local_embed_serial \
