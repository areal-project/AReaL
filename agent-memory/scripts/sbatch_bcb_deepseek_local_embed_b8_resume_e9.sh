#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-b8-resume-e9
...
#SBATCH --output=logs/bcb_ds_local_embed_b8_resume_e9_%j.log
#SBATCH --error=logs/bcb_ds_local_embed_b8_resume_e9_%j.log
...
LLM_PORT=8000
EMBED_PORT=8001
RESUME_FROM="./results/deepseek_memrl_local_embed_b8/bigcodebench_eval/instruct_full/memory/20260509_150406_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_rl-on/epoch9/snapshot/step_304"

echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (batch_size=8, resume from epoch 9 step 304)"
...
for i in \$(seq 1 300); do
    ...
    if [ \$i -eq 300 ]; then
        echo '[ERROR] Embedding vLLM failed to start after 300s'
...
# --- Start LLM vLLM on GPUs 0-3 (TP=4), port 8000, REDUCED GPU utilization ---
echo '[INFO] Starting LLM vLLM (DeepSeek-R1-32B, TP=4) on GPUs 0-3 with gpu-memory-utilization=0.80...'
...
for i in \$(seq 1 600); do
    ...
    if [ \$i -eq 600 ]; then
        echo '[ERROR] LLM vLLM server failed to start after 600s'
...
# --- Run MemRL experiment (resume from epoch 9 step 304) ---
echo ''
echo '=========================================='
echo '[MEMRL] Resuming BCB MemRL from epoch 9 step 304 (10 epochs total)'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_local_embed_b8.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_memrl_local_embed_b8 \
