#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld MemRL resume → MemRL traj (sequential, vLLM, 3x H200)
#
# GPU layout:
#   GPU 0: Qwen3-Embedding-8B (vLLM)
#   GPU 1-2: Qwen2.5-72B TP=2
#
# Sequential: MemRL proc resume (S9 b20 →完) → MemRL traj (from scratch, 10 section)
# ============================================================================
set -e
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_memrl_seq_vllm_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B MemRL resume → MemRL traj (vLLM, 3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
# --- Launch Qwen2.5-72B (TP=2, vLLM) on GPU 1-2 ---
...
# ============================================================================
# 1. MemRL proc resume (from S9 b20)
# ============================================================================
export MEMRL_RUN_ID="20260709-151359"

cat > /tmp/alf_memrl_proc_resume_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
experiment:
  ...
  experiment_name: alfworld_memrl_qwen72b
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
echo "[EXP] === Phase 1: MemRL (proc) resume ==="
python3 run/run_alfworld.py --config /tmp/alf_memrl_proc_resume_$$.yaml --skip_initial_eval
echo "[EXP] MemRL (proc) resume exit=$? at $(date)"
...
cat > /tmp/alf_memrl_traj_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
  base_url: http://localhost:${LLM_PORT_1}/v1/
...
echo "[EXP] === Phase 2: MemRL (traj) from scratch ==="
python3 run/run_alfworld.py --config /tmp/alf_memrl_traj_$$.yaml --skip_initial_eval
