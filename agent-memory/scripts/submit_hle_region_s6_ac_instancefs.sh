...
RID="yl-hle-region-s6-ac-instancefs-$(date +%Y%m%d-%H%M%S)"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_proceduralization_structured_fillfix_yl-hle-region-proc-structuredfs-fillfix-20260803-223554-regionprocstructfill/snapshot/6
echo "[submit] HLE structured FS, completed structured fill-fix S6 source: $SOURCE_CKPT"
REMOTE="/tmp/${RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_s6_ac_instancefs_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
SITE="/tmp/hle_region_s6_ac_instancefs_site_${HLE_RID}"
rm -rf "$SITE"; mkdir -p "$SITE"
pip install --target "$SITE" -i https://pypi.antfin-inc.com/simple/ tensorboard pandas tqdm concurrent-log-handler hdbscan >/tmp/hle_region_s6_ac_instancefs_deps.log 2>&1
export MEMRL_HLE_SAFE_SOURCE="$ROOT/MemRL"
export PYTHONPATH="$ROOT/MemRL/scripts/hle_instance_fs_overlay:$SITE:$ROOT/MemRL:$ROOT/.local/lib/python3.12/site-packages:${PYTHONPATH:-}"
...
SAFE_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_s6_ac_instancefs_${HLE_RID}-regionac"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_proceduralization_structured_fillfix_yl-hle-region-proc-structuredfs-fillfix-20260803-223554-regionprocstructfill/snapshot/6
...
export HLE_CKPT="$REGION_CKPT" CFG="/tmp/hle_region_s6_ac_instancefs_${HLE_RID}.yaml"
echo "[HLE REGION S6 A+C INSTANCE FS] rid=$HLE_RID same_region_config=true FS_mode=hle_structured slots=1 independent_pool=true min_success=2 min_sim=0.50 signature=category+raw_subject min_evidence=2 abstain=original_region_top3 exact_task_gate=true lambda_max=1.0 confidence_k=1 score_formula=unchanged"
python3 -c "import sys; sys.path.insert(0, '/storage/openpsi/users/yl/agent-memory/MemRL/scripts/hle_instance_fs_overlay'); import memrl.run.hle_region_runner as rm; assert 'hle_instance_fs_overlay' in rm.__file__; assert 'hle_structured' in open(rm.__file__).read(); print('[HLE STRUCTURED FS] OVERLAY_PRECHECK_OK', rm.__file__)"
python3 "$ROOT/MemRL/scripts/test_hle_instance_fs_overlay.py"
echo "[HLE INSTANCE FS] DYNAMIC_GATE_TEST_OK"
python3 - <<'PY'
import os,yaml
...
cfg['memory'].update(build_strategy='proceduralization',user_id='hle_region_s6_ac_instancefs_user')
cfg['experiment'].update(
    experiment_name='hle_regionfs_s6_ac_instancefs',
    ...
    failure_summary_require_exact_task=True,
)
...
export MEMRL_RUN_ID="${HLE_RID}-regionac" MEMRL_REASONING_EFFORT=high MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_EMBED_MIN_INTERVAL=2.5
python3 "$ROOT/MemRL/scripts/hle_instance_fs_overlay/run_hle_region.py" \
  ...
  --shrinkage_top_n 1 --shrinkage_lambda_max 1.0 --shrinkage_confidence_k 1.0 \
