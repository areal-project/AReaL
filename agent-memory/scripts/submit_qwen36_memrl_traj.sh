#!/bin/bash
# Submit Qwen3.6 ALFWorld MemRL trajectory baseline (2x H200).
set -euo pipefail
...
export WORKER_NUM=0 JOB_TAG="${JOB_TAG:-qwen36-memrl-traj}" GPU_TYPE=h200 GPU_NUM=2 LAUNCH_CONTAINER_MODE=dev_local
export JOB_NAME="yl-alf-qwen36-memrltraj-${TS}"
export KM_IMAGE="${KM_IMAGE:-acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-vllm-20260429}"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/aistudio_alfworld_qwen36_memrl_traj.sh ${TS}"
