...
BRANCH_NAME=alfworld_region_opus47_s8_compactao_throttled_short
...
export MEMRL_EMBED_MIN_INTERVAL=1.5 MEMRL_EMBED_THROTTLE=0.5 MEMRL_EMBED_GLOBAL_MIN_INTERVAL=1.5
export MEMRL_EMBED_429_BASE_DELAY=8 MEMRL_EMBED_429_MAX_DELAY=90
export MEMRL_UPDATE_MAX_WORKERS=2 MEMRL_ALFWORLD_LLM_CONCURRENCY=4
# Unique run ID: never auto-resume into or write under the original experiment.
export MEMRL_RUN_ID="s8-compactao-throttled-short-${RUN_TAG}"
...
echo "[INFO] Short continuation: DATASET_RATIO=${DATASET_RATIO:-0.10}, compact action/observation trajectory, FS=1, concurrency=4, LLM interval=2.0s, retries=5"
