#!/bin/bash
# ============================================================================
# HLE no-memory baseline: gemini-3.5-flash via MatrixLLM API
# 纯 API 调用，不需要 GPU
# ============================================================================
...
export JOB_TAG=""
TS=$(date +%Y%m%d-%H%M%S)
export JOB_NAME="${JOB_NAME:-yl-hle-nomem-g35f-${TS}}"
...
set -e
LOGFILE=/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_hle_nomem_gemini35flash_${RUN_ID}.log
...
# ---------------------------------------------------------------------------
# 环境准备与提交（使用 CPU-only submit template）
# ---------------------------------------------------------------------------
