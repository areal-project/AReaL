...
if [[ -z "${AISTUDIO_TOKEN:-}" ]]; then
 ...
 [[ -n "$token_line" ]] || { echo 'ERROR: AISTUDIO_TOKEN unavailable' >&2; exit 1; }; eval "$token_line"
fi
export JOB_NAME="yl-llbdb-regionfs-final-fair-$(date +%m%d-%H%M)"
export KM_IMAGE='acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401'
export JOB_COMMAND='bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_region_fs_final_fair_aistudio.sh'
...
printf '%s\n' "[INFO] Submitting final fair Region-FS: $JOB_NAME"
cd /tmp
