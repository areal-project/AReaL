#!/bin/bash
#SBATCH --job-name=yl-hle-region-beta
...
#SBATCH --output=logs/hle_region_beta_%j.log
#SBATCH --error=logs/hle_region_beta_%j.log

# HLE Region + Failure Summary with BETA utility mode
# Identical to the current EMA run (932648) except --region_utility_mode beta
# Submit AFTER current baseline (932506) finishes to free slurmd-10's GPU
...
echo "=========================================="
echo "HLE Region + FS (BETA utility mode): deepseek-v3.2 via SGLang"
...
# ============================================================
# 3) Run the Region+FS experiment with BETA utility mode.
# ============================================================
echo ''
echo '[HLE-REGION-BETA] Starting HLE Region + FS (beta utility, 10 sections)'
singularity exec --nv --no-home --writable-tmpfs \
    --bind /storage:/storage \
    $RUNNER_IMG \
    bash -c "
...
python run/run_hle_region.py \
    --config configs/rl_hle_config.region_beta.yaml \
...
"
HLE_EXIT=$?
echo "[INFO] HLE Region+FS (BETA) exited with code: $HLE_EXIT"

# Cleanup SGLang server
kill $VLLM_PID 2>/dev/null
wait $VLLM_PID 2>/dev/null
echo "[INFO] SGLang server stopped. Region+FS (BETA) exit: $HLE_EXIT"
