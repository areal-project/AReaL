#!/bin/bash
#SBATCH --job-name=yl-bcb-region-metainfo
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/bcb_region_metainfo_%j.log
#SBATCH --error=logs/bcb_region_metainfo_%j.log
...
echo "=========================================="
echo "BCB Region + Meta-Info Mode (eval from failure_summary ckpt)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/bcb_region_metainfo_XXXXXX.sh)
...
echo '[INFO] Installing critical eval-time modules directly...'
...
echo '[INFO] Installing remaining BCB eval requirements...'
...
TEMP_CONFIG=/tmp/bcb_region_metainfo_config_$$.yaml
...
echo '=========================================='
echo 'Running Region + Meta-Info (eval-only from failure_summary E10 ckpt)'
echo '=========================================='

CKPT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/training/deepseek_region_failure_summary_5gpu/bigcodebench_eval/instruct_full/region/20260611_131740_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_region/snapshot/final"
...
python run/run_bcb_region.py \
    ...
    --shrinkage_confidence_k 3.0 --val_lambda_max 0.15 \
    --meta_info_mode \
    --eval_only \
    --resume_from "$CKPT" \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/training/deepseek_region_metainfo_5gpu

EXIT_CODE=$?
echo "[INFO] Region + Meta-Info exit code: $EXIT_CODE"
echo '[INFO] Done.'
exit $EXIT_CODE
INNEREOF
