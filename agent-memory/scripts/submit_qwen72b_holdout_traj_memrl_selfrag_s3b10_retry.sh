#!/bin/bash
# Retry the Qwen72B trajectory/Self-RAG holdout arms from their S3 batch-10 checkpoints.
set -euo pipefail
export AISTUDIO_LOGIN_NAME="${AISTUDIO_LOGIN_NAME:-aistudio}"
export AISTUDIO_USERNUMBER="${AISTUDIO_USERNUMBER:-477578}"
export WORKER_NUM=0
export JOB_TAG="qwen72b-holdout-traj-memrl-selfrag-s3b10-retry"
TS="20260729_095007"
RETRY_TS=$(date +%Y%m%d_%H%M%S)
export JOB_NAME="yl-alf-q72b-holdout-traj-memrl-selfrag-s3b10-retry-${RETRY_TS}"
