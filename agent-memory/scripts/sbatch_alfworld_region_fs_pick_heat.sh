#!/bin/bash
#SBATCH --job-name=yl-alf-rfs-heat
...
#SBATCH --output=logs/alf_rfs_pick_heat_%j.log
#SBATCH --error=logs/alf_rfs_pick_heat_%j.log
...
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    "configs/rl_alf_config.qwen72b_holdout_heat_region_failure_summary_4section.yaml" > "/tmp/alf_heat_rfs_$$.yaml"
...
echo "=========================================="
echo "pick_heat_then_place_in_recep — Region FS (4 sections from scratch)"
echo "Start: $(date)"
echo "=========================================="
