#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld Self-RAG ONLY (3x H200)
#
# GPU layout:
#   GPU 0: Qwen3-Embedding-8B
#   GPU 1-2: Qwen2.5-72B TP=2 → Self-RAG
# ============================================================================
set -e
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_selfrag_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B Self-RAG (3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
cat > /tmp/alf_selfrag_qwen72b_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
experiment:
  ...
  experiment_name: alfworld_selfrag_qwen72b
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
echo "[EXP] 72B Self-RAG starting..."
python3 run/run_alfworld.py --config /tmp/alf_selfrag_qwen72b_$$.yaml --selfrag --skip_initial_eval
echo "[EXP] 72B Self-RAG exit=$? at $(date)"
