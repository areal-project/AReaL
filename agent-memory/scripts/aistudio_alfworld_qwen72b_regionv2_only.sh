#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld Region v2 ONLY (3x H200)
#
# GPU layout:
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_regionv2_only_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B Region v2 only (3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
cat > /tmp/alf_regionv2_only_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
experiment:
  ...
  experiment_name: alfworld_region_qwen72b_v2_traj
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
python3 run/run_alfworld.py \
    --config /tmp/alf_regionv2_only_$$.yaml \
