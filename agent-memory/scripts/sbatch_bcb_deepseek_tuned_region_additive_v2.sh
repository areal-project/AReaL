#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-rgn-add-v2
...
#SBATCH --mem=200G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-14,slurmd-23,slurmd-45,slurmd-7,slurmd-54,slurmd-69
#SBATCH --output=logs/bcb_ds_tuned_region_additive_v2_%j.log
#SBATCH --error=logs/bcb_ds_tuned_region_additive_v2_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B Region Additive v2 (P0-P2 fixes)"
echo "  P0: Beta posterior (instead of weighted EMA)"
echo "  P1: temperature=0.3, shrinkage_top_n=1 (hard assignment)"
echo "  P2: min_cluster_size=10 (fewer, larger regions)"
...
echo ''
echo '=========================================='
echo '[REGION-ADDITIVE-V2] P0-P2 fixes:'
echo '  --region_utility_mode beta'
echo '  --region_temperature 0.3'
echo '  --shrinkage_top_n 1'
echo '  --region_min_cluster_size 10'
echo '=========================================='

python run/run_bcb_region.py \
    --config configs/rl_bcb_config.deepseek_tuned_region.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --shrinkage_top_n 1 \
    --region_min_cluster_size 10 \
    --output_dir ./results/deepseek_tuned_region_additive_v2

REGION_EXIT=\$?
echo \"[INFO] Region-Additive-v2 exited with code: \$REGION_EXIT\"
