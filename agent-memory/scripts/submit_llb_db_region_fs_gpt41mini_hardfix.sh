#!/bin/bash
# Submit LLB DB Region+FS hard-membership fix with gpt-4.1-mini to AIStudio
set -e
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbdb-region-fs-hardfix-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"

export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_region_fs_gpt41mini_hardfix_aistudio.sh"
