...
RID="yl-hle-rag-only-$(date +%Y%m%d-%H%M%S)"
RAG_CKPT=$(python3 "$ROOT/MemRL/scripts/select_latest_hle_checkpoint.py" --glob '/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_group1_rag_*/snapshot/*' --max-no-id 2500)
...
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_rag_only_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
STATE="/tmp/hle_rag_only_${HLE_RID}"
mkdir -p "$STATE"
SITE="/tmp/hle_rag_only_site_${HLE_RID}"
...
RAG_CKPT=$(python3 "$ROOT/MemRL/scripts/select_latest_hle_checkpoint.py" --glob '/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_group1_rag_*/snapshot/*' --max-no-id 2500)
echo "[remote-start] dynamically selected RAG checkpoint: $RAG_CKPT"
echo "[HLE rag-only] rid=$HLE_RID RAG=1.5s update_workers=4 shared_inflight=64"
launch(){
 ...
 (
  ...
  echo "[HLE rag-only child] mode=$mode delay=${delay}s interval=${interval}s ckpt=$ckpt update_workers=$MEMRL_UPDATE_MAX_WORKERS"
  python3 - <<'PY'
import os,yaml
...
export JOB_COMMAND="HLE_RID=$(printf '%q' "$RID") RAG_CKPT=$(printf '%q' "$RAG_CKPT") bash -c $(printf '%q' "echo $PAYLOAD | base64 -d | bash")"
