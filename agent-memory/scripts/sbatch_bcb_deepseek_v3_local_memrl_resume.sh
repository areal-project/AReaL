#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-memrl-r
...
#SBATCH --output=logs/bcb_dsv3_memrl_resume_%j.log
#SBATCH --error=logs/bcb_dsv3_memrl_resume_%j.log
...
# ==== MemRL RESUME epochs 4-10 from the completed 3-epoch run ====
# Original 932384 ran epochs 1-3 (val 50.3->51.2->52.3%, still climbing). Resume from
# the epoch-3 snapshot to extend to 10 epochs (match region+FS epoch count). Same output
# dir so the new epochs append; --resume_epoch 3 => start_epoch=4.
RESUME_FROM="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_memrl/bigcodebench_eval/instruct_full/memory/20260618_211416_deepseek-ai_DeepSeek-V3_rl-on/epoch3/snapshot/3"
echo ''
echo '=========================================='
echo '[MemRL] resume epochs 4-10 (retrieve_k=10) from epoch-3 snapshot'
echo '=========================================='
run_in_runner "python run/run_bcb.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    --resume_from '$RESUME_FROM' --resume_epoch 3 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    --checkpoint_interval 100 --max_checkpoints 3 \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_memrl"
MEMRL_EXIT=$?
echo "[INFO] MemRL exit code: $MEMRL_EXIT"

# Cleanup servers.
kill $EMBED_PID $VLLM_PID 2>/dev/null
wait $EMBED_PID 2>/dev/null
wait $VLLM_PID 2>/dev/null
echo "[INFO] Servers stopped. MemRL exit: $MEMRL_EXIT"
