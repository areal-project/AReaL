#!/bin/bash
# GPT-5 mini ALFWorld Region+FS proceduralization with corrected split + stable topology.
...
RUN_TAG=20260801-211800
RUN_ID=gpt5mini-proc-stability-20260801-211800
BRANCH_NAME=alfworld_regionfs_gpt5mini_proc_topology_stable
OUTPUT_ROOT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_gpt5mini_proc_stability_20260801
EXP_DIR="$OUTPUT_ROOT/alfworld/exp_${BRANCH_NAME}_${RUN_ID}"
SNAPSHOT_ROOT="$EXP_DIR/local_cache/snapshot"
LOGFILE="$MEMRL_DIR/logs/slurm_alf_gpt5mini_proc_stability_${RUN_TAG}.log"
TMPROOT="/dev/shm/alf_gpt5mini_proc_stability_${RUN_TAG}"
...
export MEMRL_EMBED_RATE_LIMIT_KEY=gpt5mini-proc-stability
...
print('[PREFLIGHT] gpt-5-mini proceduralization; current source-ledger split; standard 30 steps')
PY
echo '================================================================'
echo '[CELL] GPT-5 mini proceduralization + Region+FS topology stability'
echo '[CELL] fresh memory, S1-S10, batch=32, max_steps=30, max_recent_turns=60'
echo '[CELL] eta=0.03 shrinkage_k=2.5 tau=0.60 ws/wq=0.45/0.55 FS=1'
echo '[CELL] topology init=3000, no mid-epoch edits, merge_interval=3553, cooldown=1'
