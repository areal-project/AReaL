#!/bin/bash
# Final BCB Region+FS: epoch-boundary topology edits allowed, mid-epoch edits disabled.
set -euo pipefail
...
export JOB_NAME="yl-bcb-regionfs-fixed5-resume-e1-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_region_fs_final_nocooldown_fixed5
export BCB_ENTRYPOINT=run/run_bcb_region.py
export BCB_NO_AUTORESUME=0
# Same frozen method; only remove the epoch-boundary cooldown that suppressed E2 specialization.
# Explicitly absent: precluster backfill, contract/min-compatible FS, margin abstention,
# top-3 shrinkage, fixed historical snapshot, or score modifications.
export BCB_EXTRA_ARGS="--resume_from /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_region_fs_final_nocooldown/bigcodebench_eval/instruct_full/region/20260809_154202_deepseek-ai_DeepSeek-V3_region/epoch1/snapshot/1 --resume_epoch 1 --task_cluster_k 0 --region_gating_mode additive --region_utility_mode beta --region_split_evidence_migration_mode soft_source_conserving --region_temperature 0.1 --shrinkage_top_n 1 --region_min_cluster_size 12 --region_min_samples 0 --region_cluster_selection_method leaf --region_max_region_share 0.30 --region_disable_mid_epoch_topology --region_split_range_fraction 0.20 --region_max_variance_splits_per_epoch 1 --region_split_min_effective_evidence 200 --region_smoothing_C 0.5 --propagation_eta 0.12 --propagation_k 30 --propagation_sim_min 0.40 --explore_schedule 0,4,3,2,2,1,1,1,1,0 --explore_success_ratio 0.7 --shrinkage_confidence_k 3.0 --val_lambda_max 0.15 --failure_summary_n_slots 1 --n_eval_runs 3 --eval_temperature 0.2 --multi_eval_epochs last"
LOG_BASE="$MEMRL/logs/aistudio_bcb_regionfs_fixed5_resume_e1_$STAMP"
