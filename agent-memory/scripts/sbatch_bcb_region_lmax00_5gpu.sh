#!/bin/bash
#SBATCH --job-name=yl-bcb-region-lmax00-add
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/bcb_region_lmax00_%j.log
#SBATCH --error=logs/bcb_region_lmax00_%j.log
...
# --- Run experiment ---
echo '=========================================='
echo '[REGION-LMAX00] Pure region utility (lambda_max=0.0) Domain subtask (7 types) + propagation + exploration'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --region_smoothing_C 0.5 --propagation_eta 0.03 --propagation_k 30 \
    --propagation_sim_min 0.40 --explore_schedule 0,4,3,2,2,1,1,1,1,0 \
    --explore_success_ratio 0.7 --shrinkage_lambda_max 0.0 \
    --output_dir ./results/deepseek_region_lmax00_5gpu &
MAIN_PID=$!
