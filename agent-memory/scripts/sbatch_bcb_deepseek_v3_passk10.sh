#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-pk10
...
#SBATCH --output=logs/bcb_dsv3_passk10_%j.log
#SBATCH --error=logs/bcb_dsv3_passk10_%j.log

# BCB pass@10 baseline: 10 epochs of no-memory (retrieve_k=0).
# Each epoch = independent attempt. CSR = tasks that passed at least once.
...
echo "=========================================="
echo "BCB DeepSeek-V3 pass@10 (no memory, 10 epochs)"
...
# Embedding server still needed for memory service init (even with retrieve_k=0)
...
echo ''
echo '=========================================='
echo '[pass@10] DeepSeek-V3, 10 epochs, retrieve_k=0 (no memory)'
echo '=========================================='
run_in_runner "python run/run_bcb.py \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_passk10"
EXIT_CODE=$?
