#!/bin/bash
#SBATCH --job-name=yl-bcb-old-add-new-b8
...
#SBATCH --output=logs/bcb_ds_old_region_additive_new_b8_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_old_region_additive_new_b8_4gpu_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Old Config + Additive NEW bs=8, 4 GPU)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "GPU Config: Embedding TP=4 + LLM TP=4 on GPUs 0-3"
echo "Additive NEW: Beta posterior, temp=0.3, top-1, min_cluster_size=15"
echo "=========================================="
...
echo ''
echo '=========================================='
echo '[MEMRL-OLD-ADDITIVE-NEW-B8] Starting BCB MemRL (10 epochs, bs=8)'
echo 'Config: tau=0.18, trajectory, avefact, sim_norm=0.186/0.094'
echo 'Region: Additive gating (NEW - Beta posterior, temp=0.3, top-1, min_cs=15)'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --output_dir ./results/deepseek_old_region_additive_new_b8_4gpu

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old-Additive-New-B8 exited with code: \$MEMRL_EXIT\"
