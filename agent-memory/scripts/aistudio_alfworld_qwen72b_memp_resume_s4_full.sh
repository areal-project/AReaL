#!/bin/bash
# Resume Qwen2.5-72B ALFWorld MemP from the last fully valid checkpoint (end S3).
# S4 is rerun in full because the old S4 had only 3200/3553 trajectories; resuming
# from a mid-S4 checkpoint would make the section-level train denominator incomplete.
set -euo pipefail
...
RESUME_CKPT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld/alfworld/exp_alfworld_memp_qwen72b_qwen72b_memp_v1/local_cache/snapshot/3"
RUN_TAG="${1:-$(date +%Y%m%d_%H%M%S)}"
RUN_TAG_SAFE="${RUN_TAG//_/-}"
LOGFILE="$MEMRL_DIR/logs/aistudio_qwen72b_memp_resume_s4_full_${RUN_TAG}.log"
BUSINESS_LOG_DIR="$MEMRL_DIR/logs/alfworld_memp_qwen72b_resume_s4_full_${RUN_TAG}"

EMBED_PORT="${EMBED_PORT:-19120}"
LLM_PORT="${LLM_PORT:-19320}"
WORK_TMP_BASE="/storage/openpsi/users/yl/agent-memory/.tmp"
WORK_TMP="$WORK_TMP_BASE/qwen72b_memp_resume_s4_full_${RUN_TAG}_$$"
CFG="$WORK_TMP/alf_memp_resume_s4_full.yaml"

mkdir -p "$WORK_TMP" "$BUSINESS_LOG_DIR" "$(dirname "$LOGFILE")"
chmod 700 "$WORK_TMP"
exec > >(tee -a "$LOGFILE") 2>&1

export MEMRL_RUN_ID="qwen72b-memp-resume-s4-full-${RUN_TAG_SAFE}"
...
printf '%s\n' \
  '============================================================' \
  'Qwen2.5-72B ALFWorld MemP accurate resume' \
  "Start: $(date --iso-8601=seconds)" \
  "Resume checkpoint: $RESUME_CKPT (completed S3)" \
  'Expected resume point: section 4, batch 1 (full S4 rerun)' \
...
cleanup() {
  ...
  if [[ "$WORK_TMP" == "$WORK_TMP_BASE"/qwen72b_memp_resume_s4_full_* ]]; then
    rm -rf -- "$WORK_TMP"
  fi
  exit "$status"
}
...
experiment:
  ...
  experiment_name: alfworld_memp_qwen72b_resume_s4_full_${RUN_TAG}
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
BUSINESS_LOG="$BUSINESS_LOG_DIR/memp_resume_s4_full.log"
echo "[EXP] 72B MemP resume starting at $(date --iso-8601=seconds)" | tee -a "$BUSINESS_LOG"
echo '[EXP] Correctness policy: load completed S3 and rerun all 3553 S4 samples.' | tee -a "$BUSINESS_LOG"
set +e
python3 run/run_alfworld.py --config "$CFG" --skip_initial_eval 2>&1 | tee -a "$BUSINESS_LOG"
rc=${PIPESTATUS[0]}
set -e
echo "[EXP] 72B MemP resume exit=$rc at $(date --iso-8601=seconds)" | tee -a "$BUSINESS_LOG"
exit "$rc"
