#!/bin/bash
# Opus 4.7 ALFWorld Region+FS proceduralization with corrected split + stable topology.
...
RUN_ID=opus47-proc-stability-20260731-022935
BRANCH_NAME=alfworld_regionfs_opus47_proc_topology_stable
OUTPUT_ROOT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_opus47_proc_stability_20260731
EXP_DIR="$OUTPUT_ROOT/alfworld/exp_${BRANCH_NAME}_${RUN_ID}"
SNAPSHOT_ROOT="$EXP_DIR/local_cache/snapshot"
LOGFILE="$MEMRL_DIR/logs/aistudio_alf_opus47_proc_stability_${RUN_TAG}.log"
TMPROOT="/dev/shm/alf_opus47_proc_stability_${RUN_TAG}"
...
# Recovery mode: reduce concurrent streams after prior ReadTimeout/Killed event.
export MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_LLM_MAX_RETRIES=5 MEMRL_LLM_TENACITY_ATTEMPTS=5
export MEMRL_ALFWORLD_LLM_CONCURRENCY=3
export MEMRL_EMBED_RATE_LIMIT_KEY=opus47-proc-stability
export MEMRL_EMBED_GLOBAL_MIN_INTERVAL=3.0 MEMRL_EMBED_MIN_INTERVAL=3.0 MEMRL_EMBED_THROTTLE=3.0
...
print('[PREFLIGHT] proceduralization; current source-ledger split; standard 30 steps')
PY
echo '================================================================'
echo '[CELL] Opus 4.7 proceduralization + Region+FS topology stability'
echo '[CELL] recovery from own latest complete sN_bM, S1-S10, batch=32, max_steps=30, max_recent_turns=60'
echo '[CELL] eta=0.03 shrinkage_k=2.5 tau=0.60 ws/wq=0.45/0.55 FS=1'
echo '[CELL] topology init=3000, no mid-epoch edits, merge_interval=3553, cooldown=1; recovery LLM concurrency=3'
