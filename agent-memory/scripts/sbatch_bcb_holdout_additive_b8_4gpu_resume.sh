#!/bin/bash
#SBATCH --job-name=yl-bcb-holdout-add-b8
...
#SBATCH --output=logs/bcb_ds_holdout_additive_b8_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_holdout_additive_b8_4gpu_%j.log
...
echo "=========================================="
echo "BCB Holdout Transfer (Post, bs=8, GPU 0-3)"
...
TEMP_CONFIG=/tmp/holdout_config_\$\$.yaml
sed \"s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g\" configs/rl_bcb_config.deepseek_old_holdout_bs8.yaml > \$TEMP_CONFIG
echo \"[INFO] Using ports: LLM=${LLM_PORT}, Embed=${EMBED_PORT}\"

# --- Start embedding vLLM (single GPU, low memory to share with LLM) ---
echo '[INFO] Starting embedding vLLM (Qwen3-Embedding-8B)...'
...
python run/run_bcb_holdout.py \
    ...
    --resume_from ./results/deepseek_old_holdout_additive_b8/bigcodebench_eval/instruct_full/holdout/20260519_181931_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_holdout/epoch2/snapshot/step_304 \
    --resume_epoch 2 \
    --resume_step 304 \
    --output_dir ./results/deepseek_old_holdout_additive_b8
