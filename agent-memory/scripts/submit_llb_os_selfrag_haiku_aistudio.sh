#!/bin/bash
# Submit LLB OS selfrag baseline (claude-haiku-4-5) to AIStudio. Sandbox default-ON.
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbos-selfrag-haiku-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_os_baseline_aistudio.sh configs/rl_llb_os_selfrag_haiku.yaml \"\""
