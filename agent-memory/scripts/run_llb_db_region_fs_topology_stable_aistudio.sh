#!/usr/bin/env bash
# Fresh corrected proceduralization Region topology-stability ablation.
# Only algorithmic change: E1 cluster at step 300 and one full-section topology cooldown.
set -euo pipefail
...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_topology_stable_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"

echo '=========================================='
echo 'LLB DB corrected proceduralization Region topology-stable'
echo "Host: $(hostname)"
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-topology-stable-20260727"
...
CONFIG=/tmp/rl_llb_db_region_fs_topology_stable.yaml
...
cfg['memory']['user_id']='llb_db_region_fs_topology_stable_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_topology_stable'
...
printf '%s\n' '[INFO] Fresh topology-stable run: init=300, cooldown=1 full section, embed interval=5s.'
