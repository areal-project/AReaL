#!/bin/bash
# Fresh Leaf-v2 rerun: preserve the split-fix semantics while stabilizing only
# variance-driven topology growth.
set -euo pipefail
...
export JOB_NAME="yl-bcb-leafv2-topostable-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
# Control root only; run/run_bcb_region.py creates a timestamped child run.
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_leaf_v2_topology_stable
export BCB_ENTRYPOINT=run/run_bcb_region.py
# Allow strict runner to resume from the latest full epoch on a manually
# resubmitted job. Never resume the old Leaf or Leaf split-fix output roots.
...
LOG_BASE="$MEMRL/logs/aistudio_bcb_leafv2_topology_stable_$STAMP"
