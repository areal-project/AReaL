#!/bin/bash
# Standalone 3-run multi-eval for the completed Leaf-v2 E10 checkpoint.
set -euo pipefail
...
export JOB_NAME="yl-bcb-leafv2-e10-multieval-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_leafv2_multieval_control
export BCB_NO_AUTORESUME=1
export BCB_ENTRYPOINT=scripts/run_bcb_region_multi_eval_only.py
SNAP=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_leaf_v2_topology_stable/bigcodebench_eval/instruct_full/region/20260725_152657_deepseek-ai_DeepSeek-V3_region/epoch10/snapshot/10
[ -f "$SNAP/snapshot_meta.json" ] || { echo "missing Leaf-v2 E10 snapshot: $SNAP" >&2; exit 1; }
export BCB_EXTRA_ARGS="--resume_from $SNAP --resume_epoch 10 --eval_only --task_cluster_k 0 --region_gating_mode additive --region_utility_mode beta --region_split_evidence_migration_mode soft_source_conserving --region_temperature 0.1 --shrinkage_top_n 1 --region_min_cluster_size 12 --region_min_samples 0 --region_cluster_selection_method leaf --region_max_region_share 0.30 --region_split_range_fraction 0.20 --region_max_variance_splits_per_epoch 1 --region_split_min_effective_evidence 200 --region_smoothing_C 0.5 --propagation_eta 0.12 --propagation_k 30 --propagation_sim_min 0.40 --explore_schedule 0,4,3,2,2,1,1,1,1,0 --explore_success_ratio 0.7 --shrinkage_confidence_k 3.0 --val_lambda_max 0.15 --failure_summary_n_slots 1"
LOG_BASE="$MEMRL/logs/aistudio_bcb_leafv2_e10_multieval_$STAMP"
