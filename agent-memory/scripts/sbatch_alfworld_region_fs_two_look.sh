#!/bin/bash
#SBATCH --job-name=yl-alf-region-fs-2sub
...
#SBATCH --output=logs/alf_region_fs_2sub_%j.log
#SBATCH --error=logs/alf_region_fs_2sub_%j.log

# Region failure summary holdout: pick_two + look_at (4 sections each, from scratch)
# Expected: ~20h (2 subtasks × 4 sections × ~2.5h/section)
...
for cfg in two_region_failure_summary_4section look_region_failure_summary_4section; do
...
# Phase 1: pick_two region failure summary (4 sections)
echo "=========================================="
echo "[PHASE 1/2] pick_two_obj_and_place — Region failure summary (4 sections)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_two_region_failure_summary_4section_$$.yaml" \
...
# Phase 2: look_at region failure summary (4 sections)
echo "=========================================="
echo "[PHASE 2/2] look_at_obj_in_light — Region failure summary (4 sections)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_look_region_failure_summary_4section_$$.yaml" \
    --region --region_gating_mode additive \
    --shrinkage_confidence_k 3.0 --propagation_eta 0.12 --val_lambda_max 0.15 \
