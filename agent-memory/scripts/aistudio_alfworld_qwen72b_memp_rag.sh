#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld MemP + RAG (5x H200)
#
# GPU layout:
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_memp_rag_${1:-$(date +%Y%m%d_%H%M%S)}.log"

EMBED_PORT=9000
LLM_PORT_1=9201  # MemP
LLM_PORT_2=9202  # RAG
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B MemP + RAG (5 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
# --- 1. MemP (proceduralization build, pure sim, no Q) ---
(
    cat > /tmp/alf_memp_qwen72b_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
CFGEOF
    ...
    python3 run/run_alfworld.py --config /tmp/alf_memp_qwen72b_$$.yaml --skip_initial_eval
