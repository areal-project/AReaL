#!/bin/bash
# Submit LLB OS MemP (GPT-4.1-mini, 10 sections + per-section validation) to AIStudio.
set -e
...
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbos-memp-gpt41mini-${TS}"
...
export JOB_COMMAND="export MEMRL_RUN_ID='${LLB_OS_RUN_ID}'; bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_os_memp_gpt41mini_aistudio.sh"
