#!/bin/bash
#SBATCH --job-name=yl-bcb-qwen25coder
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/bcb_qwen25coder_%j.log
#SBATCH --error=logs/bcb_qwen25coder_%j.log

MEMRL_DIR="/storage/openpsi/users/yl/agent-memory/MemRL"
SINGULARITY_IMG="/storage/openpsi/images/areal-latest.sif"
MODEL_PATH="/storage/openpsi/models/Qwen__Qwen2.5-Coder-32B-Instruct"
EMBED_MODEL_PATH="/storage/openpsi/models/Qwen3-Embedding-8B"
LLM_PORT=$((8000 + ($SLURM_JOB_ID % 100) * 2))
EMBED_PORT=$((LLM_PORT + 1))

echo "=========================================="
echo "BCB Qwen2.5-Coder-32B-Instruct: NoMem → MemRL baseline (serial)"
...
TEMP_CONFIG=/tmp/bcb_qwen25coder_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    configs/rl_bcb_config.qwen25_coder_32b.yaml > "$TEMP_CONFIG"

# Override Qwen2.5-Coder-32B-Instruct's generation_config.json which sets temperature=0.7,
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m vllm.entrypoints.openai.api_server \
    --model "$MODEL_PATH" --served-model-name Qwen/Qwen2.5-Coder-32B-Instruct \
...
python run/run_bcb.py \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/qwen25_coder_32b_nomem
...
python run/run_bcb.py \
    ...
    --strip_think \
    --checkpoint_interval 50 --max_checkpoints 3 \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/qwen25_coder_32b_memrl
