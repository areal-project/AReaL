#!/bin/bash
#SBATCH --job-name=yl-alf-region-ablation
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_region_ablation_%j.log
#SBATCH --error=logs/alf_region_ablation_%j.log

# Region ablation: 6 eval-only runs sweeping (val_lambda_max, weight_q) grid.
# Resume from 928122 S7 snapshot (region s10 soft-explore, S7 cum 66.14%).
# Each run = mode=test on valid_seen (140) + valid_unseen (134) = ~10min/run.
# Total: 6 runs × ~10min = ~60min + 30min vLLM startup = ~1.5h.
...
echo "=========================================="
echo "ALFWorld REGION ABLATION (eval grid on 928122 S7 snapshot)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/alf_region_abl_XXXXXX.sh)
...
# --- Watchdog (covers all 6 ablation runs) ---
...
# ============ Ablation loop: 6 configs ============
# (val_lambda_max, weight_q) grid:
#   (0.15, 0.5) = current default — baseline reference
#   (0.05, 0.5) — more region-heavy shrinkage
#   (0.00, 0.5) — pure region utility
#   (0.15, 0.7) — current shrinkage + boost q_z in final score
#   (0.05, 0.7) — combined: region-heavy + boost q_z
#   (0.00, 0.7) — pure region + boost q_z (most aggressive)

for vlmax in 0.15 0.05 0.0; do
    for wq in 0.5 0.7; do
        config="configs/rl_alf_config.qwen72b_region_abl_vlmax${vlmax}_wq${wq}.yaml"
        TEMP_CONFIG=/tmp/alf_region_abl_${vlmax}_${wq}_$$.yaml
        sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
            "$config" > "$TEMP_CONFIG"
        echo ""
        echo "=========================================="
        echo "[ABLATION] val_lambda_max=${vlmax}, weight_q=${wq}"
        echo "Config: $config"
        echo "=========================================="
        python run/run_alfworld.py \
            ...
            --val_lambda_max ${vlmax}
        echo "[INFO] Ablation (vlmax=${vlmax}, wq=${wq}) done."
    done
done
...
echo '[INFO] All ablations complete.'
INNEREOF
