#!/bin/bash
# Resume Qwen2.5-72B ALFWorld RAG from the verified complete end-S9 checkpoint.
# This job runs Section 10 only. It uses an isolated run/output ID and is submitted
# with retry=NEVER, so an AIS restart cannot silently fall back to the old S4 resume.
set -euo pipefail
...
RESUME_CKPT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld/alfworld/exp_alfworld_rag_qwen72b_resume_s5_full_20260721_181552_qwen72b-rag-resume-s5-full-20260721-181552/local_cache/snapshot/9"
RUN_TAG="${1:-$(date +%Y%m%d_%H%M%S)}"
RUN_TAG_SAFE="${RUN_TAG//_/-}"
LOGFILE="$MEMRL_DIR/logs/aistudio_qwen72b_rag_resume_s10_from_s9_${RUN_TAG}.log"
BUSINESS_LOG_DIR="$MEMRL_DIR/logs/alfworld_rag_qwen72b_resume_s10_from_s9_${RUN_TAG}"

EMBED_PORT="${EMBED_PORT:-19120}"
LLM_PORT="${LLM_PORT:-19320}"
WORK_TMP_BASE="/storage/openpsi/users/yl/agent-memory/.tmp"
WORK_TMP="$WORK_TMP_BASE/qwen72b_rag_resume_s10_from_s9_${RUN_TAG}_$$"
CFG="$WORK_TMP/alf_rag_resume_s10_from_s9.yaml"

mkdir -p "$WORK_TMP" "$BUSINESS_LOG_DIR" "$(dirname "$LOGFILE")"
chmod 700 "$WORK_TMP"
exec > >(tee -a "$LOGFILE") 2>&1

export MEMRL_RUN_ID="qwen72b-rag-resume-s10-from-s9-${RUN_TAG_SAFE}"
...
printf '%s\n' \
  '============================================================' \
  'Qwen2.5-72B ALFWorld RAG S10-only accurate resume' \
  "Start: $(date --iso-8601=seconds)" \
  "Resume checkpoint: $RESUME_CKPT (verified completed S9)" \
  'Expected resume point: section 10, batch 1 (full S10 only)' \
...
python3 - "$RESUME_CKPT" <<'PYCKPT'
import json, sys
from pathlib import Path
p = Path(sys.argv[1])
meta = json.loads((p / "snapshot_meta.json").read_text())
state = json.loads((p / "local_cache" / "cum_state.json").read_text())
assert int(meta["checkpoint_id"]) == 9, meta
assert int(state["total"]) == 3553, state.get("total")
assert int(state["global_step"]) == 31858, state.get("global_step")
print("[S10-RESUME-PREFLIGHT] PASS: verified completed S9 checkpoint_id=9, total=3553, global_step=31858")
PYCKPT
...
cleanup() {
  ...
  if [[ "$WORK_TMP" == "$WORK_TMP_BASE"/qwen72b_rag_resume_s10_from_s9_* ]]; then
    rm -rf -- "$WORK_TMP"
  fi
  exit "$status"
}
...
experiment:
  ...
  experiment_name: alfworld_rag_qwen72b_resume_s10_from_s9_${RUN_TAG}
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
BUSINESS_LOG="$BUSINESS_LOG_DIR/rag_resume_s10_from_s9.log"
echo "[EXP] 72B RAG S10-only resume starting at $(date --iso-8601=seconds)" | tee -a "$BUSINESS_LOG"
echo '[EXP] Correctness policy: load verified completed S9 and run all 3553 S10 samples exactly once.' | tee -a "$BUSINESS_LOG"
set +e
python3 run/run_alfworld.py --config "$CFG" --skip_initial_eval 2>&1 | tee -a "$BUSINESS_LOG"
rc=${PIPESTATUS[0]}
set -e
echo "[EXP] 72B RAG S10-only resume exit=$rc at $(date --iso-8601=seconds)" | tee -a "$BUSINESS_LOG"
exit "$rc"
