...
EXPERIMENT="${1:-memp}"
RUNNER_SCRIPT="/storage/openpsi/users/yl/agent-memory/MemRL/scripts/aistudio_bcb_runner.sh"
...
# Set BCB_RESUME_FROM=<snapshot dir> and BCB_RESUME_EPOCH=<completed epoch>.
# e.g. BCB_RESUME_FROM=.../epoch5/snapshot/5 BCB_RESUME_EPOCH=5 -> resumes at E6.
if [ -n "${BCB_RESUME_FROM:-}" ]; then
    ...
    export BCB_EXTRA_ARGS
    echo "[submit] RESUME enabled: from=${BCB_RESUME_FROM} epoch=${BCB_RESUME_EPOCH:-auto}"
fi
...
export SUBMIT_TEMPLATE="${SUBMIT_TEMPLATE:-/storage/openpsi/users/yl/agent-memory/submit_template_bcb.py}"
...
export JOB_COMMAND="bash -c 'exec > >(tee -a ${LOG_FILE_BASE}_\${POD_NAME:-node}.log) 2>&1; BCB_CONFIG=\"${BCB_CONFIG}\" BCB_EPOCHS=\"${BCB_EPOCHS}\" BCB_OUTPUT_DIR=\"${BCB_OUTPUT_DIR}\" BCB_EXTRA_ARGS=\"${BCB_EXTRA_ARGS}\" BCB_NO_AUTORESUME=\"${BCB_NO_AUTORESUME:-0}\" bash ${RUNNER_SCRIPT}'"
