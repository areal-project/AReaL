#!/bin/bash
#SBATCH --job-name=yl-bcb-val-gate
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/bcb_val_gate_%j.log
#SBATCH --error=logs/bcb_val_gate_%j.log
...
echo "=========================================="
echo "BCB Val-Only: Retrieval Gate Evaluation"
...
INNER_SCRIPT=$(mktemp /tmp/bcb_val_gate_XXXXXX.sh)
...
TEMP_CONFIG=/tmp/bcb_val_gate_config_$$.yaml
...
OUTPUT_DIR="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/training/deepseek_region_confgate_5gpu/val_gate_eval"

echo '=========================================='
echo 'Running Val-Only: Retrieval Gate (success_ratio < 0.8 → drop memory)'
echo '=========================================='

# Run 1: Gate with success_ratio < 0.8
python run/run_bcb_region.py \
    ...
    --eval_only \
    --output_dir "$OUTPUT_DIR/gate_success_ratio_08"

echo "[INFO] Gate success_ratio<0.8 exit code: $?"

echo '=========================================='
echo 'Running Val-Only: Retrieval Gate (mean_q < 0.4 → drop memory)'
echo '=========================================='

# Run 2: Gate with mean_q < 0.4
python run/run_bcb_region.py \
    --config "$TEMP_CONFIG" \
    --split instruct --subset full --epochs 1 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    ...
    --region_smoothing_C 0.5 --propagation_eta 0.12 --propagation_k 30 \
    --propagation_sim_min 0.40 --shrinkage_confidence_k 3.0 --val_lambda_max 0.15 \
    --retrieval_gate_signal mean_q --retrieval_gate_threshold 0.4 \
    --resume_from "$CHECKPOINT" \
    --eval_only \
    --output_dir "$OUTPUT_DIR/gate_mean_q_04"

echo "[INFO] Gate mean_q<0.4 exit code: $?"

echo '=========================================='
echo 'Running Val-Only: No gate (baseline with-mem, same checkpoint)'
echo '=========================================='

# Run 3: No gate baseline (for fair comparison on same run)
python run/run_bcb_region.py \
    --config "$TEMP_CONFIG" \
    --split instruct --subset full --epochs 1 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    ...
    --resume_from "$CHECKPOINT" \
    --eval_only \
    --output_dir "$OUTPUT_DIR/no_gate_baseline"

echo "[INFO] No-gate baseline exit code: $?"
