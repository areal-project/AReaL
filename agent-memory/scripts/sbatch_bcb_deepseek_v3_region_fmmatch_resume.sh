#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-fmm2
...
#SBATCH --output=logs/bcb_dsv3_reg_fmmatch_%j.log
#SBATCH --error=logs/bcb_dsv3_reg_fmmatch_%j.log

# Combined job:
#   Part 1: Resume fmmatch E10 from step 304 (complete the old experiment)
#   Part 2: fmmatch + backfill (new experiment, 10 epochs from scratch)
...
RESUME_FROM="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fmmatch/bigcodebench_eval/instruct_full/region/20260624_152627_deepseek-ai_DeepSeek-V3_region/epoch10/snapshot/step_304"

echo "=========================================="
echo "BCB DeepSeek-V3 fmmatch: Resume E10 + fmmatch_backfill fresh"
...
# ============================================================
# Part 1: Resume fmmatch E10 from step 304
# ============================================================
echo ''
echo '=========================================='
echo '[Part 1] fmmatch RESUME: E10 from step 304'
echo '=========================================='
run_in_runner "python run/run_bcb_region.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    --resume_from '${RESUME_FROM}' \
    --resume_epoch 10 --resume_step 304 \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fmmatch"
PART1_EXIT=$?
echo "[INFO] Part 1 (resume) exit code: $PART1_EXIT"

# ============================================================
# Part 2: fmmatch + backfill (fresh 10 epochs)
# ============================================================
echo ''
echo '=========================================='
echo '[Part 2] fmmatch + BACKFILL: 10 epochs from scratch'
echo '=========================================='
run_in_runner "python run/run_bcb_region.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    ...
    --failure_summary_fmmatch \
    --failure_summary_fmmatch_topk 5 \
    --failure_summary_fmmatch_backfill \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fmmatch_backfill"
PART2_EXIT=$?
echo "[INFO] Part 2 (fmmatch+backfill) exit code: $PART2_EXIT"

# Cleanup servers.
kill $EMBED_PID $VLLM_PID 2>/dev/null
wait $EMBED_PID 2>/dev/null
wait $VLLM_PID 2>/dev/null
echo "[INFO] Servers stopped. Part1=$PART1_EXIT, Part2=$PART2_EXIT"
