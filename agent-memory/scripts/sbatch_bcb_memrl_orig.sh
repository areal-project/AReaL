...
#SBATCH --output=logs/bcb_memrl_orig_%j.log
#SBATCH --error=logs/bcb_memrl_orig_%j.log

MEMRL_DIR="/storage/openpsi/users/yl/agent-memory/MemRL"
SINGULARITY_IMG="/storage/openpsi/images/areal-latest.sif"

echo "=========================================="
echo "BCB MemRL (git original config, 10 epochs)"
...
singularity exec --nv --no-home --writable-tmpfs \
    ...
    bash -c "
cd ${MEMRL_DIR}
echo '[INFO] Starting BCB MemRL (git original config) on \$(hostname)'
...
echo '[INFO] Starting BCB MemRL (git original config, 10 epochs, 1140 tasks)...'
python run/run_bcb.py --config configs/rl_bcb_config.git_original.yaml --split instruct --subset full --epochs 10 --eval_timeout 240 --untrusted_hard_timeout 300 --checkpoint_interval 50 --max_checkpoints 3
echo '[INFO] BCB MemRL (git original) completed!'
"
