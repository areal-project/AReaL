#!/bin/bash
# LLB DB MeMp baseline on AIStudio — gpt-4.1-mini
# Proceduralization + pure sim retrieval (no Q, no RL). 10 sections.
set -e
...
LOGFILE=${PROJECT_DIR}/logs/llb_db_memp_gpt41mini_${HOST_SHORT}_${TS}.log
mkdir -p ${PROJECT_DIR}/logs
exec > >(tee -a $LOGFILE) 2>&1

echo "=========================================="
echo "MeMp - LLB DB (gpt-4.1-mini)"
echo "Host: $(hostname)"
...
export MEMRL_RUN_ID=memp-db-gpt41mini-20260716
...
echo '[INFO] Running LLB DB MeMp baseline (gpt-4.1-mini)...'
