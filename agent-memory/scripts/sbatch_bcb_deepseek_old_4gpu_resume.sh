#!/bin/bash
#SBATCH --job-name=yl-bcb-old-4gpu
...
#SBATCH --mem=1000G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-23,slurmd-45
#SBATCH --output=logs/bcb_ds_old_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_old_4gpu_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Old Config, 4 GPU)"
...
sed \"s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g\" configs/rl_bcb_config.deepseek_old_bs1.yaml > \$TEMP_CONFIG
echo \"[INFO] Using ports: LLM=${LLM_PORT}, Embed=${EMBED_PORT}\"

# --- Start embedding vLLM on GPU 4 (dedicated, no sharing with LLM) ---
...
# --- Start LLM vLLM on GPUs 0-3 (TP=4), dedicated GPUs (embed on GPU 4) ---
...
# --- Run MemRL experiment with old config (bs=1) ---
echo ''
echo '=========================================='
echo '[MEMRL-OLD] Starting BCB MemRL with old config (10 epochs, bs=1)'
...
python run/run_bcb.py \
    ...
    --strip_think \
    --resume_from ./results/deepseek_memrl_old_4gpu/bigcodebench_eval/instruct_full/memory/20260519_185351_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_rl-on/epoch2/snapshot/step_650 \
    --resume_epoch 2 \
    --resume_step 650 \
    --output_dir ./results/deepseek_memrl_old_4gpu

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old exited with code: \$MEMRL_EXIT\"
