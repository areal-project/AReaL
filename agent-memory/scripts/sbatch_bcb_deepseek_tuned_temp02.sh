#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-tuned-t02
...
#SBATCH --output=logs/bcb_ds_tuned_temp02_%j.log
#SBATCH --error=logs/bcb_ds_tuned_temp02_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Tuned params, temp=0.2)"
...
# --- Run MemRL experiment with tuned parameters + temp=0.2 ---
echo ''
echo '=========================================='
echo '[MEMRL-TUNED-TEMP02] Starting BCB MemRL with tuned parameters + temperature 0.2'
echo 'Changes: tau=0.30, proceduralization, query, sim_norm=0.31/0.10, temp=0.2'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_tuned_temp02.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_memrl_tuned_temp02

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Tuned-Temp02 exited with code: \$MEMRL_EXIT\"
