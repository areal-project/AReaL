#!/bin/bash
# Qwen3.6 ALFWorld Mem0: GPU0 embed, GPU1 action(+reasoning), GPU2 extraction(no reasoning)
set -euo pipefail
...
export MEMRL_RUN_ID=qwen36_mem0_v1
...
PYFASTEMBED
MEM0_SNAPSHOT="${MEM0_CKPT}/snapshot/s3_b10"
...
print(f"[CHECKPOINT] Mem0 snapshot s3_b10 valid; global_step={state.get('global_step')}")
PYCHECKPOINT
SERVER_PIDS=()
...
experiment:
  ...
  ckpt_resume_path: ${MEM0_CKPT}/snapshot/s3_b10
...
echo "[RESUME] fixed Mem0 checkpoint: ${MEM0_SNAPSHOT}"
