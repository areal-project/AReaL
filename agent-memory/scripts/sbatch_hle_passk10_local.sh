#!/bin/bash
#SBATCH --job-name=yl-hle-passk10
...
#SBATCH --output=logs/hle_passk10_%j.log
#SBATCH --error=logs/hle_passk10_%j.log

# HLE Pass@10 baseline: 10 independent attempts per question, no memory
# Cumulative SR = fraction solved in at least 1 of 10 tries
# Expected: pass@10 > pass@1 (21.4%), provides upper bound for no-memory approach
...
echo "=========================================="
echo "HLE Pass@10 Baseline: deepseek-v3.2 via SGLang"
...
for i in $(seq 1 5400); do
    ...
    if [ $i -eq 5400 ]; then
        echo "[ERROR] SGLang server failed to start after 5400s"
...
# 3) Run pass@10 baseline
echo ''
echo '[BASELINE] HLE Pass@10 (10 independent attempts, no memory)'
singularity exec --nv --no-home --writable-tmpfs \
    --bind /storage:/storage \
    $RUNNER_IMG \
    bash -c "
...
python run/run_hle.py \
    --config configs/rl_hle_config.passk10_local.yaml \
...
"
PASSK10_EXIT=$?
echo "[INFO] Pass@10 baseline exited with code: $PASSK10_EXIT"

# Cleanup
kill $VLLM_PID 2>/dev/null
wait $VLLM_PID 2>/dev/null
echo "[INFO] Done. Exit: $PASSK10_EXIT"
