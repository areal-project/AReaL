...
# ON_FAILURE retries early platform/container failures as well as application-level
# failures. The stable MEMRL_RUN_ID makes every retry resume the existing snapshot
# lineage instead of starting a second experiment from Section 1.
rs = RetryStrategy(retry_policy=RetryPolicy.ON_FAILURE, max_attempt=5)
km_conf = KMConf(
...
_default_job_name = f"yl-llb-db-regionfs-final-fair-recover-{_time.strftime('%m%d-%H%M')}"
