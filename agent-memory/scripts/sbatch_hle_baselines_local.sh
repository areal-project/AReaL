#!/bin/bash
#SBATCH --job-name=yl-hle-sglang-v32
...
#SBATCH --output=logs/hle_sglang_v32_%j.log
#SBATCH --error=logs/hle_sglang_v32_%j.log
...
echo "=========================================="
echo "HLE Baselines (no-memory + MemRL) serial: deepseek-v3.2 via SGLang"
...
#    No mem packages needed here -- pure serving.
# ============================================================
...
# ============================================================
# 3) Run the experiment in the RUNNER image (mem packages here).
# ============================================================
run_in_runner() {
    singularity exec --nv --no-home --writable-tmpfs \
        --bind /storage:/storage \
...
# --- Baseline 1: no-memory (pass@1) — SKIPPED ---
# no-memory baseline already completed (21.4% SR, saved at
# .../yl-mem-region/hle/exp_hle_nomem_local_20260618-202820).
# sim_threshold / num_sections changes do NOT affect no-memory, so we reuse it
# and only run MemRL here.
NOMEM_EXIT=0

# --- Baseline 2: MemRL (memory, no region) ---
echo ''
echo '[BASELINE] HLE MemRL (memory, no region)'
run_in_runner "python run/run_hle.py \
    --config configs/rl_hle_config.memrl_local.yaml \
...
MEMRL_EXIT=$?
echo "[INFO] MemRL baseline exited with code: $MEMRL_EXIT"

# Cleanup vLLM server
kill $VLLM_PID 2>/dev/null
wait $VLLM_PID 2>/dev/null
echo "[INFO] vLLM server stopped. no-mem exit: $NOMEM_EXIT | MemRL exit: $MEMRL_EXIT"
