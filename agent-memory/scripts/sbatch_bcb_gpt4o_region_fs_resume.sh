#!/bin/bash
#SBATCH --job-name=yl-bcb-gpt4o-regfs-r
...
#SBATCH --mem=380G
#SBATCH --gres=gpu:0
#SBATCH --output=logs/bcb_gpt4o_regfs_resume_%j.log
#SBATCH --error=logs/bcb_gpt4o_regfs_resume_%j.log

# BCB Region + Failure Summary on gpt-4o-2024-11-20 + text-embedding-3-large (both API).
# Control-variable vs V3+Qwen3-Embedding region run: stronger LLM + stronger embedding.
# gpt-4o does NOT trigger the 428 content filter; no local GPU needed (pure API).
...
echo "=========================================="
echo "BCB Region+FS: gpt-4o + text-embedding-3-large (API, no GPU)"
echo "Job ID: $SLURM_JOB_ID | Node: $SLURMD_NODENAME | Start: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/bcb_gpt4o_region_XXXXXX.sh)
...
echo '=========================================='
echo '[Region+FS] gpt-4o, 10 epochs, failure_summary_n_slots=1'
echo '=========================================='
python run/run_bcb_region.py \
    ...
    --resume_from '/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/gpt4o_region_fs/bigcodebench_eval/instruct_full/region/20260621_151127_gpt-4o-2024-11-20_region/epoch5/snapshot/step_304' \
    --resume_epoch 5 \
    --resume_step 304 \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/gpt4o_region_fs

EXIT_CODE=$?
echo "[INFO] Region+FS exit code: $EXIT_CODE"
exit $EXIT_CODE
INNEREOF
