...
echo "=========================================="
echo "ALFWorld opus-4-7 Region+FS trajectory-resume (AIStudio)"
echo "Start: $(date)"
echo "=========================================="
...
export MEMRL_ALFWORLD_LLM_CONCURRENCY=16
...
TRAJ_CONFIG="$TMPDIR/rl_alf_config.opus47_region_traj_resume.yaml"
sed 's/build_strategy: "proceduralization"/build_strategy: "trajectory"/' \
    configs/rl_alf_config.opus47_region.yaml > "$TRAJ_CONFIG"
echo "[INFO] Effective memory build_strategy: $(grep build_strategy "$TRAJ_CONFIG")"
echo "[INFO] ALFWorld LLM concurrency: $MEMRL_ALFWORLD_LLM_CONCURRENCY"
echo "[INFO] Failure summary slots: 1"
echo "[INFO] Starting ALFWorld opus-4-7 Region+FS trajectory (auto-resume from latest ckpt)..."
