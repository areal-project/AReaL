#!/bin/bash
#SBATCH --job-name=yl-alf-qwen72b-baseline
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_qwen72b_baseline_%j.log
#SBATCH --error=logs/alf_qwen72b_baseline_%j.log
...
echo "=========================================="
echo "ALFWorld BASELINE (no region): Qwen2.5-72B-Instruct (TP=4) + Qwen3-Embedding-8B (local) [2-section]"
...
TEMP_CONFIG=/tmp/alf_qwen72b_baseline_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" configs/rl_alf_config.qwen72b_baseline_2section.yaml > "$TEMP_CONFIG"
...
# --- Run experiment ---
echo '=========================================='
echo '[ALFWorld-Qwen72B-Baseline-2section] Qwen2.5-72B-Instruct + text-embedding-3-large (API)'
echo 'Task types: 7 natural subtasks, max_steps=30'
echo '=========================================='

python run/run_alfworld.py \
    --config "$TEMP_CONFIG" --skip_initial_eval &
MAIN_PID=$!
...
echo "[INFO] ALFWorld Qwen72B baseline exited with code: $ALF_EXIT"
