#!/bin/bash
# Submit repaired Region+FS E10 strict-control eval (val_lambda_max=0.60) to AIS.
set -euo pipefail
...
export WORKER_NUM="0"
export JOB_TAG="qwen72b-region-fs-e10-vlmax060"
TS=$(date +%Y%m%d_%H%M%S)
export JOB_NAME="yl-alf-q72b-rfs-e10-v060-${TS}"
export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/aistudio_alfworld_qwen72b_region_fs_vlmax060.sh ${TS}"
export GPU_NUM=3
