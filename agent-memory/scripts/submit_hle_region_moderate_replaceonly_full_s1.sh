...
RID="yl-hle-region-moderate-replaceonly-$(date +%Y%m%d-%H%M%S)"
...
echo "[submit] HLE full Moderate Region+FS replace-only, clean historical MemRL-proceduralization S1 source: $SOURCE_CKPT"
REMOTE="/tmp/${RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
