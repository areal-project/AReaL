#!/bin/bash
# Submit LLB OS baselines MemP+RAG (serial in one job, one GPU) to AIStudio.
# HOLD: submit after MemRL + pass@10 finish.
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbos-memp-rag-haiku-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_os_baselines_memp_rag_aistudio.sh"
