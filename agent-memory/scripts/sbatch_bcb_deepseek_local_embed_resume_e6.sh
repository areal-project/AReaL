#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-resume-e6
...
#SBATCH --output=logs/bcb_ds_local_embed_resume_e6_%j.log
#SBATCH --error=logs/bcb_ds_local_embed_resume_e6_%j.log
...
LLM_PORT=8000
EMBED_PORT=8001
RESUME_FROM="./results/deepseek_memrl_local_embed/bigcodebench_eval/instruct_full/memory/20260507_142810_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_rl-on/epoch6/snapshot/step_250"

echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Resume from epoch 6 step 250)"
...
# --- Run MemRL experiment (resume from epoch 6 step 250) ---
echo ''
echo '=========================================='
echo '[MEMRL] Resuming BCB MemRL from epoch 6 step 250 (10 epochs total)'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_local_embed.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_memrl_local_embed \
