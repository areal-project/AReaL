#!/bin/bash
# Qwen2.5-72B ALFWorld Region+FS Cell A rerun after split-posterior fix.
...
OUTPUT_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_regionfs_cell_a_splitfix_20260722"
LOG_ROOT="$MEMRL_DIR/logs/alfworld_regionfs_qwen72b_cell_a_splitfix_${RUN_TAG}"
DRIVER_LOG="$LOG_ROOT/driver.log"
WORK_TMP_BASE="/storage/openpsi/users/yl/agent-memory/.tmp"
WORK_TMP="$WORK_TMP_BASE/qwen72b_regionfs_cell_a_splitfix_${RUN_TAG}_$$"
...
echo "================================================================"
echo "Qwen2.5-72B Region+FS Cell A rerun with split-posterior fix"
...
echo "Split fix: inherit parent prior alpha/beta; partition only actual posterior evidence"
echo "One cell: full 10 epochs from empty memory; FS slots=1"
echo "Logs: $LOG_ROOT"
...
assert RegionManager.__init__.__defaults__[-1] == "soft_source_conserving"
...
run_cell cell_a eta0p03_splitfix 0.03 2.5 0.60 0.45 0.55

echo "[ALL-DONE] Region+FS Cell A split-fix rerun completed at $(date --iso-8601=seconds)"
