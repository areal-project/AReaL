#!/bin/bash
#SBATCH --job-name=yl-bcb-old-add-old-4gpu
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:4
#SBATCH --exclude=slurmd-24,slurmd-45
#SBATCH --output=logs/bcb_ds_old_region_additive_old_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_old_region_additive_old_4gpu_%j.log
...
# Dynamic ports based on job ID to avoid conflicts
LLM_PORT=$((8000 + ($SLURM_JOB_ID % 100) * 2))
EMBED_PORT=$((LLM_PORT + 1))

echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Old Config + Additive OLD, 4 GPU)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "GPU Config: Embedding TP=4 + LLM TP=4 on GPUs 0-3"
echo "Additive: OLD version (EMA utility, default params)"
echo "=========================================="
...
TEMP_CONFIG=/tmp/region_config_\$\$.yaml
sed \"s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g\" configs/rl_bcb_config.deepseek_old_region_bs1.yaml > \$TEMP_CONFIG
echo \"[INFO] Using ports: LLM=${LLM_PORT}, Embed=${EMBED_PORT}\"

# --- Start embedding vLLM (TP=4, spread across all 4 GPUs, minimal memory) ---
echo '[INFO] Starting embedding vLLM (Qwen3-Embedding-8B, TP=4)...'
...
# --- Start LLM vLLM (TP=4, uses all 4 allocated GPUs) ---
echo '[INFO] Starting LLM vLLM (DeepSeek-R1-32B, TP=4)...'
python -m vllm.entrypoints.openai.api_server \
    --model ${MODEL_PATH} \
...
# --- Run MemRL experiment with old config + additive OLD (bs=1) ---
echo ''
echo '=========================================='
echo '[MEMRL-OLD-ADDITIVE-OLD-RESUME] Resuming from E1 step 400'
echo 'Config: tau=0.18, trajectory, avefact, sim_norm=0.186/0.094'
echo 'Region: Additive gating, EMA utility, min_cluster_size=15'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --resume_from ./results/deepseek_old_region_additive_old_4gpu/bigcodebench_eval/instruct_full/region/20260519_111427_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_region/epoch1/snapshot/step_400 \
    --resume_epoch 1 \
    --resume_step 400 \
    --output_dir ./results/deepseek_old_region_additive_old_4gpu

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old-Additive-Old exited with code: \$MEMRL_EXIT\"
