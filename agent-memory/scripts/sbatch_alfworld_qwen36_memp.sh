#!/bin/bash
#SBATCH --job-name=yl-alf-qwen36-memp
...
#SBATCH --output=logs/alf_qwen36_memp_%j.log
#SBATCH --error=logs/alf_qwen36_memp_%j.log

# Qwen3.6 MemP resume from S1 checkpoint (S1 train done, eval crashed)
...
CKPT_PATH="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld/alfworld/exp_alfworld_memp_qwen36_20260628-205709/local_cache"

echo "=========================================="
echo "ALFWorld Qwen3.6: MemP resume from S1 → eval + S2+"
...
experiment:
  ...
  ckpt_resume_path: /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld/alfworld/exp_alfworld_memp_qwen36_20260628-205709/local_cache
...
rl_config:
  ...
  weight_q: 0.5
EOF
echo "[MemP] Resuming from S1 checkpoint"
python run/run_alfworld.py --config /tmp/memp_resume.yaml --skip_initial_eval
