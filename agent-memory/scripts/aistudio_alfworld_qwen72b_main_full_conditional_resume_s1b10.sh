#!/bin/bash
# Latest region-dev Full conditional RFS continuation from complete s1_b10.
...
OUT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_qwen72b_main_full_conditional_resume_s1b10_20260810
RESUME_ROOT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_qwen72b_main_full_conditional_latest_20260810/alfworld/exp_alfworld_qwen72b_main_full_conditional_latest_20260810_171437_qwen72b-main-full-conditional-latest-20260810-171437/local_cache
LOG="$MEMRL_DIR/logs/alfworld_qwen72b_main_full_conditional_resume_s1b10_$TAG"
WORK_TMP=/storage/openpsi/users/yl/agent-memory/.tmp/q72b_main_full_conditional_resume_s1b10_${TAG}_$$
...
experiment:
  ...
  experiment_name: alfworld_qwen72b_main_full_conditional_resume_s1b10_${TAG}
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
YAML
export MEMRL_RUN_ID="qwen72b-main-full-conditional-resume-s1b10-${SAFE}"
echo "[RESUME-PREFLIGHT] source=${RESUME_ROOT}/snapshot/s1_b10 expected_start=section1_batch11"
python3 - <<PYRESUME
import json,pathlib
p=pathlib.Path("${RESUME_ROOT}/snapshot/s1_b10")
m=json.loads((p/"snapshot_meta.json").read_text())
assert m.get("checkpoint_id")=="s1_b10",m
assert m.get("visible_memories")==1280,m
for rel in ("cube/textual_memory.json","local_cache/cum_state.json","local_cache/region_manager.json","local_cache/q_cache.json"):
 q=p/rel;assert q.is_file() and q.stat().st_size>10,q
print("[OK] complete s1_b10 resume checkpoint",p)
PYRESUME
echo "[ARM] full_conditional latest_source=true resume=s1_b10 capability=true shrinkage=true FS=1 force_recall=false run_id=$MEMRL_RUN_ID"
