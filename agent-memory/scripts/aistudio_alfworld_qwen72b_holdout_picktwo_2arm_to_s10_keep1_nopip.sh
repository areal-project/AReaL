#!/bin/bash
# Hard ALFWorld holdout continuation: MemRL/Self-RAG only, resume from last healthy checkpoints through S10.
# GPU0 embed; GPU1-2 MemRL; GPU3-4 Self-RAG.
set -euo pipefail
...
EP="${EMBED_PORT:-20190}"; MP="${MEMRL_PORT:-20290}"; SP="${SELFRAG_PORT:-20390}"; NCCL_MEMRL="${NCCL_MEMRL:-30690}"; NCCL_SELFRAG="${NCCL_SELFRAG:-30790}"
...
CUDA_VISIBLE_DEVICES=1,2 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port $MP --context-length 32768 --trust-remote-code --nccl-port "$NCCL_MEMRL" & PIDS+=("$!")
CUDA_VISIBLE_DEVICES=3,4 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port $SP --context-length 32768 --trust-remote-code --nccl-port "$NCCL_SELFRAG" & PIDS+=("$!")
...
wait_server embed $EP; wait_server memrl $MP; wait_server selfrag $SP
...
echo "[ALL-DONE] memrl=$ra selfrag=$rb"; [[ $ra -eq 0 && $rb -eq 0 ]]
