#!/bin/bash
# Main-experiment ablation arm: Category-Q only. Full train 3553, ID 140, OOD 134.
...
OUT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_qwen72b_main_region_ablation_category_q_20260808
LOG="$MEMRL_DIR/logs/alfworld_qwen72b_main_region_ablation_category_q_$TAG"
WORK_TMP=/storage/openpsi/users/yl/agent-memory/.tmp/q72b_main_region_ablation_category_q_${TAG}_$$
...
wait_server embed "$EP"; wait_server category_q "$LP"
export TMPDIR="$WORK_TMP" TMP="$WORK_TMP" TEMP="$WORK_TMP"
CFG="$WORK_TMP/category_q.yaml"
cat > "$CFG" <<YAML
...
experiment:
  ...
  experiment_name: alfworld_qwen72b_ablation_category_q_${TAG}
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
YAML
export MEMRL_RUN_ID="qwen72b-main-ablation-category-q-${SAFE}"
echo "[ARM] category_q value_mode=category_q cluster_space=capability RFS=0 run_id=$MEMRL_RUN_ID"
python3 "$OVERLAY/run/run_alfworld.py" --config "$CFG" \
  ...
  --region_value_mode category_q --region_cluster_space capability \
  2>&1 | tee -a "$LOG/category_q.log"
echo "[ALL-DONE] category_q=0 $(date -Is)"
