...
RID="yl-hle-group1-threeway-$(date +%Y%m%d-%H%M%S)"
RAG_CKPT=$(python3 "$ROOT/MemRL/scripts/select_latest_hle_checkpoint.py" --glob '/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_group1_rag_*/snapshot/*' --max-no-id 0)
...
echo "[submit] selected RAG checkpoint: $RAG_CKPT"
...
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_group1_threeway_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
STATE="/tmp/hle_group1_${HLE_RID}"
mkdir -p "$STATE"
SITE="/tmp/hle_group1_site_${HLE_RID}"
...
export MEMRL_LLM_GLOBAL_MAX_INFLIGHT=64 MEMRL_LLM_INFLIGHT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/.llm_inflight MEMRL_LLM_INFLIGHT_KEY=hle-group1-gemini35flash
python3 -c "import hdbscan; print('[HLE group1] HDBSCAN_PRECHECK_OK', getattr(hdbscan, '__version__', 'installed'))"
echo "[HLE group1] rid=$HLE_RID RAG=1.5s@0m SelfRAG=2.0s@5m RegionTraj=2.0s@10m update_workers=4 global_queue=disabled shared_inflight=64"
launch(){
 ...
 (
  ...
  echo "[HLE group1 child] mode=$mode delay=${delay}s interval=${interval}s ckpt=$ckpt update_workers=$MEMRL_UPDATE_MAX_WORKERS"
  python3 - <<'PY'
import os,yaml
...
PY
  ...
  if [ "$mode" = selfrag ]; then
    ...
    echo "[HLE group1 region] HDBSCAN_REQUIRED=true checkpointV2=enabled"
...
launch rag 0 1.5 "$RAG_CKPT" "${HLE_RID}-rag"
launch selfrag 300 2.0 "$SELFRAG_CKPT" "${HLE_RID}-selfrag"
launch regiontraj 600 2.0 "$REGION_CKPT" "${HLE_RID}-regiontraj"
status=0
...
export JOB_COMMAND="HLE_RID=$(printf '%q' "$RID") RAG_CKPT=$(printf '%q' "$RAG_CKPT") SELFRAG_CKPT=$(printf '%q' "$SELFRAG_CKPT") REGION_CKPT=$(printf '%q' "$REGION_CKPT") bash -c $(printf '%q' "echo $PAYLOAD | base64 -d | bash")"
