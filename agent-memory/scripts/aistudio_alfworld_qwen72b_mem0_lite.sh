#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld Mem0 — lightweight version (3x H200)
# Game interaction and Mem0 fact extraction share the same LLM server.
# If extraction failures occur, switch to aistudio_alfworld_qwen72b_mem0.sh (5 GPU).
#
# GPU layout:
#   GPU 0: Qwen3-Embedding-8B
#   GPU 1-2: Qwen2.5-72B TP=2 → game + extraction (shared)
# ============================================================================
set -e
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_mem0_lite_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B Mem0 lite (3 GPU, shared LLM)"
echo "Start: $(date)"
echo "=========================================="
...
cat > /tmp/alf_mem0_lite_qwen72b_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
echo "[EXP] 72B Mem0 (lite, shared LLM) starting..."
python3 run/run_alfworld.py --config /tmp/alf_mem0_lite_qwen72b_$$.yaml --mem0 --skip_initial_eval
