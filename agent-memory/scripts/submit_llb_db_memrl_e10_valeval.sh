...
# Load the existing submission credential without printing it.
...
export JOB_NAME="yl-llbdb-memrl-e10-valeval-$(date +%m%d-%H%M)"
export JOB_COMMAND='bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_memrl_e10_valeval_aistudio.sh'
...
printf '%s\n' "[INFO] Submitting read-only E10 validation job: $JOB_NAME"
cd /tmp
