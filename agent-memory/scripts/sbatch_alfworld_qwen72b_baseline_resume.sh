#!/bin/bash
#SBATCH --job-name=yl-alf-qwen72b-baseline-resume
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_qwen72b_baseline_resume_%j.log
#SBATCH --error=logs/alf_qwen72b_baseline_resume_%j.log
...
echo "=========================================="
echo "ALFWorld BASELINE RESUME from 927704 snapshot/2 → s10"
...
TEMP_CONFIG=/tmp/alf_qwen72b_baseline_resume_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" configs/rl_alf_config.qwen72b_baseline_resume.yaml > "$TEMP_CONFIG"
...
# --- Run experiment ---
echo '=========================================='
echo '[ALFWorld-Qwen72B-Baseline-RESUME-s10] Qwen2.5-72B-Instruct + text-embedding-3-large (API)'
...
echo "[INFO] ALFWorld Qwen72B baseline RESUME exited with code: $ALF_EXIT"
