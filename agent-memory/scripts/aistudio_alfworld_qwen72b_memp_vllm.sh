#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld MemP only (vLLM, 3x H200)
# proceduralization build, pure similarity retrieval, no Q-value
#
# GPU layout:
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_memp_vllm_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B MemP (vLLM, 3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
echo "[EXP] 72B MemP (vLLM) starting..."
python3 run/run_alfworld.py --config /tmp/alf_memp_vllm_$$.yaml --skip_initial_eval
