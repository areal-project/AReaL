#!/bin/bash
# LLB DB Region+FS hard-membership fix Section-3 resume on AIStudio — gpt-4.1-mini
...
LOGFILE=${PROJECT_DIR}/logs/llb_db_region_fs_gpt41mini_hardfix_s3_${HOST_SHORT}_${TS}.log
mkdir -p ${PROJECT_DIR}/logs
exec > >(tee -a $LOGFILE) 2>&1

echo "=========================================="
echo "Region+FS HARD-FIX S3 - LLB DB (gpt-4.1-mini)"
echo "Host: $(hostname)"
...
export MEMRL_RUN_ID=region-fs-db-gpt41mini-hardfix-s3-rerun-${TS}
...
python3 run/run_llb.py \
    --config configs/rl_llb_db_region_fs_hardfix_s3.yaml \
