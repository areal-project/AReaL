...
SOURCE_SNAPSHOT_ROOT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld/alfworld/exp_alfworld_region_opus47_20260623-100806/local_cache/snapshot
...
BRANCH_NAME=alfworld_region_opus47_s8_full_peta006
LOGFILE="$MEMRL_DIR/logs/aistudio_alf_opus47_s8_short_${RUN_TAG}.log"
...
export TMPDIR=/dev/shm/alf_opus47_s8_short TEMP=/dev/shm/alf_opus47_s8_short TMP=/dev/shm/alf_opus47_s8_short
mkdir -p "$TMPDIR"
...
export MEMRL_RUN_ID="s8-short-${RUN_TAG}"
...
cfg['experiment']['num_sections'] = 9       # resume S8, run one continuation section (S9)
cfg['experiment']['dataset_ratio'] = 1.0    # full 3553-task train split for paper-comparable Train SR
...
echo "[INFO] Full continuation epoch: 3553 train tasks (112 batches), trajectory, FS=1, concurrency=16"
echo "[INFO] Tuning: propagation_eta=0.06; corrected ID eval after S9; OOD disabled"
