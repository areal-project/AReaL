...
#SBATCH --output=logs/bcb_mem0_%j.log
#SBATCH --error=logs/bcb_mem0_%j.log

# BCB Mem0 baseline: mem0 library fact extraction + retrieval
# 10 epochs, uses Mem0MemoryService
...
LLM_PORT=8000
EMBED_PORT=8001
CONFIG=configs/rl_bcb_config.mem0_local.yaml

echo "=========================================="
echo "BCB Mem0 Baseline: DeepSeek-V3 via SGLang"
...
echo ''
echo '=========================================='
echo '[Mem0] DeepSeek-V3, mem0 library, 10 epochs'
echo '=========================================='
run_in_runner "python run/run_bcb.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    --mem0 --mem0_infer true \
    --checkpoint_interval 100 --max_checkpoints 3 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_mem0"
EXIT_CODE=$?
