#!/bin/bash
# Fresh pick-two Region-quality-fixed E1-E10.
...
EP="${EMBED_PORT:-22390}"; RP="${REGION_PORT:-22490}"; NCCL_REGION="${NCCL_REGION:-32890}"
OUT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_holdout_qwen72b_picktwo_region_quality_fixed_20260804
LOG="$MEMRL_DIR/logs/alfworld_holdout_picktwo_qwen72b_region_quality_fixed_$TAG"
WORK_TMP=/storage/openpsi/users/yl/agent-memory/.tmp/q72b_picktwo_region_quality_fixed_${TAG}_$$
...
experiment:
  ...
  experiment_name: alfworld_holdout_picktwo_qwen72b_region_quality_fixed_${TAG}
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
YAML
export MEMRL_RUN_ID="qwen72b-picktwo-region-quality-fixed-${SAFE}"
echo "[QUALITY-CONFIG] fresh=true holdout=pick_two train_expected=2740 holdout_expected=837 init_step=2500 precluster=soft_source_backfill:1.0 freeze_after_initial=true temperature=0.05 top_n=2 propagation=0.01/8/0.65 mem_cache=30000 FS_slots=1 tau=0.62 ws/wq=0.5/0.5 keep=1"
python3 "$OVERLAY/run/run_alfworld.py" \
  ...
  --region_freeze_after_initial_cluster \
