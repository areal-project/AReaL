...
class LocalDBContainer:
    ...
    def __init__(self, image: str = "mysql"):
        self.deleted = False
        datadir_root = os.environ.get("MEMRL_DB_DATADIR_ROOT")
        if datadir_root:
            os.makedirs(datadir_root, exist_ok=True)
            self._datadir = tempfile.mkdtemp(prefix="llb_mysql_", dir=datadir_root)
        else:
            self._datadir = tempfile.mkdtemp(prefix="llb_mysql_")

        # Shared storage is suitable for large DB files, but may not support
        # Unix sockets. Keep socket/pid files on node-local /tmp.
        socket_root = os.environ.get("MEMRL_DB_SOCKET_ROOT", "/tmp")
        os.makedirs(socket_root, exist_ok=True)
        self._socket_dir = tempfile.mkdtemp(prefix="llb_mysql_sock_", dir=socket_root)
        self._socket_path = os.path.join(self._socket_dir, "mysql.sock")
        self._pid_path = os.path.join(self._socket_dir, "mysqld.pid")
    ...
    def _start_mysqld(self) -> None:
        ...
        logger.info(
            "Started local mysqld on port %d, datadir=%s, socket_dir=%s",
            self.port,
            self._datadir,
            self._socket_dir,
        )
        ...
        if not self.deleted:
            ...
            try:
                ...
                try:
                    ...
                    pass
            # /storage is permanent shared data: never recursively delete it
            # or any DB datadir located under it from job cleanup code.
            datadir_real = os.path.realpath(self._datadir)
            if datadir_real == "/storage" or datadir_real.startswith("/storage/"):
                logger.info("Preserving MariaDB datadir on permanent storage: %s", self._datadir)
            else:
                shutil.rmtree(self._datadir, ignore_errors=True)

            socket_real = os.path.realpath(self._socket_dir)
            if socket_real == "/tmp" or socket_real.startswith("/tmp/"):
                shutil.rmtree(self._socket_dir, ignore_errors=True)
            else:
                logger.warning("Refusing to remove non-/tmp socket directory: %s", self._socket_dir)
            self.deleted = True
