...
RID="yl-hle-region-moderate-full-$(date +%Y%m%d-%H%M%S)"
...
echo "[submit] HLE full Moderate Region+FS, clean historical MemRL-proceduralization S1 source: $SOURCE_CKPT"
REMOTE="/tmp/${RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_moderate_full_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
SITE="/tmp/hle_region_moderate_full_site_${HLE_RID}"
rm -rf "$SITE"; mkdir -p "$SITE"
pip install --target "$SITE" -i https://pypi.antfin-inc.com/simple/ tensorboard pandas tqdm concurrent-log-handler hdbscan >/tmp/hle_region_moderate_full_deps.log 2>&1
...
SAFE_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_moderate_full_${HLE_RID}-regionmoderatefull"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_memrl_gemini35flash_20260707-115246/snapshot/1
...
export HLE_CKPT="$REGION_CKPT" CFG="/tmp/hle_region_moderate_full_${HLE_RID}.yaml"
echo "[HLE REGION MODERATE FULL S1 REGION+FS] rid=$HLE_RID FS_mode=hle_structured slots=1 independent_pool=true min_success=2 min_sim=0.70 signature=category+raw_subject min_evidence=3 abstain=original_region_top3 lambda_max=0.8 confidence_k=3 score_formula=unchanged"
...
cfg['memory'].update(build_strategy='proceduralization',user_id='hle_region_moderate_full_user')
cfg['experiment'].update(
    experiment_name='hle_regionfs_moderate_full',
...
export MEMRL_RUN_ID="${HLE_RID}-regionmoderatefull" MEMRL_REASONING_EFFORT=high MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_EMBED_MIN_INTERVAL=2.5
