#!/bin/bash
#SBATCH --job-name=yl-nomem-nofilter
#SBATCH --partition=all
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --output=logs/bcb_nomem_nofilter_%j.log
#SBATCH --error=logs/bcb_nomem_nofilter_%j.log

MEMRL_DIR="/storage/openpsi/users/yl/agent-memory/MemRL"
SINGULARITY_IMG="/storage/openpsi/images/areal-latest.sif"

echo "=========================================="
echo "BCB No-Memory NoFilter Baseline (750+300 clean tasks)"
...
singularity exec --nv --no-home --writable-tmpfs \
    ...
    bash -c "
cd ${MEMRL_DIR}
echo '[INFO] Starting BCB No-Memory NoFilter on \$(hostname)'
...
echo '[INFO] Starting BCB No-Memory NoFilter (git original config, retrieve_k=0, 1 epoch, 750 train + 300 val)...'
python run/run_bcb.py --config configs/rl_bcb_config.git_original.yaml --split instruct --subset full --epochs 1 --retrieve_k 0 --eval_timeout 240 --untrusted_hard_timeout 300 --checkpoint_interval 50 --max_checkpoints 3 --split_file configs/bigcodebench/splits/full_seed42_nofilter.json
echo '[INFO] BCB No-Memory NoFilter completed!'
"
