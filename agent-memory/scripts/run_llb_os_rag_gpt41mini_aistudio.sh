#!/bin/bash
# Run only the LLB OS RAG GPT-4.1-mini main experiment on AIStudio.
...
LOGFILE=${PROJECT_DIR}/logs/llb_os_rag_gpt41mini_${HOST_SHORT}_${TS}.log
mkdir -p "${PROJECT_DIR}/logs"
exec > >(tee -a "$LOGFILE") 2>&1

echo "=========================================="
echo "LLB OS RAG (GPT-4.1-mini, 10 sections + per-section validation)"
...
source "$PROJECT_DIR/scripts/llb_os_auto_resume.sh" "llb_os_rag_gpt41mini"
...
echo '[INFO] Starting GPT-4.1-mini RAG only...'
python3 run/run_llb.py --config configs/rl_llb_os_rag_gpt41mini.yaml

echo "=========================================="
echo "LLB OS RAG completed: $(date)"
echo "=========================================="
