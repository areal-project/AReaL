#!/bin/bash
#SBATCH --job-name=yl-bcb-old-region-add-4gpu
...
#SBATCH --output=logs/bcb_ds_old_region_additive_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_old_region_additive_4gpu_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Old Config + Region Additive, 4 GPU)"
...
# --- Start embedding vLLM on GPUs 0-3 (TP=4), port 8001 ---
echo '[INFO] Starting embedding vLLM (Qwen3-Embedding-8B, TP=4) on GPUs 0-3...'
...
# --- Run MemRL experiment with old config + region additive (bs=1) ---
echo ''
echo '=========================================='
echo '[MEMRL-OLD-REGION-ADDITIVE] Starting BCB MemRL (10 epochs, bs=1)'
echo 'Config: tau=0.18, trajectory, avefact, sim_norm=0.186/0.094'
echo 'Region: Additive gating'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --output_dir ./results/deepseek_old_region_additive_4gpu

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old-Region-Additive exited with code: \$MEMRL_EXIT\"
