#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld Region ONLY — resume from S9 (3x H200)
#
# GPU layout (3 GPUs):
#   GPU 0: Qwen3-Embedding-8B
#   GPU 1-2: Qwen2.5-72B TP=2 → Region (resume)
# ============================================================================
set -e
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_region_resume_${1:-$(date +%Y%m%d_%H%M%S)}.log"

EMBED_PORT=9000
LLM_PORT_1=9201

# Resume from the same run as the preempted job
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B Region resume (3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
# ============================================================================
# Region (same config as current run, auto-resume picks up latest snapshot)
# ============================================================================
cat > /tmp/alf_region_resume_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
experiment:
  ...
  experiment_name: alfworld_region_qwen72b
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
echo "[EXP] 72B Region resume starting..."
python3 run/run_alfworld.py \
    --config /tmp/alf_region_resume_$$.yaml \
