#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-tuned-rgn-post
...
#SBATCH --output=logs/bcb_ds_tuned_region_post_resume_%j.log
#SBATCH --error=logs/bcb_ds_tuned_region_post_resume_%j.log
...
# Resume from epoch 1 step 300 checkpoint
RESUME_SNAPSHOT="results/deepseek_tuned_region/bigcodebench_eval/instruct_full/region/20260516_104427_deepseek-ai_DeepSeek-R1-Distill-Qwen-32B_region/epoch1/snapshot/step_300"
RESUME_EPOCH=1
RESUME_STEP=300

echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Region Post + Tuned - RESUME)"
...
echo ''
echo '=========================================='
echo '[REGION-POST-RESUME] Resuming from epoch ${RESUME_EPOCH} step ${RESUME_STEP}'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --k_global 30 \
    --k_local 10 \
    --output_dir ./results/deepseek_tuned_region \
    --resume_from ${RESUME_SNAPSHOT} \
    --resume_epoch ${RESUME_EPOCH} \
    --resume_step ${RESUME_STEP}

REGION_EXIT=\$?
echo \"[INFO] Region-Post-Resume exited with code: \$REGION_EXIT\"
