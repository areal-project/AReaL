...
export JOB_TAG=""
TS=$(date +%Y%m%d-%H%M%S)
export JOB_NAME="${JOB_NAME:-yl-hle-rag-selfrag-resume1b9-${TS}}"
...
(
    ...
    python3 run/run_hle.py \
        --config configs/rl_hle_config.rag_gemini35flash_resume_1b9.yaml \
...
(
    ...
    python3 run/run_hle.py \
        --config configs/rl_hle_config.selfrag_gemini35flash_resume_1b9.yaml \
        --train data/hle/hle_test.parquet \
        --self_rag \
        --judge_model gpt-4o-2024-11-20 \
        --judge_base_url https://matrixllm.alipay.com/v1/ \
