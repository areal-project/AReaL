...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_structured_full_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"
echo '=========================================='
echo 'LLB DB Region+FS structured full 10-epoch training (gpt-4.1-mini)'
...
export MEMRL_RUN_ID=region-fs-db-gpt41mini-structured-full-20260802
...
export MEMRL_FINAL_MEMORY_DEDUP=1
...
cfg['memory']['user_id']='llb_db_region_fs_structured_full_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_structured_full'
...
cfg['experiment']['llb_dedup_by_task_id']=True
cfg['rl_config']['weight_sim']=0.7; cfg['rl_config']['weight_q']=0.3
dst.write_text(yaml.safe_dump(cfg,sort_keys=False))
PYCFG

printf '%s\n' '[INFO] Fresh corrected Region training with Region × SQL-signature structured FS.'
