#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-m8-r
...
#SBATCH --output=logs/bcb_dsv3_reg_mcs8_%j.log
#SBATCH --error=logs/bcb_dsv3_reg_mcs8_%j.log

# Resume Region+FS mcs=8 from E3 (job 934840 SGLang crash at E4 train 344/798)
...
RESUME_FROM="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_mcs8/bigcodebench_eval/instruct_full/region/20260628_164059_deepseek-ai_DeepSeek-V3_region/epoch3/snapshot/3"

echo "=========================================="
echo "BCB DeepSeek-V3 Region+FS mcs=8 RESUME from E3"
...
echo ''
echo '=========================================='
echo '[Region+FS mcs=8 RESUME] from E3, epochs 4-10'
echo '=========================================='
run_in_runner "python run/run_bcb_region.py \
    ...
    --region_temperature 0.1 --shrinkage_top_n 1 --region_min_cluster_size 8 \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_mcs8"
EXIT_CODE=$?
echo "[INFO] mcs=8 resume exit code: $EXIT_CODE"
