#!/bin/bash
#SBATCH --job-name=yl-alf-7b-global-fs
...
echo '[INFO] Building dataset with global failure summary...'
...
python examples/alfworld/dataset.py \
    ...
    --output_dir /tmp/areal/alfworld_dataset_global_fs \
    --split train \
    --failure_summary ${AREAL_DIR}/examples/alfworld/game_file_to_global_fs_paraphrase.json \
    --fs_ratio 0.5
...
python examples/alfworld/train.py \
    ...
    experiment_name=alfworld-7b-grpo-global-fs \
    train_dataset.path=/tmp/areal/alfworld_dataset_global_fs/combined \
    valid_dataset.path=/tmp/areal/alfworld_dataset_global_fs/combined \
