#!/bin/bash
#SBATCH --job-name=yl-bcb-gpt4o-leaf
...
#SBATCH --mem=64G
#SBATCH --gres=gpu:0
#SBATCH --output=logs/bcb_gpt4o_leaf_%j.log
#SBATCH --error=logs/bcb_gpt4o_leaf_%j.log

# BCB Region+FS with leaf clustering on gpt-4o + text-embedding-3-large (API).
# Same leaf params as V3 experiment: mcs=12, leaf, max_region_share=0.30.
...
echo "=========================================="
echo "BCB Region+FS leaf: gpt-4o + text-embedding-3-large (API, no GPU)"
echo "Job ID: $SLURM_JOB_ID | Node: $SLURMD_NODENAME | Start: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/bcb_gpt4o_leaf_XXXXXX.sh)
...
echo '[INFO] Installing full BCB eval-time deps...'
...
echo '=========================================='
echo '[Region+FS leaf] gpt-4o, 10 epochs'
echo '=========================================='
...
EXIT_CODE=$?
echo "[INFO] Region+FS leaf exit code: $EXIT_CODE"
exit $EXIT_CODE
INNEREOF
