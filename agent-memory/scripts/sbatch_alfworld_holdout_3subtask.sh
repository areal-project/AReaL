#!/bin/bash
#SBATCH --job-name=yl-alf-holdout-3sub
...
#SBATCH --output=logs/alf_holdout_3sub_%j.log
#SBATCH --error=logs/alf_holdout_3sub_%j.log

# 3-subtask holdout baseline: no-mem + memrl for each
# Subtasks: pick_heat, pick_two, look_at_obj_in_light
# 6 phases serial, shared vLLM. Expected ~30h.
...
echo "=========================================="
echo "ALFWorld 3-subtask holdout (heat/two/look): no-mem + memrl"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/alf_3sub_XXXXXX.sh)
...
# Patch all configs
for cfg in heat_nomem heat_memrl two_nomem two_memrl look_nomem look_memrl; do
  sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
      "configs/rl_alf_config.qwen72b_holdout_${cfg}.yaml" > "/tmp/alf_${cfg}_$$.yaml"
done
...
# ============ PHASE 1: pick_heat no-mem ============
echo "=========================================="
echo "[PHASE 1/6] pick_heat_then_place_in_recep — NO MEM"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_heat_nomem_$$.yaml" \
    --holdout_subtask alf/pick_heat_then_place_in_recep --holdout_eval_pools train,valid
echo "[PHASE 1] exit=$? at $(date)"

# ============ PHASE 2: pick_heat memrl ============
echo "=========================================="
echo "[PHASE 2/6] pick_heat_then_place_in_recep — MEMRL (2 section)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_heat_memrl_$$.yaml" \
    --holdout_subtask alf/pick_heat_then_place_in_recep --holdout_eval_pools train,valid \
    --skip_initial_eval
echo "[PHASE 2] exit=$? at $(date)"

# ============ PHASE 3: pick_two no-mem ============
echo "=========================================="
echo "[PHASE 3/6] pick_two_obj_and_place — NO MEM"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_two_nomem_$$.yaml" \
    --holdout_subtask alf/pick_two_obj_and_place --holdout_eval_pools train,valid
echo "[PHASE 3] exit=$? at $(date)"

# ============ PHASE 4: pick_two memrl ============
echo "=========================================="
echo "[PHASE 4/6] pick_two_obj_and_place — MEMRL (2 section)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_two_memrl_$$.yaml" \
    --holdout_subtask alf/pick_two_obj_and_place --holdout_eval_pools train,valid \
    --skip_initial_eval
echo "[PHASE 4] exit=$? at $(date)"

# ============ PHASE 5: look_at no-mem ============
echo "=========================================="
echo "[PHASE 5/6] look_at_obj_in_light — NO MEM"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_look_nomem_$$.yaml" \
    --holdout_subtask alf/look_at_obj_in_light --holdout_eval_pools train,valid
echo "[PHASE 5] exit=$? at $(date)"

# ============ PHASE 6: look_at memrl ============
echo "=========================================="
echo "[PHASE 6/6] look_at_obj_in_light — MEMRL (2 section)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py --config "/tmp/alf_look_memrl_$$.yaml" \
    --holdout_subtask alf/look_at_obj_in_light --holdout_eval_pools train,valid \
    --skip_initial_eval
echo "[PHASE 6] exit=$? at $(date)"
