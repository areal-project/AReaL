...
RID="yl-hle-region-proc-structuredfs-$(date +%Y%m%d-%H%M%S)"
...
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_proc_structuredfs_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
SITE="/tmp/hle_region_proc_structuredfs_site_${HLE_RID}"
rm -rf "$SITE"; mkdir -p "$SITE"
pip install --target "$SITE" -i https://pypi.antfin-inc.com/simple/ tensorboard pandas tqdm concurrent-log-handler hdbscan >/tmp/hle_region_proc_structuredfs_deps.log 2>&1
...
SAFE_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_proceduralization_structured_${HLE_RID}-regionprocstruct"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_memrl_gemini35flash_20260707-115246/snapshot/1
...
export HLE_CKPT="$REGION_CKPT" CFG="/tmp/hle_region_proc_structuredfs_${HLE_RID}.yaml"
echo "[HLE REGION PROC STRUCTURED FS] rid=$HLE_RID same_region_config=true FS_mode=hle_structured slots=1 independent_pool=true min_success=2 min_sim=0.50 signature=category+raw_subject min_evidence=2"
...
cfg['memory'].update(build_strategy='proceduralization',user_id='hle_region_proc_structuredfs_user')
cfg['experiment'].update(
    experiment_name='hle_regionfs_proceduralization_structured',
...
export MEMRL_RUN_ID="${HLE_RID}-regionprocstruct" MEMRL_REASONING_EFFORT=high MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_EMBED_MIN_INTERVAL=2.5
