#!/usr/bin/env bash
# Fresh DB main-method submission: no resume of any earlier final-fair lineage.
set -euo pipefail
export AISTUDIO_LOGIN_NAME=aistudio AISTUDIO_USERNUMBER=477578 WORKER_NUM=0 JOB_TAG='llb-db-regionfs-stable-fs1-fixed5'
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-stable-fs1-fixed5-${STAMP}"
export JOB_NAME="yl-llbdb-regionfs-stable-fs1-fixed5-${STAMP}"
export KM_IMAGE='acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401'
export JOB_COMMAND="export MEMRL_RUN_ID='${MEMRL_RUN_ID}'; bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_region_fs_stable_fs1_fixed5_aistudio.sh"
...
printf '%s\n' "[INFO] Submitting fresh stable-FS1-fixed5 DB Region-FS: $JOB_NAME"
printf '%s\n' "[INFO] MEMRL_RUN_ID=$MEMRL_RUN_ID (fresh; never resumes final-fair-20260807)"
cd /tmp
