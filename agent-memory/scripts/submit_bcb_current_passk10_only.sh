#!/bin/bash
# Current-serving Pass@10 on 798 train tasks. Round 1 is no-memory independent train SR.
set -euo pipefail
...
export JOB_NAME="yl-bcb-current-passk10-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.passk10_local.yaml
export BCB_EPOCHS=10
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_current_passk10
export BCB_ENTRYPOINT=run/run_bcb.py
export BCB_NO_AUTORESUME=1
export BCB_EXTRA_ARGS="--baseline_mode passk --baseline_k 10"
LOG_BASE="$MEMRL/logs/aistudio_bcb_current_passk10_$STAMP"
