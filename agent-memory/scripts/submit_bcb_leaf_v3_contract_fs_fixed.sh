#!/bin/bash
# Corrected Leaf-v3 rerun. On the first launch the new output root is empty;
# on a platform retry, strict runner must resume the latest *completed* epoch.
# It will ignore partial step snapshots and never read the prior invalid root.
set -euo pipefail
...
export JOB_NAME="yl-bcb-leafv3-contractfs-fixed-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
# Dedicated corrected root: no accidental read/write of the invalid Leaf-v3 state.
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_leaf_v3_contract_fs_fixed
...
LOG_BASE="$MEMRL/logs/aistudio_bcb_leafv3_contract_fs_fixed_$STAMP"
