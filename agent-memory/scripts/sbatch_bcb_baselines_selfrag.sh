#!/bin/bash
#SBATCH --job-name=yl-bcb-selfrag
...
#SBATCH --output=logs/bcb_selfrag_%j.log
#SBATCH --error=logs/bcb_selfrag_%j.log

# BCB Self-RAG baseline: RAG + LLM critique to filter irrelevant memories
# 10 epochs, trajectory storage, sim retrieval + LLM relevance filter
...
LLM_PORT=8000
EMBED_PORT=8001
CONFIG=configs/rl_bcb_config.selfrag_local.yaml

echo "=========================================="
echo "BCB Self-RAG Baseline: DeepSeek-V3 via SGLang"
...
echo ''
echo '=========================================='
echo '[Self-RAG] DeepSeek-V3, trajectory + LLM critique, 10 epochs'
echo '=========================================='
run_in_runner "python run/run_bcb.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    --self_rag --self_rag_inject_k 5 \
    --checkpoint_interval 100 --max_checkpoints 3 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_selfrag"
EXIT_CODE=$?
