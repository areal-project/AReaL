#!/bin/bash
#SBATCH --job-name=yl-bcb-gpt4o-regfs1
...
#SBATCH --mem=380G
#SBATCH --gres=gpu:0
#SBATCH --output=logs/bcb_gpt4o_regfs_bs1_%j.log
#SBATCH --error=logs/bcb_gpt4o_regfs_bs1_%j.log

# BCB Region+FS on gpt-4o-2024-11-20 + text-embedding-3-large (both API).
# Uses REPRODUCE baseline parameters (tau=0.38, sim_norm=0.31/0.10) for fair comparison.
# batch_size=1 to match memfix/compact baselines.
# Region+FS (no fmmatch): failure_summary uses region-aggregated summaries.
# min_cluster_size=5 + variance_weighted_dist for better region separation.

MEMRL_DIR="/storage/openpsi/users/yl/agent-memory/MemRL"
SINGULARITY_IMG="/storage/openpsi/images/areal-latest.sif"
CONFIG=configs/rl_bcb_config.gpt4o_region_fs_bs1.yaml

echo "=========================================="
echo "BCB Region+FS: gpt-4o, reproduce params, bs=1, mcs=5, var_weighted"
echo "Job ID: $SLURM_JOB_ID | Node: $SLURMD_NODENAME | Start: $(date)"
echo "=========================================="

INNER_SCRIPT=$(mktemp /tmp/bcb_gpt4o_regfs_bs1_XXXXXX.sh)
...
echo '=========================================='
echo '[Region+FS] gpt-4o, reproduce params, bs=1, 10 epochs, mcs=5, var_weighted'
echo '=========================================='
python run/run_bcb_region.py \
    ...
    --region_temperature 0.1 --shrinkage_top_n 1 --region_min_cluster_size 5 \
    --region_variance_weighted_dist \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/gpt4o_region_fs_bs1

EXIT_CODE=$?
echo "[INFO] Region+FS bs1 exit code: $EXIT_CODE"
exit $EXIT_CODE
INNEREOF
