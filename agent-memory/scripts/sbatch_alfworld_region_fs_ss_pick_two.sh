#!/bin/bash
#SBATCH --job-name=yl-alf-fs-ss-p2
...
#SBATCH --output=logs/alf_fs_ss_pick_two_%j.log
#SBATCH --error=logs/alf_fs_ss_pick_two_%j.log

# Region failure summary + success summary: pick_two 4 sections FROM SCRATCH
# Compares failure+success summary vs pure failure summary (932133) on full curve.
...
# Config: from scratch (disable resume), rename experiment to avoid output collision
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    ...
    sed 's|ckpt_resume_enabled: true|ckpt_resume_enabled: false|; s|_region_failure_summary_4section"|_region_fs_ss_4section"|' \
    > "/tmp/alf_two_fs_ss_$$.yaml"
...
echo "=========================================="
echo "pick_two_obj_and_place — Region FS + Success Summary (4 sections from scratch)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_two_fs_ss_$$.yaml" \
    ...
    --success_summary_n_slots 1 \
    --success_summary_mode append
echo "[pick_two S3+S4 fs+ss] exit=$? at $(date)"
