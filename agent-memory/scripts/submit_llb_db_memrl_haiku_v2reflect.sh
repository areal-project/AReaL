#!/bin/bash
# ============================================================================
# Submit LLB DB MemRL run with CORRECTED reflection prompt (v2) to AIStudio
# Full 10-epoch run; MEMRL_LLB_REFLECTION_PROMPT=v2 set inside the launcher.
# ============================================================================
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
# Job naming convention: yl-benchmark-method-model (+timestamp)
export JOB_NAME="yl-llbdb-memrl-haiku-v2reflect-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"

export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_memrl_haiku_aistudio_v2reflect.sh"
