#!/bin/bash
# Submit the sandboxed LocalOSContainer end-to-end test to AIStudio. /storage-safe:
# the test creates its OWN canary under logs/ and verifies it survives; it never
# deletes anything else under /storage.
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbos-sandbox-test-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_aistudio_os_sandbox_test.sh"
