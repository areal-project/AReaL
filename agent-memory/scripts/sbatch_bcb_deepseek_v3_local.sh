#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-local
...
#SBATCH --output=logs/bcb_dsv3_local_%j.log
#SBATCH --error=logs/bcb_dsv3_local_%j.log
...
run_in_runner "python run/run_bcb.py \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_nomem"
NOMEM_EXIT=$?
echo "[INFO] No-Memory exit code: $NOMEM_EXIT"
...
echo "[INFO] Servers stopped. no-mem exit: $NOMEM_EXIT | MemRL exit: $MEMRL_EXIT"
