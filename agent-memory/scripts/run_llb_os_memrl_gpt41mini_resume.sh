#!/bin/bash
# RESUME the stalled haiku OS MemRL run from its latest checkpoint.
# Setting MEMRL_RUN_ID to the stalled run's exp-dir suffix makes ck_dir point at
# the SAME experiment dir, so run_llb auto-resumes from the latest snapshot
# (snapshot/3_b9 -> continues Section 3 at batch 10). Sandbox default-ON.
set +e
...
LOGFILE=${PROJECT_DIR}/logs/llb_os_memrl_gpt41mini_resume_${HOST_SHORT}_${TS}.log
mkdir -p ${PROJECT_DIR}/logs
exec > >(tee -a $LOGFILE) 2>&1

echo "=========================================="
echo "RESUME LLB OS MemRL (claude-gpt-4.1-mini) from ckpt"
...
# >>> resume: pin the exp dir of the stalled run so it auto-resumes from snapshot/3_b9 <<<
export MEMRL_RUN_ID=20260715-114242
...
echo '[INFO] Resuming LLB OS MemRL (haiku) from latest snapshot...'
python3 run/run_llb.py --config configs/rl_llb_os_memrl_gpt41mini.yaml
