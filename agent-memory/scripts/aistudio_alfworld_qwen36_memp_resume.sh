#!/bin/bash
# AIStudio: Qwen3.6 ALFWorld — MemP continuation from the verified S10 batch checkpoint.
# GPU 0: embedding server; GPU 1: Qwen3.6 vLLM + MemP continuation.
set -euo pipefail
...
LOGFILE="$MEMRL_DIR/logs/aistudio_qwen36_memp_resume_${RUN_STAMP}.log"

EMBED_PORT=9000
LLM_PORT=9101
export MEMRL_RUN_ID="qwen36_memp_resume_s10b20"
...
echo "=========================================="
echo "AIStudio: Qwen3.6 ALFWorld — MemP continuation"
echo "Run ID: $MEMRL_RUN_ID (isolated continuation namespace)"
echo "Resume snapshot: $MEMP_RESUME_SNAPSHOT"
echo "Start: $(date)"
echo "=========================================="
...
experiment:
  ...
  experiment_name: alfworld_memp_qwen36_resume_s10b20
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
  ...
  ckpt_resume_path: "MEMP_RESUME_SOURCE_PLACEHOLDER"
...
sed -i "s|MEMP_RESUME_SOURCE_PLACEHOLDER|$MEMP_RESUME_SOURCE|" "$CFG"
echo "[PREFLIGHT] MemP resume checkpoint verified: $MEMP_RESUME_SNAPSHOT"
echo "[EXP] MemP continuation starting with config $CFG"
python3 run/run_alfworld.py --config "$CFG" --skip_initial_eval

echo "[DONE] MemP continuation completed at $(date)"
