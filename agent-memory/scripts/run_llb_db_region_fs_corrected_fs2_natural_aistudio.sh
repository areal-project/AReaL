...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_corrected_fs2_natural_gpt41mini_${HOST_SHORT}_${TS}.log"
...
export MEMRL_RUN_ID="${MEMRL_RUN_ID:-region-fs-db-gpt41mini-corrected-fs2-natural-$(date +%Y%m%d-%H%M%S)}"
...
CONFIG=/tmp/rl_llb_db_region_fs_corrected_fs2_natural.yaml
...
cfg['memory']['user_id']='llb_db_region_fs_corrected_fs2_natural_' + safe_run_id
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_corrected_fs2_natural'
...
printf '%s\n' '[INFO] Fresh DB main-method run: corrected section-end topology (no cooldown); FS=2 natural selection; recall=10; final selector top-5.'
...
assert cfg['memory']['k_retrieve'] == 10
assert cfg['rl_config']['topk'] == 5
print('[PRECHECK] fresh checkpoint=false; DB corrected topology=section-end only/no cooldown; candidate recall=10; final selector topk=5; natural FS=2')
PYVERIFY
