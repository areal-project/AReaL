#!/bin/bash
# Qwen2.5-72B ALFWorld Region+FS Cell A topology-stability ablation after split-posterior fix.
...
OUTPUT_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_regionfs_cell_a_conditional_fs_20260810"
LOG_ROOT="$MEMRL_DIR/logs/alfworld_regionfs_qwen72b_cell_a_conditional_fs_${RUN_TAG}"
DRIVER_LOG="$LOG_ROOT/driver.log"
WORK_TMP_BASE="/storage/openpsi/users/yl/agent-memory/.tmp"
WORK_TMP="$WORK_TMP_BASE/qwen72b_regionfs_cell_a_conditional_fs_${RUN_TAG}_$$"
...
echo "================================================================"
echo "Qwen2.5-72B Region+FS Cell A topology-stable conditional-RFS rerun"
...
echo "RFS semantics: replace only failures already selected in baseline top-3; all-success top-3 falls back unchanged"
echo "Logs: $LOG_ROOT"
...
export MEMRL_REGION_STABILITY_SOURCE="$MEMRL_DIR"
export PYTHONPATH="$MEMRL_DIR/scripts/region_stability_conditional_fs_overlay:$MEMRL_DIR:/storage/openpsi/users/yl/agent-memory/.local/lib/python3.12/site-packages:${PYTHONPATH:-}"
...
PYFIX
OVERLAY_DIR="$MEMRL_DIR/scripts/region_stability_conditional_fs_overlay"
...
print("[TOPOLOGY-STABILITY-PREFLIGHT] PASS: init=3000; no mid-epoch edits; initial cluster enters cooldown; 1 full-section cooldown")
PYTOPO
...
run_cell() {
    ...
    export MEMRL_RUN_ID="qwen72b-regionfs-conditional-${label}-fs1-${RUN_TAG_SAFE}"

    {
        ...
        echo "[CELL-CONFIG] topology=init_step=3000,mid_epoch=false,merge_interval=3553,cooldown_sections=1"
        echo "[CELL-CONFIG] experiment_name=$exp_name"
        echo "[CELL-CONFIG] MEMRL_RUN_ID=$MEMRL_RUN_ID"
        echo "[CELL-CONFIG] config=$cfg"
        echo "============================================================"
        python3 "$MEMRL_DIR/scripts/region_stability_conditional_fs_overlay/run_alfworld.py" \
            --config "$cfg" \
...
run_cell cell_a eta0p03_topology_stable 0.03 2.5 0.60 0.45 0.55

echo "[ALL-DONE] Region+FS Cell A topology-stability ablation completed at $(date --iso-8601=seconds)"
