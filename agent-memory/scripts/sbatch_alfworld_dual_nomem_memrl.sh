#!/bin/bash
#SBATCH --job-name=yl-alf-dual-nomem-memrl
...
#SBATCH --mem=800G
#SBATCH --gres=gpu:6
#SBATCH --output=logs/alf_dual_nomem_memrl_%j.log
#SBATCH --error=logs/alf_dual_nomem_memrl_%j.log

# Machine A: no-mem + memrl for both DSv4-Flash and Qwen3.6
...
echo "=========================================="
echo "All servers ready. Starting experiments..."
echo "=========================================="

# --- DSv4-Flash: no-mem → memrl (background) ---
(
    echo "[DSv4] Phase 1: no-mem eval"
...
# --- Qwen3.6: no-mem → memrl (background) ---
(
    echo "[Qwen3.6] Phase 1: no-mem eval"
    python run/run_alfworld.py --config configs/rl_alf_config.qwen36_nomem.yaml --eval_train
    echo "[Qwen3.6 no-mem] exit=$? at $(date)"

    echo "[Qwen3.6] Phase 2: memrl (10 sections)"
    python run/run_alfworld.py --config configs/rl_alf_config.qwen36_memrl.yaml --skip_initial_eval
