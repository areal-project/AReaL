#!/bin/bash
#SBATCH --job-name=yl-alf-memrl-resume-2sub
...
#SBATCH --output=logs/alf_memrl_resume_2sub_%j.log
#SBATCH --error=logs/alf_memrl_resume_2sub_%j.log

# MemRL holdout resume: pick_two + look_at (S2 → S3/S4)
# Expected: ~12h (2 subtasks × 2 remaining sections × ~3h/section)
...
INNER_SCRIPT=$(mktemp /tmp/alf_memrl_res_XXXXXX.sh)
...
for cfg in two_memrl_4section_resume look_memrl_4section_resume; do
...
# Phase 1: pick_two memrl resume S2 → S3/S4
echo "=========================================="
echo "[PHASE 1/2] pick_two_obj_and_place — MemRL resume S2 → S3/S4"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_two_memrl_4section_resume_$$.yaml" \
    --holdout_subtask alf/pick_two_obj_and_place \
    --holdout_eval_pools train,valid \
    --skip_initial_eval
echo "[PHASE 1] exit=$? at $(date)"

# Phase 2: look_at memrl resume S2 → S3/S4
echo "=========================================="
echo "[PHASE 2/2] look_at_obj_in_light — MemRL resume S2 → S3/S4"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_look_memrl_4section_resume_$$.yaml" \
