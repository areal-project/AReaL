...
OUTPUT_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_regionfs_cell_a_topology_stable_20260723"
LOG_ROOT="$MEMRL_DIR/logs/alfworld_regionfs_qwen72b_cell_a_topology_stable_${RUN_TAG}"
DRIVER_LOG="$LOG_ROOT/driver.log"
WORK_TMP_BASE="/storage/openpsi/users/yl/agent-memory/.tmp"
WORK_TMP="$WORK_TMP_BASE/qwen72b_regionfs_cell_a_topology_stable_${RUN_TAG}_$$"
...
echo "================================================================"
echo "Qwen2.5-72B Region+FS Cell A topology-stability-only ablation"
...
echo "Only changed variable: stable Region topology schedule (init=3000, no mid-epoch edits, cooldown=1 section)"
echo "Logs: $LOG_ROOT"
...
export PYTHONPATH="$MEMRL_DIR/scripts/region_stability_overlay:$MEMRL_DIR:/storage/openpsi/users/yl/agent-memory/.local/lib/python3.12/site-packages:${PYTHONPATH:-}"
...
PYFIX
OVERLAY_DIR="$MEMRL_DIR/scripts/region_stability_overlay"
...
run_cell() {
...

    {
        ...
        echo "============================================================"
        python3 "$MEMRL_DIR/scripts/region_stability_overlay/run_alfworld.py" \
            --config "$cfg" \
