#!/bin/bash
# AIS submit: GPT-5 mini Region+FS trajectory, one section only.
set -euo pipefail
...
export WORKER_NUM=0
export JOB_NAME="yl-alf-gpt5mini-regionfs-traj-s1-$(date +%m%d-%H%M)"
...
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/ais_run_alf_gpt5mini_regionfs_trajectory_s1.sh"
