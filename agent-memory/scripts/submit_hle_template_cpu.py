...
'''
HLE 纯 API 调用，不需要 GPU。
cpu 单位是核, memory 单位是 MB
'''
app_name = "areals3"
...
# HLE is API-only but aistudio requires GPU. Use h200 1-card (confirmed working).
master = ExecConf(cpu=16, memory=64000, disk_m=100000, gpu_num=0, num=1)
worker = ExecConf(cpu=16, memory=64000, disk_m=100000, gpu_num=0, num=worker_num)
km_conf = KMConf(
