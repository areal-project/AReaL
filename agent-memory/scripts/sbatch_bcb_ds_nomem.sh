#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-nomem
#SBATCH --partition=all
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --output=logs/bcb_ds_nomem_%j.log
#SBATCH --error=logs/bcb_ds_nomem_%j.log

# Must run on the same node as the vLLM server job.
# Usage: sbatch --nodelist=slurmd-XX scripts/sbatch_bcb_ds_nomem.sh
...
echo "=========================================="
echo "BCB DeepSeek No-Memory Baseline"
...
echo '[INFO] Starting BCB No-Memory baseline (1 epoch, retrieve_k=0)...'
python run/run_bcb.py \
    ...
    --strip_think
echo '[INFO] No-Memory baseline completed!'
"
