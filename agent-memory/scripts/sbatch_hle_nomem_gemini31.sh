#!/bin/bash
#SBATCH --job-name=yl-hle-nomem-gem
#SBATCH --partition=all
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=32
#SBATCH --mem=64G
#SBATCH --output=logs/hle_nomem_gemini31_%j.log
#SBATCH --error=logs/hle_nomem_gemini31_%j.log

# HLE no-memory baseline with gemini-3.1-pro-preview (API-only, no GPU)
# Single pass, k_retrieve=0, 1 section only

MEMRL_DIR="/storage/openpsi/users/yl/agent-memory/MemRL"
RUNNER_IMG="/storage/openpsi/images/areal-latest.sif"

echo "=========================================="
echo "HLE No-Memory Baseline: gemini-3.1-pro-preview via MatrixLLM API"
...
python3 run/run_hle.py \
    --config configs/rl_hle_config.nomem_gemini31.yaml \
