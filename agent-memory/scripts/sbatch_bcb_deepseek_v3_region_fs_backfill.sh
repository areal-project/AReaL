#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-rfs-bf
...
# Region+FS + backfill (10 epochs from scratch)
# Same as original Region+FS but with success backfill to prevent context starvation.
# No fmmatch — uses the simpler region-aggregated failure summary.
...
echo "=========================================="
echo "BCB DeepSeek-V3 Region+FS + BACKFILL (fresh 10 epochs)"
...
echo ''
echo '=========================================='
echo '[Region+FS + BACKFILL] DeepSeek-V3, 10 epochs from scratch'
echo '=========================================='
run_in_runner "python run/run_bcb_region.py \
    ...
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_backfill"
EXIT_CODE=$?
echo "[INFO] Region+FS+backfill exit code: $EXIT_CODE"
