#!/bin/bash
#SBATCH --job-name=yl-alf-qwen36-mem0
...
#SBATCH --output=logs/alf_qwen36_mem0_%j.log
#SBATCH --error=logs/alf_qwen36_mem0_%j.log

# Mem0: own LLM on GPU 7, share region's embed (8301)
...
echo "=========================================="
echo "ALFWorld Qwen3.6: Mem0 (LLM:8800, Embed shared:8301)"
...
rl_config:
  ...
  weight_q: 0.5
EOF
echo "[Mem0] Running 10 sections (shared embed from region:8301)"
python run/run_alfworld.py --config /tmp/mem0.yaml --mem0 --skip_initial_eval
echo "[Mem0] exit=$? at $(date)"
'
