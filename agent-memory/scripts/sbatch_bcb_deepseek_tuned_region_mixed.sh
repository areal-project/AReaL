#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-tuned-rgn-mix
...
#SBATCH --mem=200G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-14,slurmd-27
#SBATCH --output=logs/bcb_ds_tuned_region_mixed_%j.log
#SBATCH --error=logs/bcb_ds_tuned_region_mixed_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Region + Tuned Config)"
...
# Ensure we are on the region-dev branch (region code lives there)
...
# --- Run Region experiment with tuned config ---
echo ''
echo '=========================================='
echo '[REGION+TUNED+MIXED] Starting BCB Region with mixed retrieval strategy'
echo 'Config: tau=0.30, proceduralization, query, region_retrieve_mode=mixed'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --k_global 30 \
    --k_local 10 \
    --region_retrieve_mode mixed \
    --output_dir ./results/deepseek_tuned_region_mixed \
