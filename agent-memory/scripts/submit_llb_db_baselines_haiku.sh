#!/bin/bash
# Submit LLB DB baseline to AIStudio
# Usage: BASELINE=rag bash scripts/submit_llb_db_baselines_haiku.sh
#    or: bash scripts/submit_llb_db_baselines_haiku.sh rag
set -e
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbdb-${BASELINE}-haiku-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"

export JOB_COMMAND="BASELINE=${BASELINE} bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_baselines_haiku_aistudio.sh"
