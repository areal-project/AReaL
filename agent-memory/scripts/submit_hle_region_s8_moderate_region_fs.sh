...
RID="yl-hle-region-s8-moderatefs-$(date +%Y%m%d-%H%M%S)"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_proceduralization_structured_fillfix_yl-hle-region-proc-structuredfs-fillfix-20260803-223554-regionprocstructfill/snapshot/8
echo "[submit] HLE moderate Region+FS, completed structured fill-fix S8 source: $SOURCE_CKPT"
REMOTE="/tmp/${RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_s8_moderatefs_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
SITE="/tmp/hle_region_s8_moderatefs_site_${HLE_RID}"
rm -rf "$SITE"; mkdir -p "$SITE"
pip install --target "$SITE" -i https://pypi.antfin-inc.com/simple/ tensorboard pandas tqdm concurrent-log-handler hdbscan >/tmp/hle_region_s8_moderatefs_deps.log 2>&1
...
SAFE_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_s8_moderatefs_${HLE_RID}-regionmoderate"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_proceduralization_structured_fillfix_yl-hle-region-proc-structuredfs-fillfix-20260803-223554-regionprocstructfill/snapshot/8
if find "$SAFE_ROOT/snapshot" -mindepth 1 -maxdepth 1 -type d -name '*' -print -quit 2>/dev/null | grep -q .; then
  ...
  echo "[remote-start] no own checkpoint yet; using fixed completed fill-fix S8 source: $REGION_CKPT"
fi
export HLE_CKPT="$REGION_CKPT" CFG="/tmp/hle_region_s8_moderatefs_${HLE_RID}.yaml"
echo "[HLE REGION S8 MODERATE REGION+FS] rid=$HLE_RID FS_mode=hle_structured slots=1 independent_pool=true min_success=2 min_sim=0.70 signature=category+raw_subject min_evidence=3 abstain=original_region_top3 lambda_max=0.8 confidence_k=3 score_formula=unchanged"
...
cfg['memory'].update(build_strategy='proceduralization',user_id='hle_region_s8_moderatefs_user')
cfg['experiment'].update(
    experiment_name='hle_regionfs_s8_moderatefs',
...
export MEMRL_RUN_ID="${HLE_RID}-regionmoderate" MEMRL_REASONING_EFFORT=high MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_EMBED_MIN_INTERVAL=2.5
