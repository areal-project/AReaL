...
#SBATCH --mem=800G
#SBATCH --gres=gpu:8
#SBATCH --exclude=slurmd-2,slurmd-3
#SBATCH --output=logs/bcb_memp_%j.log
#SBATCH --error=logs/bcb_memp_%j.log

# BCB MemP baseline: procedural memory without Q-value driven selection
# 10 epochs, proceduralization + pure sim retrieval (ablation of MemRL)
...
LLM_PORT=8000
EMBED_PORT=8001
CONFIG=configs/rl_bcb_config.memp_local.yaml

echo "=========================================="
echo "BCB MemP Baseline: DeepSeek-V3 via SGLang"
...
echo ''
echo '=========================================='
echo '[MemP] DeepSeek-V3, proceduralization + sim-only, 10 epochs'
echo '=========================================='
run_in_runner "python run/run_bcb.py \
    --config $CONFIG \
    --split instruct --subset full --epochs 10 \
    --n_eval_runs 3 --eval_temperature 0.2 --multi_eval_epochs last \
    --checkpoint_interval 100 --max_checkpoints 3 \
    --eval_timeout 240 --untrusted_hard_timeout 300 \
    --output_dir /storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_memp"
EXIT_CODE=$?
