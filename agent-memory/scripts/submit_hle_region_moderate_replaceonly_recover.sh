...
JOB_RID="yl-hle-region-moderate-replaceonly-recover-$(date +%Y%m%d-%H%M%S)"
LINEAGE_RID="yl-hle-region-moderate-replaceonly-20260810-220359"
...
echo "[submit] recover HLE Moderate replace-only lineage=$LINEAGE_RID job=$JOB_RID"
REMOTE="/tmp/${JOB_RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_moderate_replaceonly_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
SITE="/tmp/hle_region_moderate_replaceonly_site_${HLE_RID}"
rm -rf "$SITE"; mkdir -p "$SITE"
pip install --target "$SITE" -i https://pypi.antfin-inc.com/simple/ tensorboard pandas tqdm concurrent-log-handler hdbscan >/tmp/hle_region_moderate_replaceonly_deps.log 2>&1
...
SAFE_ROOT="/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_regionfs_moderate_replaceonly_${HLE_RID}-regionmoderaterpl"
SOURCE_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_memrl_gemini35flash_20260707-115246/snapshot/1
if find "$SAFE_ROOT/snapshot" -mindepth 1 -maxdepth 1 -type d -name '*' -print -quit 2>/dev/null | grep -q .; then
  ...
  echo "[remote-start] no own checkpoint yet; using fixed clean historical MemRL-proceduralization S1 source: $REGION_CKPT"
fi
export HLE_CKPT="$REGION_CKPT" CFG="/tmp/hle_region_moderate_replaceonly_${HLE_RID}.yaml"
echo "[HLE REGION MODERATE REPLACE-ONLY FULL S1] rid=$HLE_RID FS_mode=hle_structured slots=1 independent_pool=false selected_failure_only=true min_success=2 min_sim=0.70 signature=category+raw_subject min_evidence=3 abstain=original_region_top3 lambda_max=0.8 confidence_k=3 score_formula=unchanged"
...
echo "[HLE STRUCTURED FS] DYNAMIC_FALLBACK_TEST_OK"
python3 - <<'PY'
import os,yaml
...
cfg['memory'].update(build_strategy='proceduralization',user_id='hle_region_moderate_replaceonly_user')
cfg['experiment'].update(
    experiment_name='hle_regionfs_moderate_replaceonly',
    ...
    failure_summary_independent_pool=False,
    failure_summary_min_success=2,
    failure_summary_min_similarity=0.70,
    failure_summary_structured_min_evidence=3,
...
export MEMRL_RUN_ID="${HLE_RID}-regionmoderaterpl" MEMRL_REASONING_EFFORT=high MEMRL_LLM_MIN_INTERVAL=2.5 MEMRL_EMBED_MIN_INTERVAL=2.5
python3 "$ROOT/MemRL/scripts/hle_structured_fs_overlay/run_hle_region.py" \
  ...
  --shrinkage_top_n 1 --shrinkage_lambda_max 0.8 --shrinkage_confidence_k 3.0 \
...
export PYTHONPATH=/tmp/yl_pypai AISTUDIO_LOGIN_NAME=aistudio AISTUDIO_USERNUMBER=477578 WORKER_NUM=0 JOB_TAG='' JOB_NAME="$JOB_RID" KM_IMAGE='acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401'
export JOB_COMMAND="HLE_RID=$(printf '%q' "$LINEAGE_RID") bash -c $(printf '%q' "echo $PAYLOAD | base64 -d | bash")"
export LAUNCH_CONTAINER_MODE=dev_local PYPAI_HOME="/tmp/pypai_${JOB_RID}" TMPDIR=/tmp
cd /tmp
