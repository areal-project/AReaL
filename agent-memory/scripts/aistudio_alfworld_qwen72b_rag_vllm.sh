#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld RAG only (vLLM, 3x H200)
#
# GPU layout:
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_rag_vllm_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B RAG (vLLM, 3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
cat > /tmp/alf_rag_vllm_qwen72b_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
echo "[EXP] 72B RAG (vLLM) starting..."
python3 run/run_alfworld.py --config /tmp/alf_rag_vllm_qwen72b_$$.yaml --skip_initial_eval
