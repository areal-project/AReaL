...
RID="yl-hle-region-selfrag-two-$(date +%Y%m%d-%H%M%S)"
...
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_region_selfrag_two_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
STATE="/tmp/hle_region_selfrag_${HLE_RID}"
mkdir -p "$STATE"
SITE="/tmp/hle_region_selfrag_site_${HLE_RID}"
...
echo "[HLE two-way] rid=$HLE_RID RegionTraj=2.0s@0m SelfRAG=2.0s@5m update_workers=4 shared_inflight=64"
launch(){
 ...
 (
  ...
  echo "[HLE two-way child] mode=$mode delay=${delay}s interval=${interval}s ckpt=$ckpt update_workers=$MEMRL_UPDATE_MAX_WORKERS"
  python3 - <<'PY'
import os,yaml
...
PY
  ...
  if [ "$mode" = selfrag ]; then
    ...
    echo "[HLE two-way region] HDBSCAN_REQUIRED=true checkpointV2=enabled"
...
export JOB_COMMAND="HLE_RID=$(printf '%q' "$RID") REGION_CKPT=$(printf '%q' "$REGION_CKPT") SELFRAG_CKPT=$(printf '%q' "$SELFRAG_CKPT") bash -c $(printf '%q' "echo $PAYLOAD | base64 -d | bash")"
