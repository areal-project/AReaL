#!/bin/bash
# Qwen2.5-72B ALFWorld Region+FS eval-only grid on the repaired E10 snapshot.
# Sweeps only val_lambda_max: 0.15 / 0.30 / 0.45. All other v2 parameters fixed.
set -euo pipefail
...
LOGFILE="$MEMRL_DIR/logs/aistudio_qwen72b_region_fs_vlmax_grid_${RUN_TAG}.log"
...
echo "============================================================"
echo "Qwen72B ALFWorld Region+FS val_lambda_max grid"
echo "Snapshot: $SNAPSHOT_PATH"
echo "Grid: 0.15 0.30 0.45"
echo "Start: $(date)"
echo "============================================================"
...
CUDA_VISIBLE_DEVICES=1,2 python3 -m sglang.launch_server \
    ...
    --context-length 32768 --nccl-port 29501 &
LLM_PID=$!
...
echo "[INFO] Both servers ready. Starting three eval-only cells."

for VLMAX in 0.15 0.30 0.45; do
...
CFGEOF
    ...
    echo "============================================================"
    echo "[GRID] $CELL_TAG: val_lambda_max=$VLMAX, Region=ON, FS slots=2"
    echo "[GRID] Fixed: tau=0.58, wq=0.65, ws=0.35, shrinkage_k=2.5, no_z_norm"
    echo "============================================================"
    ...
    rm -f "$CONFIG"
    echo "[GRID] Completed $CELL_TAG at $(date)"
done

echo "[DONE] All three Region+FS grid cells completed at $(date)"
