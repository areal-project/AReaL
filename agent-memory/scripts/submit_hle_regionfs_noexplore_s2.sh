...
RID="yl-hle-regionfs-traj-noexplore-s2-$(date +%Y%m%d-%H%M%S)"
REGION_CKPT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/hle/exp_hle_group1_region_trajectory_yl-hle-region-selfrag-two-20260727-003308-regiontraj/snapshot/1_incomplete_final
echo "[submit] intentional S2 fork checkpoint (do not overwrite source): $REGION_CKPT"
REMOTE="/tmp/${RID}.sh"
cat > "$REMOTE" <<'REMOTE'
#!/usr/bin/env bash
set -euo pipefail
ROOT=/storage/openpsi/users/yl/agent-memory
cd "$ROOT/MemRL"
LOG="$ROOT/MemRL/logs/aistudio_hle_regionfs_traj_noexplore_s2_${HLE_RID}.log"
exec > >(tee -a "$LOG") 2>&1
STATE="/tmp/hle_region_noexplore_${HLE_RID}"
mkdir -p "$STATE"
SITE="/tmp/hle_region_noexplore_site_${HLE_RID}"
...
echo "[HLE Region+FS no-explore] rid=$HLE_RID S2 fork interval=3.0s exploration=OFF update_workers=4 shared_inflight=64"
launch(){
 ...
 (
  ...
  echo "[HLE Region+FS no-explore child] mode=$mode delay=${delay}s interval=${interval}s ckpt=$ckpt update_workers=$MEMRL_UPDATE_MAX_WORKERS"
  python3 - <<'PY'
import os,yaml
...
if m in ('rag','selfrag'):
    ...
    cfg['experiment'].update(experiment_name='hle_regionfs_trajectory_noexplore_s2',ckpt_resume_enabled=True,ckpt_resume_path=os.environ['HLE_CKPT'],ckpt_resume_epoch=0,ckpt_resume_prefer_current_run=True,ckpt_save_every_n_batches=5)
...
launch regiontraj 0 3.0 "$REGION_CKPT" "${HLE_RID}-regiontraj"
status=0
