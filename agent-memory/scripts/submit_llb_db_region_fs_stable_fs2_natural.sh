#!/usr/bin/env bash
# Fresh contrast: stable topology with corrected proceduralization's natural FS=2.
set -euo pipefail
export AISTUDIO_LOGIN_NAME=aistudio AISTUDIO_USERNUMBER=477578 WORKER_NUM=0 JOB_TAG='llb-db-regionfs-stable-fs2-natural'
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-stable-fs2-natural-${STAMP}"
export JOB_NAME="yl-llbdb-regionfs-stable-fs2-natural-${STAMP}"
export KM_IMAGE='acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401'
export JOB_COMMAND="export MEMRL_RUN_ID='${MEMRL_RUN_ID}'; bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_region_fs_stable_fs2_natural_aistudio.sh"
...
printf '%s\n' "[INFO] Submitting fresh DB contrast: stable topology + corrected natural FS=2: $JOB_NAME"
printf '%s\n' "[INFO] MEMRL_RUN_ID=$MEMRL_RUN_ID (fresh; no old checkpoint; no forced FS backfill)"
cd /tmp
