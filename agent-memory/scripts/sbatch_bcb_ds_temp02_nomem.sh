#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-t02-nomem
...
#SBATCH --output=logs/bcb_ds_temp02_nomem_%j.log
#SBATCH --error=logs/bcb_ds_temp02_nomem_%j.log
...
echo "=========================================="
echo "BCB DeepSeek No-Memory Baseline (temperature=0.2)"
...
# --- Run No-Memory Baseline (1 epoch, retrieve_k=0, temperature=0.2) ---
echo ''
echo '=========================================='
echo '[NOMEM] Starting BCB No-Memory baseline (temp=0.2, 1 epoch)'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_temp02_nomem.yaml \
    --split instruct \
    --subset full \
    --epochs 1 \
    --retrieve_k 0 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_temp02_nomem

NOMEM_EXIT=\$?
echo \"[INFO] No-Memory baseline exited with code: \$NOMEM_EXIT\"
