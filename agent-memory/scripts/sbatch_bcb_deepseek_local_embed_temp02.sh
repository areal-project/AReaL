#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-t02-mem
...
#SBATCH --mem=200G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-13
#SBATCH --output=logs/bcb_ds_local_embed_temp02_%j.log
#SBATCH --error=logs/bcb_ds_local_embed_temp02_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (serial, temp=0.2, MemRL)"
...
# --- Run MemRL experiment (serial, temp=0.2, 10 epochs) ---
echo ''
echo '=========================================='
echo '[MEMRL] Starting BCB MemRL serial temp=0.2 (10 epochs)'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_local_embed_temp02.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_memrl_local_embed_temp02
