#!/bin/bash
#SBATCH --job-name=yl-bcb-old-post-5gpu-v2
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-23,slurmd-45,slurmd-89
#SBATCH --output=logs/bcb_ds_old_region_post_5gpu_v2_%j.log
#SBATCH --error=logs/bcb_ds_old_region_post_5gpu_v2_%j.log
...
# --- Run Post Region experiment (bs=8) ---
echo ''
echo '=========================================='
echo '[MEMRL-OLD-ADD] Starting BCB MemRL Additive Region (10 epochs, bs=8)'
echo 'Config: tau=0.18, trajectory, avefact'
echo 'Region: Post multiplicative, beta, temp=0.3, top-1, min_cs=80, C=0.5'
echo '=========================================='
...
python run/run_bcb_region.py \
    ...
    --output_dir ./results/deepseek_old_region_post_5gpu_v2

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old-Post-v2 exited with code: \$MEMRL_EXIT\"
