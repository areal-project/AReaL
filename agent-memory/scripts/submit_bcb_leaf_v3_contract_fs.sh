#!/bin/bash
# Fresh Leaf-v3 full rerun.  Leaf-v2 topology/evidence semantics are unchanged;
# only the one-slot FS is contract-gated to avoid generic cross-contract advice.
set -euo pipefail
...
# Keep master/worker attempt identity coupled: never leave orphan retries.
export AIS_MAX_ATTEMPT=1
export SUBMIT_TEMPLATE="$MEMRL/scripts/submit_template_bcb_strict.py"
export JOB_NAME="yl-bcb-leafv3-contractfs-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
# New control root: never resume or overwrite Leaf-v1/v2 artifacts.
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs_leaf_v3_contract_fs
...
LOG_BASE="$MEMRL/logs/aistudio_bcb_leafv3_contract_fs_$STAMP"
