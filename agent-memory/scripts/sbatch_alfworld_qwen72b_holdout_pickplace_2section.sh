#!/bin/bash
#SBATCH --job-name=yl-alf-qwen72b-holdout-pp
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_qwen72b_holdout_pickplace_%j.log
#SBATCH --error=logs/alf_qwen72b_holdout_pickplace_%j.log
...
echo "=========================================="
echo "ALFWorld HOLDOUT (pick_and_place_simple) + region: Qwen2.5-72B-Instruct (TP=4) [2-section]"
...
TEMP_CONFIG=/tmp/alf_qwen72b_holdout_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" configs/rl_alf_config.qwen72b_holdout_pickplace_2section.yaml > "$TEMP_CONFIG"
