...
def main() -> None:
    ...
    logger.info("Region config: min_cluster_size=%d, retrieve_mode=%s, gating_mode=%s, temperature=%.2f, shrinkage_top_n=%d, utility_mode=%s",
    ...
    try:
        if not args.resume_from:
            raise SystemExit("region multi-eval-only requires --resume_from")
        runner.n_eval_runs = 3
        runner.eval_temperature = 0.2
        base = runner.run_eval_only()
        ...
        with open(os.path.join(str(run_dir), "summary.json"), "w") as f:
            _json.dump({
                "eval_only": True, "source_checkpoint": args.resume_from,
                "epoch": eval_epoch, "standard_eval": base, "multi_eval": multi_res,
            }, f, indent=2)
    finally:
