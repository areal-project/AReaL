#!/bin/bash
#SBATCH --job-name=yl-alf-rfs-p2-s3s4
...
#SBATCH --output=logs/alf_rfs_pick_two_s3s4_%j.log
#SBATCH --error=logs/alf_rfs_pick_two_s3s4_%j.log

# Region failure summary: pick_two S3+S4 train+eval
# Resume from S2 checkpoint, train S3 → eval → train S4 → eval
...
# Config: resume from S2 checkpoint (ckpt_resume_epoch: 2), train S3+S4
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    ...
    sed '/ckpt_resume_enabled/a\  ckpt_resume_epoch: 2' > "/tmp/alf_two_rfs_from_s2_$$.yaml"
...
echo "=========================================="
echo "pick_two_obj_and_place — Region FS S3+S4 (resume from S2)"
echo "Start: $(date)"
echo "=========================================="
python run/run_alfworld.py \
    --config "/tmp/alf_two_rfs_from_s2_$$.yaml" \
...
echo "[pick_two S3+S4] exit=$? at $(date)"
