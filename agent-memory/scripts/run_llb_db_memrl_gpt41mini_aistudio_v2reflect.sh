#!/bin/bash
# LLB DB MemRL run with gpt-4.1-mini (v2 reflection + db_pattern) on AIStudio
set -e
...
LOGFILE=${PROJECT_DIR}/logs/llb_db_memrl_gpt41mini_v2reflect_${HOST_SHORT}_${TS}.log
mkdir -p ${PROJECT_DIR}/logs
exec > >(tee -a $LOGFILE) 2>&1

echo "=========================================="
echo "MemRL - LLB DB v2-reflection run (gpt-4.1-mini)"
echo "Host: $(hostname)"
...
# Override model to gpt-4.1-mini
export MEMRL_LLM_MODEL=gpt-4.1-mini-2025-04-14
export MEMRL_RUN_ID=v2reflect-dbpattern-gpt41mini-20260715
...
echo '[INFO] Running LLB DB MemRL v2-reflection (gpt-4.1-mini)...'
