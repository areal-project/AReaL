...
export JOB_TAG=""
TS=$(date +%Y%m%d-%H%M%S)
export JOB_NAME="${JOB_NAME:-yl-hle-memrl-g35f-${TS}}"
...
set -e
LOGFILE=/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_hle_memrl_gemini35flash_${RUN_ID}.log
...
# This ensures code changes on /storage are always picked up, even after retry.
