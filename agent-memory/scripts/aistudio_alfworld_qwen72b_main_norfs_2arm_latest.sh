#!/bin/bash
# Latest region-dev strict No-RFS ablation:
#   arm 1: Category-Q only
#   arm 2: Capability-Region shrinkage, No-RFS
...
read -r EP CATP CAPP NC1 NC2 < <(python3 - <<'PYPORTS'
import socket
ports=[]
for _ in range(5):
...
OUT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_qwen72b_main_norfs_2arm_latest_20260810
LOG="$MEMRL_DIR/logs/alfworld_qwen72b_main_norfs_2arm_latest_$TAG"
WORK_TMP=/storage/openpsi/users/yl/agent-memory/.tmp/q72b_main_norfs_2arm_latest_${TAG}_$$
mkdir -p "$OUT" "$LOG" "$WORK_TMP"; chmod 700 "$WORK_TMP"; exec > >(tee -a "$LOG/driver.log") 2>&1
echo "[PORTS] embed=$EP category=$CATP capability=$CAPP nccl=$NC1/$NC2"
...
CUDA_VISIBLE_DEVICES=1,2 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port "$CATP" --context-length 32768 --trust-remote-code --nccl-port "$NC1" & CPID=$!; PIDS+=("$CPID"); wait_server category "$CATP" "$CPID"
CUDA_VISIBLE_DEVICES=3,4 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port "$CAPP" --context-length 32768 --trust-remote-code --nccl-port "$NC2" & FPID=$!; PIDS+=("$FPID"); wait_server capability "$CAPP" "$FPID"
...
CAT_CFG="$WORK_TMP/category.yaml"; CAP_CFG="$WORK_TMP/capability.yaml"
write_cfg "$CAT_CFG" "alfworld_qwen72b_category_q_latest_${TAG}" "$CATP"
write_cfg "$CAP_CFG" "alfworld_qwen72b_capability_norfs_latest_${TAG}" "$CAPP"
...
run_arm(){ local label=$1 cfg=$2 value_mode=$3; export MEMRL_RUN_ID="qwen72b-main-norfs-${label}-${SAFE}"; echo "[ARM] $label latest_source=true value_mode=$value_mode FS=0 run_id=$MEMRL_RUN_ID"; python3 run/run_alfworld.py --config "$cfg" "${common[@]}" --region_value_mode "$value_mode" 2>&1 | tee -a "$LOG/$label.log"; }
run_arm category_q "$CAT_CFG" category_q & A=$!
run_arm capability_norfs "$CAP_CFG" shrinkage & B=$!
set +e; wait $A; ra=$?; wait $B; rb=$?; set -e
echo "[ALL-DONE] category_q=$ra capability_norfs=$rb"; [[ $ra -eq 0 && $rb -eq 0 ]]
