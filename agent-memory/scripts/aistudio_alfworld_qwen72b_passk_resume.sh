#!/bin/bash
# ============================================================================
# AIStudio: Qwen2.5-72B ALFWorld Pass@10 ONLY — resume from R5 (3x H200)
#
# GPU layout (3 GPUs):
#   GPU 0: Qwen3-Embedding-8B
#   GPU 1-2: Qwen2.5-72B TP=2 → Pass@10 (resume)
# ============================================================================
set -e
...
LOGFILE="/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_qwen72b_passk_resume_${1:-$(date +%Y%m%d_%H%M%S)}.log"
...
echo "=========================================="
echo "AIStudio: Qwen2.5-72B Pass@10 resume (3 GPU)"
echo "Start: $(date)"
echo "=========================================="
...
# --- Launch Qwen2.5-72B (TP=2) on GPU 1-2 ---
...
echo "[INFO] Waiting for LLM on port $LLM_PORT_1..."
...
# ============================================================================
# Pass@10 (resume from baseline_passk_state.json via same MEMRL_RUN_ID)
# ============================================================================
cat > /tmp/alf_passk_resume_$$.yaml << CFGEOF
llm:
  provider: openai
  api_key: EMPTY
...
experiment:
  ...
  experiment_name: alfworld_passk10_qwen72b
  mode: train
  num_sections: 10
  batch_size: 128
  dataset_ratio: 1.0
...
echo "[EXP] 72B Pass@10 resume starting..."
python3 run/run_alfworld.py --config /tmp/alf_passk_resume_$$.yaml --skip_initial_eval
echo "[EXP] 72B Pass@10 exit=$? at $(date)"

echo "=========================================="
echo "Pass@10 complete. End: $(date)"
echo "=========================================="
