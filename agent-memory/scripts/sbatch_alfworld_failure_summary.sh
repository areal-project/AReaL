#!/bin/bash
#SBATCH --job-name=yl-alf-failure-summary
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_failure_summary_%j.log
#SBATCH --error=logs/alf_failure_summary_%j.log

# Region Failure Summary holdout experiment (single arm, fast validation)
# Base: 927950 region holdout (additive + shrinkage_k=3.0 + propagation_eta=0.12)
# Change: force 2 failure slots + replace with region failure summary
# Training + eval both affected (failure summary injected in all retrieval)
#
# Baseline: 927950 S2 holdout SR = 65.45%, plain MemRL 929119 = 64.73%
# See docs/REGION_FAILURE_SUMMARY.md
#
# Expected wall time: ~10-12h (region holdout 2-section)
...
echo "=========================================="
echo "ALFWorld Region Failure Summary holdout (pick_and_place_simple)"
...
# Patch config with current job's ports
CONFIG=/tmp/alf_failure_summary_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" \
    configs/rl_alf_config.qwen72b_holdout_pickplace_failure_summary_2section.yaml > "$CONFIG"
...
# --- Run: Region + failure summary (2 failure slots, replaced by region summary) ---
echo '=========================================='
echo 'Region Failure Summary holdout: force 2 failure slots + region summary injection'
echo "Start: $(date)"
echo '=========================================='

python run/run_alfworld.py \
    --config "$CONFIG" --region --region_gating_mode additive \
...
EXIT_CODE=$?
echo "[INFO] Experiment exited with code: $EXIT_CODE at $(date)"
