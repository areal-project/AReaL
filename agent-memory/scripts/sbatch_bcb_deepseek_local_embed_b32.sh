...
#SBATCH --output=logs/bcb_ds_local_embed_b32_%j.log
#SBATCH --error=logs/bcb_ds_local_embed_b32_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (batch_size=32)"
...
# --- Run MemRL experiment (batch_size=32) ---
echo ''
echo '=========================================='
echo '[MEMRL] Starting BCB MemRL with local embedding (batch_size=32, 10 epochs)'
echo '=========================================='

python run/run_bcb.py \
    --config configs/rl_bcb_config.deepseek_local_embed_b32.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --strip_think \
    --output_dir ./results/deepseek_memrl_local_embed_b32
