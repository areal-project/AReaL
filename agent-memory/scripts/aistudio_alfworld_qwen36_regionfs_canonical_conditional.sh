#!/bin/bash
# AIStudio: Qwen3.6 ALFWorld — Region+FS trajectory memory only — canonical conditional-RFS fresh main experiment (72B Cell A scale parameters).
...
export MEMRL_RUN_ID="qwen36_regionfs_canonical_conditional_v1"
# Region-only override: safe no-op for missing/invalid batched actions.
export MEMRL_ALFWORLD_FALLBACK_ACTION="look"
# Avoid fast_downward shared-library copies exhausting CPFS /tmp quota.
WORK_TMP="/dev/shm/q36r72align_${RUN_STAMP}_$$"
...
echo "Run ID: $MEMRL_RUN_ID (fresh canonical conditional-RFS; fixed K=3)"
echo "[CONFIG] Region-only fallback action: $MEMRL_ALFWORLD_FALLBACK_ACTION"
echo "[CONTRACT] MemRL-equivalent fixed budget K=3; conditional RFS content replacement only; no failure force-recall; no extra FS slot"
echo "Start: $(date)"
echo "=========================================="
...
experiment:
  ...
  save_memories: true
  # Fresh clean S1 run; no legacy topology state is imported.
  ckpt_resume_enabled: false
  ckpt_resume_path: ""
  ckpt_resume_epoch: null
  batch_checkpoint_interval: 2
  batch_checkpoint_keep: 4
  n_eval_runs: 4
...
echo "[EXP] Canonical conditional-RFS Region+FS fresh run starting with config $CFG"
