#!/bin/bash
#SBATCH --job-name=yl-bcb-old-post-new-b8
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:4
#SBATCH --exclude=slurmd-23,slurmd-24,slurmd-45,slurmd-89
#SBATCH --output=logs/bcb_ds_old_region_post_new_b8_4gpu_%j.log
#SBATCH --error=logs/bcb_ds_old_region_post_new_b8_4gpu_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Old Config + Post NEW bs=8, 5 GPU)"
echo "Job ID: $SLURM_JOB_ID"
echo "Node: $SLURMD_NODENAME"
echo "Start time: $(date)"
echo "GPU Config: Embedding on GPU 4 + LLM TP=4 on GPUs 0-3 (separated)"
echo "Post NEW: Beta posterior, multiplicative gating, temp=0.3, top-1, min_cluster_size=15"
echo "=========================================="
...
# --- Start embedding vLLM on GPU 4 (dedicated, no sharing with LLM), port EMBED_PORT ---
...
# Wait for CUDA graph capture to complete before sending requests
echo '[INFO] Waiting 60s for CUDA graph capture to finish...'
sleep 60

# --- Run MemRL experiment with old config + additive NEW (bs=8) ---
echo ''
echo '=========================================='
echo '[MEMRL-OLD-POST-NEW-B8] Starting BCB MemRL (10 epochs, bs=8)'
echo 'Config: tau=0.18, trajectory, avefact, sim_norm=0.186/0.094'
echo 'Region: Post-retrieve multiplicative gating (NEW - Beta posterior, temp=0.3, top-1, min_cs=15)'
echo '=========================================='

python run/run_bcb_region.py \
    --config configs/rl_bcb_config.deepseek_old_region_bs8.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --region_temperature 0.3 \
    --shrinkage_top_n 1 \
    --region_min_cluster_size 15 \
    --output_dir ./results/deepseek_old_region_post_new_b8_4gpu

MEMRL_EXIT=\$?
echo \"[INFO] MemRL-Old-Post-New-B8 exited with code: \$MEMRL_EXIT\"
