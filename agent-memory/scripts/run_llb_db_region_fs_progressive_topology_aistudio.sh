#!/usr/bin/env bash
# Fresh corrected proceduralization DB progressive-topology screening run.
...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_progressive_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"

echo '=========================================='
echo 'LLB DB corrected Region progressive-topology E1-E3 screening'
echo "Host: $(hostname)"
...
export MEMRL_REGION_SPLIT_RANGE_FRACTION=0.15
export MEMRL_REGION_MAX_VARIANCE_SPLITS_PER_EPOCH=1
export MEMRL_REGION_SPLIT_MIN_EFFECTIVE_EVIDENCE=200
export MEMRL_REGION_PROGRESSIVE_BEST_SPLIT=1
export MEMRL_REGION_MAX_MERGES_PER_EPOCH=1
export MEMRL_REGION_SPLIT_MIN_CHILD_SIZE=30
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-progressive-topology-20260729"
...
CONFIG=/tmp/rl_llb_db_region_fs_progressive.yaml
...
cfg['memory']['user_id']='llb_db_region_fs_progressive_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_progressive'
...
printf '%s\n' '[INFO] Fresh progressive topology: max_split=1, max_merge=1, evidence>=200, child>=30.'
