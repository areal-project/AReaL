#!/bin/bash
#SBATCH --job-name=yl-alf-7b-inline-fs
...
#SBATCH --output=logs/alf_7b_inline_fs_%j.log
#SBATCH --error=logs/alf_7b_inline_fs_%j.log
...
echo '[INFO] Building dataset with inline failure summaries...'
...
python examples/alfworld/dataset.py \
    ...
    --output_dir /tmp/areal/alfworld_dataset_inline_fs \
    --split train \
    --failure_summary ${AREAL_DIR}/examples/alfworld/game_file_to_inline_fs.json \
    --fs_ratio 0.5
...
python examples/alfworld/train.py \
    ...
    experiment_name=alfworld-7b-grpo-inline-fs \
    train_dataset.path=/tmp/areal/alfworld_dataset_inline_fs/combined \
    valid_dataset.path=/tmp/areal/alfworld_dataset_inline_fs/combined \
