#!/bin/bash
# Hard ALFWorld holdout continuation: Region-only resume through S10.
...
CUDA_VISIBLE_DEVICES=1,2 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port $RP --context-length 32768 --trust-remote-code --nccl-port 29690 & PIDS+=("$!")
