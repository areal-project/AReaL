#!/bin/bash
# Submit LLB DB MeMp baseline (gpt-4.1-mini) to AIStudio
set -e
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbdb-memp-gpt41mini-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"

export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_memp_gpt41mini_aistudio.sh"
