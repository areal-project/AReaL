...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_splitprior_fix_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"

echo '=========================================='
echo 'LLB DB Region+FS fresh split-prior-fix rerun (gpt-4.1-mini)'
echo "Host: $(hostname)"
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-splitpriorfix-20260722"
...
cfg['memory']['user_id']='llb_db_region_fs_splitpriorfix_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_splitpriorfix'
...
printf '%s\n' '[INFO] Fresh run: checkpoint loading disabled; split posterior fix active in current source.'
