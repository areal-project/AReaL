#!/bin/bash
# Submit the accurate Qwen72B RAG continuation as an independent 3xH200 AIS job.
set -euo pipefail
export AISTUDIO_LOGIN_NAME="${AISTUDIO_LOGIN_NAME:-aistudio}"
export AISTUDIO_USERNUMBER="${AISTUDIO_USERNUMBER:-477578}"
export WORKER_NUM=0
export JOB_TAG="qwen72b-rag-resume-s5-full"
TS=$(date +%Y%m%d_%H%M%S)
export JOB_NAME="yl-alf-q72b-rag-resume-s5-full-${TS}"
export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-vllm-20260429"
export JOB_COMMAND="EMBED_PORT=19120 LLM_PORT=19320 bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/aistudio_alfworld_qwen72b_rag_resume_s5_full.sh ${TS}"
export GPU_NUM=3
