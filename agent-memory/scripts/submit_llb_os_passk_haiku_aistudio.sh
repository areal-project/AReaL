#!/bin/bash
# Submit LLB OS pass@10 baseline (claude-haiku-4-5) to AIStudio.
# HOLD until MemRL result is known.
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbos-passk-haiku-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_os_passk_haiku_aistudio.sh"
