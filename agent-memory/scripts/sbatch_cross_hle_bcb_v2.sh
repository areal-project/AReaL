#!/bin/bash
#SBATCH --job-name=cross_hle_bcb_v2
#SBATCH --partition=all
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=48:00:00
#SBATCH --output=/storage/openpsi/users/yl/agent-memory/MemRL/logs/cross_hle_bcb_v2_%j.out
#SBATCH --error=/storage/openpsi/users/yl/agent-memory/MemRL/logs/cross_hle_bcb_v2_%j.err
...
cp /storage/openpsi/users/yl/.claude/config.yaml /tmp/litellm_hle_bcb.yaml

echo "[INFO] Starting LiteLLM on port ${LITELLM_PORT}..."
python3 -m litellm.proxy.proxy_cli --config /tmp/litellm_hle_bcb.yaml --port ${LITELLM_PORT} --host 127.0.0.1 &
LITELLM_PID=$!
...
echo "[INFO] Starting HLE→BCB cross-benchmark experiment..."
...
python scripts/run_cross_benchmark_experiment.py \
    ...
    --epochs 10 \
    --batch_size 5 \
    --name "cross_hle_bcb_v2" \
    --mode local

echo "[INFO] Stopping LiteLLM..."
kill ${LITELLM_PID} 2>/dev/null || true

echo "[INFO] HLE→BCB experiment completed"
CONTAINER_SCRIPT
