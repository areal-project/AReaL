...
#SBATCH --output=logs/hle_memp_%j.log
#SBATCH --error=logs/hle_memp_%j.log

# HLE MemP baseline: procedural memory without Q-value driven selection
# 10 sections, accumulates memory but retrieval uses pure sim (no Q)
...
echo "=========================================="
echo "HLE MemP Baseline: deepseek-v3.2 via SGLang"
...
python run/run_hle.py \
    --config configs/rl_hle_config.memp_local.yaml \
