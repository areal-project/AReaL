#!/bin/bash
#SBATCH --job-name=yl-alf-qwen72b-region-s10-vlmax005
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_qwen72b_region_s10_vlmax005_%j.log
#SBATCH --error=logs/alf_qwen72b_region_s10_vlmax005_%j.log
...
echo "=========================================="
echo "ALFWorld REGION s10 + soft explore + vlmax=0.05 (ablation best): Qwen2.5-72B-Instruct (TP=4) + Qwen3-Embedding-8B (local)"
...
TEMP_CONFIG=/tmp/alf_qwen72b_region_s10_vlmax005_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" configs/rl_alf_config.qwen72b_region_10section_softexplore_vlmax005.yaml > "$TEMP_CONFIG"
...
# --- Run experiment ---
echo '=========================================='
echo '[ALFWorld-Qwen72B-Region-s10-vlmax005] Qwen2.5-72B-Instruct + text-embedding-3-large (API)'
...
echo "[INFO] ALFWorld Qwen72B region-s10-vlmax005 exited with code: $ALF_EXIT"
