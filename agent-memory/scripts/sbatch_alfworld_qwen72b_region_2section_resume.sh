#!/bin/bash
#SBATCH --job-name=yl-alf-qwen72b-region-resume
...
#SBATCH --mem=400G
#SBATCH --gres=gpu:5
#SBATCH --output=logs/alf_qwen72b_region_2section_resume_%j.log
#SBATCH --error=logs/alf_qwen72b_region_2section_resume_%j.log
...
echo "=========================================="
echo "ALFWorld REGION RESUME from 927808 s2_b80: Qwen2.5-72B-Instruct (TP=4) + Qwen3-Embedding-8B (local)"
...
TEMP_CONFIG=/tmp/alf_qwen72b_region_resume_config_$$.yaml
sed "s|localhost:8000|localhost:${LLM_PORT}|g; s|localhost:8001|localhost:${EMBED_PORT}|g" configs/rl_alf_config.qwen72b_region_2section_resume.yaml > "$TEMP_CONFIG"
...
# --- Run experiment ---
echo '=========================================='
echo '[ALFWorld-Qwen72B-Region-2section-RESUME] Qwen2.5-72B-Instruct + text-embedding-3-large (API)'
...
echo "[INFO] ALFWorld Qwen72B region-2section-RESUME exited with code: $ALF_EXIT"
