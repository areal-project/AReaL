#!/bin/bash
# Submit LLB DB all baselines (single job) to AIStudio
set -e
...
export JOB_TAG=""
TS=$(date +%m%d-%H%M)
export JOB_NAME="yl-llbdb-all-baselines-haiku-${TS}"

export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-sglang-20260401"

export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/run_llb_db_all_baselines_haiku_aistudio.sh"
...
echo "[INFO] Submitting all baselines job=${JOB_NAME}"
