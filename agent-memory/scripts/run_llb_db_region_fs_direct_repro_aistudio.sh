#!/usr/bin/env bash
# Fresh LLB-DB Region+FS rerun after split-posterior prior inheritance fix.
# This is deliberately a fresh proceduralization run: it never loads an older
# Region checkpoint whose child posterior may contain split pseudo-evidence.
set -euo pipefail
...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_direct_repro_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"

echo '=========================================='
echo 'LLB DB corrected direct Region-FS selection-preserving reproduction (gpt-4.1-mini)'
echo "Host: $(hostname)"
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-direct-repro-20260807"
...
CONFIG=/tmp/rl_llb_db_region_fs_splitprior_fix.yaml
...
cfg['memory']['user_id']='llb_db_region_fs_direct_repro_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_direct_repro'
...
printf '%s\n' '[INFO] Fresh direct Region-FS reproduction: corrected selection/Q IDs, direct region.failure_summary.'
