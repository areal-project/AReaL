#!/bin/bash
#SBATCH --job-name=yl-bcb-dsv3-regfs-r
...
#SBATCH --mem=800G
#SBATCH --gres=gpu:8
#SBATCH --exclude=slurmd-24,slurmd-16,slurmd-13
#SBATCH --output=logs/bcb_dsv3_regfs_resume_%j.log
#SBATCH --error=logs/bcb_dsv3_regfs_resume_%j.log
...
# ============================================================
# 1) LLM server FIRST: DeepSeek-V3 via SGLang (TP=8).
#    Must start BEFORE embedding so all 8 GPUs have equal free memory (SGLang's
#    "unbalanced memory" check fails if embedding is already on GPU 0). Once SGLang
#    finishes loading and serving, embedding can safely start on GPU 0 afterwards.
# ============================================================
...
# ============================================================
# 2) Embedding server AFTER LLM: Qwen3-Embedding-8B pinned to GPU 0.
#    SGLang already loaded, so GPU 0 has the LLM shard + KV cache allocated.
#    Embedding (~15GB) fits in the remaining space. gpu-memory-utilization 0.12
#    ensures it only takes ~17GB on GPU 0.
# ============================================================
...
run_in_runner "python run/run_bcb_region.py \
    ...
    --resume_from '/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_local_region_fs/bigcodebench_eval/instruct_full/region/20260619_153259_deepseek-ai_DeepSeek-V3_region/epoch7/snapshot/7' \
    --resume_epoch 7 \
