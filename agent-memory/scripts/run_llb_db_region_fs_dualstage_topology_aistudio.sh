#!/usr/bin/env bash
# Fresh corrected proceduralization DB dual-stage early-topology screening run.
...
LOGFILE="$PROJECT_DIR/logs/llb_db_region_fs_dualstage_gpt41mini_${HOST_SHORT}_${TS}.log"
exec > >(tee -a "$LOGFILE") 2>&1
cd "$PROJECT_DIR"

echo '=========================================='
echo 'LLB DB corrected Region dual-stage early-topology E1-E3 screening'
echo "Host: $(hostname)"
...
export MEMRL_REGION_TOPOLOGY_MAINTENANCE_STEPS=270
...
export MEMRL_RUN_ID="region-fs-db-gpt41mini-dualstage-topology-20260731"
...
CONFIG=/tmp/rl_llb_db_region_fs_dualstage.yaml
...
cfg['memory']['user_id']='llb_db_region_fs_dualstage_user'
cfg['experiment']['experiment_name']='llb_db_region_fs_gpt41mini_dualstage'
...
printf '%s\n' '[INFO] Fresh dual-stage topology: init_step=180, maintenance_step=270, split_fraction=.12, max_split=1, max_merge=1, evidence>=120, child>=25.'
