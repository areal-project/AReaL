#!/bin/bash
#SBATCH --job-name=yl-bcb-ds-rgn-add-b32
...
#SBATCH --mem=200G
#SBATCH --gres=gpu:5
#SBATCH --exclude=slurmd-24,slurmd-14,slurmd-23
#SBATCH --output=logs/bcb_ds_tuned_region_additive_b32_%j.log
#SBATCH --error=logs/bcb_ds_tuned_region_additive_b32_%j.log
...
echo "=========================================="
echo "BCB DeepSeek-R1-32B + Qwen3-Embedding-8B (Region Additive b32 + P0-P2 fixes)"
...
echo ''
echo '=========================================='
echo '[REGION-ADDITIVE-B32] Additive + Beta posterior + temp=0.3 + top-1 + batch=32'
echo '=========================================='

python run/run_bcb_region.py \
    --config configs/rl_bcb_config.deepseek_tuned_region_b32.yaml \
    --split instruct \
    --subset full \
    --epochs 10 \
    ...
    --output_dir ./results/deepseek_tuned_region_additive_b32

REGION_EXIT=\$?
echo \"[INFO] Region-Additive-B32 exited with code: \$REGION_EXIT\"
