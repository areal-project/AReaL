#!/bin/bash
# ============================================================================
# HLE Region+FS Conservative: gemini-3.5-flash via MatrixLLM API
# Per docs/experiments/hle/region_fs_tuning_recommendations_20260717.md §14
# Runs alongside the existing "current" region job for direct comparison.
# ============================================================================
...
export JOB_TAG=""
TS=$(date +%Y%m%d-%H%M%S)
export JOB_NAME="${JOB_NAME:-yl-hle-region-conservative-${TS}}"
...
set -e
LOGFILE=/storage/openpsi/users/yl/agent-memory/MemRL/logs/aistudio_hle_region_conservative_gemini35flash_${RUN_ID}.log
...
python3 run/run_hle_region.py \
    --config configs/rl_hle_config.region_gemini35flash_conservative.yaml \
    ...
    --k_local 10 \
    --shrinkage_top_n 1 \
    --shrinkage_lambda_max 0.9 \
    --shrinkage_confidence_k 0.5 \
    --region_temperature 0.025 \
