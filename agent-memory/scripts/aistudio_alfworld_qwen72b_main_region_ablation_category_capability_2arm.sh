#!/bin/bash
# Main-experiment ablation: Category-Q + Capability-Region No-RFS. Shared embedding, two independent Qwen72B.
# Full train 3553, ID 140, OOD 134. GPU0 embedding; 2x Qwen72B TP2.
set -euo pipefail
...
read -r EP CP CAPP NC1 NC2 < <(python3 - <<'PY'
import socket
p=[]
for _ in range(5):
...
OUT=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/alfworld_qwen72b_main_region_ablation_category_capability_20260809
LOG="$MEMRL_DIR/logs/alfworld_qwen72b_main_region_ablation_category_capability_$TAG"
WORK_TMP=/storage/openpsi/users/yl/agent-memory/.tmp/q72b_main_region_ablation_category_capability_${TAG}_$$
mkdir -p "$OUT" "$LOG" "$WORK_TMP"; chmod 700 "$WORK_TMP"; exec > >(tee -a "$LOG/driver.log") 2>&1
echo "[PORTS] embed=$EP category=$CP capability=$CAPP nccl=$NC1/$NC2"
...
wait_server(){ n=$1 p=$2 pid=$3; for i in $(seq 1 1800); do
  if ! kill -0 "$pid" 2>/dev/null; then echo "[ERROR] $n server pid=$pid exited before readiness"; wait "$pid" || true; return 1; fi
  curl -fsS "http://127.0.0.1:$p/v1/models" 2>/dev/null | grep -q model && { echo "[READY] $n ${i}s pid=$pid"; return; }
  sleep 1
done; echo "[ERROR] $n readiness timeout port=$p pid=$pid"; return 1; }
# Start services strictly serially. Concurrent TP server initialization can race
# on SGLang/PyTorch internal TCPStore ports even when API/NCCL ports are unique.
CUDA_VISIBLE_DEVICES=0 python3 -m sglang.launch_server --model-path "$EMBED" --served-model-name Qwen/Qwen3-Embedding-8B --host 127.0.0.1 --port "$EP" --context-length 8192 --trust-remote-code --is-embedding & EPID=$!; PIDS+=("$EPID")
wait_server embed "$EP" "$EPID"
CUDA_VISIBLE_DEVICES=1,2 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port "$CP" --context-length 32768 --trust-remote-code --nccl-port "$NC1" & CPID=$!; PIDS+=("$CPID")
wait_server category "$CP" "$CPID"
CUDA_VISIBLE_DEVICES=3,4 python3 -m sglang.launch_server --model-path "$MODEL" --served-model-name Qwen2.5-72B-Instruct --tp 2 --host 127.0.0.1 --port "$CAPP" --context-length 32768 --trust-remote-code --nccl-port "$NC2" & CAPPID=$!; PIDS+=("$CAPPID")
wait_server capability "$CAPP" "$CAPPID"
...
set +e; wait $A; ra=$?; wait $C; rc=$?; set -e
echo "[ALL-DONE] category=$ra capability=$rc"; [[ $ra -eq 0 && $rc -eq 0 ]]
