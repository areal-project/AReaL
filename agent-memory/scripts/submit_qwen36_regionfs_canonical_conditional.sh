#!/bin/bash
# Submit fresh Qwen3.6 canonical conditional-RFS Region+FS run.
set -euo pipefail
...
export WORKER_NUM="0"
export JOB_TAG="qwen36-regionfs-canonical-conditional"
export JOB_NAME="yl-alf-qwen36-regionfs-canonical-conditional-${TS}"
export KM_IMAGE="acr-sh-ant-registry-vpc.cn-shanghai.cr.aliyuncs.com/gpu/areal-runtime:dev-vllm-20260429"
export JOB_COMMAND="bash /storage/openpsi/users/yl/agent-memory/MemRL/scripts/aistudio_alfworld_qwen36_regionfs_canonical_conditional.sh ${TS}"
export GPU_NUM="2"
