#!/bin/bash
# Final BCB Region+FS method: correct evidence + stable topology + original Region FS.
set -euo pipefail
...
export JOB_NAME="yl-bcb-regionfs-final-stable-$STAMP"
export BCB_CONFIG=configs/rl_bcb_config.deepseek_v3_local_region.yaml
export BCB_EPOCHS=10
export BCB_OUTPUT_DIR=/storage/openpsi/experiments/checkpoints/admin/yl-mem-region/bigcodebench/deepseek_v3_region_fs_final_stable
export BCB_ENTRYPOINT=run/run_bcb_region.py
export BCB_NO_AUTORESUME=0
# Fixed method: source-conserving split + stable topology + original 1-slot Region FS.
...
LOG_BASE="$MEMRL/logs/aistudio_bcb_regionfs_final_stable_$STAMP"
