...
# Standalone aggregation runner for new-encoding models using vllm_direct provider.
#
# Like run_aggregation_new_encoding_only.sh, but uses the vllm_direct LLM
# service provider which applies the chat template CLIENT-SIDE via
# tokenizer.apply_chat_template() and sends a flat prompt to /v1/completions.
#
# Usage:
#   bash scripts/run_aggregation_new_encoding_vllm_direct.sh \
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_aggregation_new_encoding_vllm_direct.sh [options]
...
Server options:
  ...
  --tensor-parallel-size N         Tensor parallel size (default: 8)
...
Note: This script uses the vllm_direct provider which applies the chat
template client-side via tokenizer.apply_chat_template() and sends requests
to /v1/completions instead of /v1/chat/completions.
EOF
}
...
wait_for_server_ready() {
  ...
  start_ts="$(date +%s)"
  echo "Waiting for server at http://127.0.0.1:${SERVER_PORT}/v1/models ..."
...
# vllm_direct uses the model name directly (no openai/ prefix)
REQUEST_MODEL="$MODEL_NAME"

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="$ROOT_DIR/results/test_time_compute/aggregation_new_encoding_vllm_direct"
mkdir -p "$OUTPUT_BASE"
...
# -----------------------------
echo "========================================"
echo "New-encoding aggregation (vllm_direct)"
echo "========================================"
echo "Result files: ${#RESULT_FILES[@]}"
echo "Model: $MODEL_NAME -> $MODEL_PATH"
echo "Request model: $REQUEST_MODEL"
echo "Provider: vllm_direct (client-side chat template)"
