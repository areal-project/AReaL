...
# Standalone aggregation runner for new-encoding models only.
#
# Takes existing generation result file(s) and runs pairwise aggregation
# using a new-encoding vLLM server.
#
# Usage:
#   bash scripts/run_aggregation_new_encoding_only.sh \
...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run_aggregation_new_encoding_only.sh [options]
...
REQUEST_MODEL="$(ensure_openai_model_prefix "$MODEL_NAME")"

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
OUTPUT_BASE="$ROOT_DIR/results/test_time_compute/aggregation_new_encoding_only"
mkdir -p "$OUTPUT_BASE"
...
# -----------------------------
echo "========================================"
echo "New-encoding aggregation only"
echo "========================================"
