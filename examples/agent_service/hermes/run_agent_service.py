...
"""Launch the Agent Service with the Hermes Agent.

Mirrors examples/agent_service/run_agent_service.py but loads the
Hermes-backed AgentRunnable.  The Worker instantiates one Hermes ``AIAgent``
per session in-process, so only the ``hermes-agent`` package needs to be
importable (``pip install hermes-agent``); no externally-running gateway.
...
conversation; the per-session Hermes ``AIAgent`` drives its configured upstream
LLM internally.

Self-evolution (optional)
-------------------------
Pass ``--use-areal-inference --inf-base-url http://<inf-gateway>`` to route the
Hermes agent's LLM calls through AReaL's own inference service instead of the
env upstream.  The Agent Service is fully decoupled from the training side, so
**this example** mints the per-session ``sk-sess-*`` itself by POSTing to the
...
``session_api_key`` fields straight through to the agent (their presence opts
the turn into self-evolution), so its LLM calls flow through the inference
service under that key and the trajectory is captured for training.  Set the
reward yourself afterwards by POSTing to the inference service's
``/rl/set_reward`` with the same ``sk-sess-*``.  Requires a running AReaL
inference gateway.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Agent Service — Hermes")
    parser.add_argument(
        "--admin-api-key",
        default="",
    ...
    parser.add_argument(
        "--fileroot",
        default=os.path.join(tempfile.gettempdir(), "areal-hermes"),
        help="Working root for logs and name-resolve records",
    )
    parser.add_argument(
        "--upstream-url",
        default=os.environ.get("HERMES_UPSTREAM_BASE_URL", ""),
        help="Upstream LLM base URL the Hermes agent routes to",
    )
    parser.add_argument(
        "--upstream-model",
        default=os.environ.get("HERMES_UPSTREAM_MODEL", "default"),
    ...
    if args.use_areal_inference:
        ...
        session_api_key = _start_inference_session(
            inf_base_url, args.inf_admin_key, task_id=f"hermes-{int(time.time())}"
        )
        # Surface the per-session key so the trajectory can be scored afterwards:
        #   python set_reward.py <inf-base-url> --api-key <key> --reward <r>
        print(f"Session API key (use with set_reward.py): {session_api_key}")
        inference = {
    ...
    upstream_key = os.environ.get("HERMES_UPSTREAM_API_KEY", "")
    ...
    if not inference and (not upstream_url or not upstream_key):
        raise SystemExit(
            "HERMES_UPSTREAM_BASE_URL and HERMES_UPSTREAM_API_KEY must be "
    ...
    scheduler = LocalScheduler(
        experiment_name="hermes-agent-service",
        trial_name="run0",
        gpu_devices=[],
    ...
    env_vars = {
        "HERMES_UPSTREAM_BASE_URL": upstream_url,
        "HERMES_UPSTREAM_API_KEY": upstream_key,
        "HERMES_UPSTREAM_MODEL": args.upstream_model,
    }
    for passthrough in (
        "HERMES_MAX_TURNS",
