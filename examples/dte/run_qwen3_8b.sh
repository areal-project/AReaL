...
# Qwen3-8B AdamW DTE validation launcher.
#
# The default colocation profile records the validated 2026-08-06 20-step run:
#   Slurm job: 948425
...
#   gconfig.n_samples=4
#
# Separation mode uses one 8-GPU node by default, with 4 GPUs for training
# and 4 GPUs for inference:
#   TOPOLOGY=separation
#   actor.backend=megatron:d1p1t4
#   rollout.backend=sglang:d1t4p1
#
...
usage() {
  cat <<'USAGE'
Usage:
  examples/dte/run_qwen3_8b.sh

Useful environment overrides:
  TOPOLOGY=colocation|separation  Select colocated or separated update mode.
...
Defaults to the validated Qwen3-8B AdamW DTE 20-step colocation
configuration from Slurm job 948425.
USAGE
}
...
if [[ "${TOPOLOGY}" == "colocation" ]]; then
  ...
  export ACTOR_BACKEND="${ACTOR_BACKEND:-megatron:d1p1t4}"
  export ROLLOUT_BACKEND="${ROLLOUT_BACKEND:-sglang:d1t4p1}"
  export CLUSTER_GPUS="${CLUSTER_GPUS:-8}"
  export SBATCH_GPUS="${SBATCH_GPUS:-8}"
  export JOB_NAME="${JOB_NAME:-pyq-dte-q3-8b-sep20}"
  export SGLANG_MEM_FRACTION="${SGLANG_MEM_FRACTION:-0.72}"
fi
export EXP_NAME="${EXP_NAME:-pyq-areal-port-dte-qwen3-8b}"
export TRIAL_SUFFIX="${TRIAL_SUFFIX:-$(date +%m%d_%H%M%S)}"
export TRIAL_NAME="${TRIAL_NAME:-qwen3_8b_${TOPOLOGY}_adamw_20step_bs4_ns4_${TRIAL_SUFFIX}}"
