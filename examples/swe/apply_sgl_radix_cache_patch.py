#!/usr/bin/env python3
"""
Patch: Enable Bailing Hybrid model radix cache support in SGLang 0.5.9 / 0.5.10.post1
...
PATCHES = [
    ...
    # 3b: check in get_dp_padding_mode
    {
        ...
        # sglang 0.5.10.post1 rewrote the method body (dp_size prologue +
        # extend-only SUM_LEN shortcut); the flag check still goes first.
        "variants": [
            {
                "old": (
                    "    @classmethod\n"
...
def _candidates(patch):
    yield patch["old"], patch["new"]
    for variant in patch.get("variants", []):
        yield variant["old"], variant["new"]
...
def apply_patch(root, patch, dry_run=False):
    ...
    match = next(
        ((old, new) for old, new in _candidates(patch) if old in content), None
    )
    if match is None:
...
def revert_patch(root, patch):
    ...
    with open(fp) as f:
        content = f.read()
    for old, new in _candidates(patch):
        if new in content:
