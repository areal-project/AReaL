...
#SBATCH -J zjw-monolith-swe-miniswe
#SBATCH -N 1
#SBATCH --ntasks=1
...
#SBATCH --gres=gpu:0
#SBATCH -t 7-00:00:00
#SBATCH -o /storage/openpsi/users/zjw531248/logs/monolith-swe-miniswe-%j.out
#SBATCH -e /storage/openpsi/users/zjw531248/logs/monolith-swe-miniswe-%j.err
...
CONFIG_PATH=${CONFIG_PATH:-examples/swe/swe_colocate_miniswe.yaml}
IMAGE=${AREAL_IMAGE:-/storage/openpsi/images/areal-dev-sglang-20260401.sif}
ROLLOUT_IMAGE=${ROLLOUT_IMAGE:-/storage/openpsi/images/areal-dev-sglang-20260427.sif}
...
TRITON_WARMCACHE_DIR=${TRITON_WARMCACHE_DIR:-/storage/openpsi/users/chucai.dzq/triton_warmcache}

EXP_NAME=${EXP_NAME:-zjw-monolith-swe-miniswe}
...
while true; do
  ...
  srun --mpi=pmi2 --ntasks=1 --cpus-per-task=1 --mem-per-cpu=1000M \
    singularity exec --pid --writable-tmpfs \
      ...
      --env "ROLLOUT_IMAGE=${ROLLOUT_IMAGE}" \
      ...
      --env "TRITON_WARMCACHE_DIR=${TRITON_WARMCACHE_DIR}" \
