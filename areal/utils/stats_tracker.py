...
class DistributedStatsTracker:
    ...
    def _device_for_placeholder_tensor(self, group=None):
        # Without a collective group the tensor is only used for local
        # arithmetic; keep it on CPU. A GPU-side allocation here can OOM
        # colocated setups whose device is fully budgeted by training +
        # inference engines.
        if group is None:
            return "cpu"
        try:
