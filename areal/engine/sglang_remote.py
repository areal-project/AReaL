...
class SGLangBackend:
    ...
    def launch_server(self, server_args: dict[str, Any]) -> subprocess.Popen:
        ...
        # not guaranteed by SLURM task dispatch (Problem 33: collision degrades the
        ...
        if awex_gpus_per_server is not None:
            ...
            if slurm_localid is not None:
                ...
                print(
                    f"[AWEX] colocate base_gpu_id override: SLURM_LOCALID={slurm_localid}"
                    f" x gpus_per_server={awex_gpus_per_server} -> base_gpu_id={base_gpu_id}",
                    flush=True,
                )
                logger.info(
        ...
        if awex_colocate or awex_meta_addr:
            ...
            print(
                f"[AWEX] AWEX mode: using awex_sglang_plugin entry, cmd={cmd[:4]}",
                flush=True,
            )
