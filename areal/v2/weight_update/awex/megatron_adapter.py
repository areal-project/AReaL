...
def _install_awex_qwen2_converter() -> None:
    """Align AWEX's generic Qwen2 converter with SGLang's HF parameter names."""
    from awex.converter.mcore_converter import McoreToHFWeightConverter
    from awex.models.registry import ModelRegistry

    class _Qwen2McoreToHFWeightConverter(McoreToHFWeightConverter):
    ...
    if isinstance(model_config, dict):
        ...
        updated_config["mcore_converter"] = _Qwen2McoreToHFWeightConverter
        ModelRegistry.models["Qwen2ForCausalLM"] = updated_config
    else:
        model_config.mcore_converter = _Qwen2McoreToHFWeightConverter
...
class AwexMegatronAdapter(AwexTrainingAdapter):
    ...
    def _ensure_awex_converter(
        ...
        if self._engine.hf_config.architectures[0] == "Qwen2ForCausalLM":
