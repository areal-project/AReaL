...
class DiffusionInferenceEngine:
    ...
    def _ddim_step_with_logprob(
        ...
        This is the single most numerically delicate piece of the engine; the
        formulas mirror ``DDIMScheduler.step`` exactly and must not be improvised.
        """
        import torch

        scheduler = self.scheduler
        # ---- Recover scheduler coefficients for this timestep ----
        prev_timestep = (
            timestep
        ...
        prev_sample_mean = (
            alpha_prod_t_prev ** (0.5) * pred_original + pred_sample_direction
        )
        ...
        # ---- Gaussian log-density of the sampled transition, summed over dims --
        # log N(x; mu, sigma^2) = -0.5 * ((x-mu)^2 / sigma^2 + log(2*pi*sigma^2))
        std = std_dev_t + 1e-8
        log_prob = -0.5 * (
            ((prev_sample - prev_sample_mean) ** 2) / (std**2)
            + 2 * math.log(float(std))
            + math.log(2 * math.pi)
        )
        # Sum over channel/spatial dims -> per-sample scalar log-prob.
