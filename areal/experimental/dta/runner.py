...
def _get_forkpos(lens, lcp_lens, block_size: int | None) -> list:
    """
    Compute all fork positions that DTARunner's stack must track.
...
class DTARunner:
    ...
    DTARunner stores only necessary KV caches, logits at fork
    ...
    def __init__(
        ...
        """
        Initialize DTARunner with model config, device and buffer sizes.
    ...
    def backward(self, model, token_trie, loss_fn, block_size: int) -> float:
        """
        Perform backward pass over all sequences in a TokenTrie.
        ...
        for i in range(len(inputs)):
            ...
            next_anchor = lcp_lens[i] if i < len(inputs) - 1 else 0
            B = new_tokens.numel()
            next_pop_len = self.cur_len + B - next_anchor

            if next_pop_len > block_size:
                n_blocks = ceil(next_pop_len / block_size)
                block_size_actual = ceil(next_pop_len / n_blocks)
                next_anchor = self.cur_len + B - block_size_actual

            self.push(new_tokens, attach_list, next_anchor)
