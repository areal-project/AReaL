...
class CompressedTrie:
    ...
    def get_stats(self, mode: str, block_size: int | None = None) -> dict:
        """Get traversal stats for this compressed trie."""
        if mode == "forward":
            _, lens, lcp_lens = self.get_order_forward()
        elif mode == "backward":
            _, lens, lcp_lens = self.get_order_backward()
        else:
            raise ValueError(f"Unsupported mode: {mode}")
        return _get_stats(lens, lcp_lens, mode, block_size)

    def get_subtrie(self, seq_set: set[int]) -> "CompressedTrie":
        """Build a compressed subtrie containing the selected leaf sequence IDs."""
        lens, lcp_lens = self.get_lens(seq_set)
