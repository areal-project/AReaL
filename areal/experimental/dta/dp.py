...
class DTAPartitionResult:
    """DTA partition result plus trie state reusable by callers."""

    bins: list[list[int]]
    leaf_bins: list[list[int]]
...
def pred_time(
    ...
    stats = compressed_trie.get_stats(mode, block_size)
...
def LB_by_TM(token_seqs, time_model, config: SimpleNamespace):
    ...
    for i in range(n_leaf_seqs):
        ...
        bin_compressed_trie = compressed_trie.get_subtrie(set(leaf_bins[min_bin]))
...
def try_divide(
    ...
    while start < n_seqs:
        ...
        while L < R:
            mid = (L + R + 1) // 2
            cur_subtrie = compressed_trie.get_subtrie(set(range(start, mid + 1)))
            est_time = pred_time(
...
def LB_by_DFS_and_TM(
    token_seqs, time_model, config: SimpleNamespace
) -> DTAPartitionResult:
    token_trie = TokenTrie(token_seqs)
    n_leaf_seqs = len(token_trie.inputs)
    compressed_trie = CompressedTrie(token_trie.lens, token_trie.lcp_lens)
    ...
    if n_leaf_seqs == 0:
        leaf_bins = [[] for _ in range(K)]
        return DTAPartitionResult(
            bins=[[] for _ in range(K)],
            leaf_bins=leaf_bins,
            token_trie=token_trie,
            compressed_trie=compressed_trie,
        )
