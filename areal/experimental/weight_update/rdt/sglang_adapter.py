...
class RDTSGLangAdapter:
    ...
    def rdt_execute_weight_update(self, version: int = 0) -> None:
        ...
        logger.info(
            f"RDT update: Pulling from TW shards {required_indices} for pair '{pair_name}' v{version}"
        )

        transport = self._tensor_transport or get_tensor_transport()
        if transport == "YR":
            method_name = "rdt_get_weights_tensor_yr"
        elif transport == "NIXL":
            method_name = "rdt_get_weights_tensor_nixl"
        else:
        ...
        logger.info(f"RDT update: Completed for pair '{pair_name}' v{version}")
