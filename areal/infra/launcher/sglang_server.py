...
def main(argv):
    # Slurm job cancellation sends SIGTERM, which by default terminates
    # Python without running finally blocks and orphans the sglang process
    # tree (plugin + its scheduler/detokenizer children), leaving residue
    # that drains the node. Convert SIGTERM/SIGINT into SystemExit so the
    # finally below cleans up the whole tree via kill_process_tree.
