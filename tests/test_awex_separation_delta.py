...
def test_megatron_separation_delta_commits_tracker_after_receiver_barrier(
    ...
    monkeypatch.setattr(mod, "separation_post_apply_verify_enabled", lambda: True)
    monkeypatch.setattr(
        mod,
        "verify_separation_post_apply",
        lambda *args, **kwargs: events.append(
            ("verify", kwargs["version"], kwargs["mode"], kwargs["role"])
        ),
    )
    adapter = object.__new__(mod.AwexMegatronAdapter)
    adapter._transfer_plan = object()
    ...
    adapter._delta_capture_synced_state = lambda params: {}
    adapter._delta_prepare_masks_for_separation = lambda params, version: (
    ...
    adapter._delta_commit_separation_verification_snapshot = (
        lambda params, masks, version, prepare_failed, **kwargs: events.append(
            ("snapshot", version)
        )
    )
    adapter._delta_mark_synced = lambda version, state: events.append(
        ("synced", version)
    )
...
def test_megatron_separation_snapshot_detector_builds_sparse_index_masks(monkeypatch):
    """Snapshot separation computes masks from the tracker baseline."""
    mod = common._load_megatron_adapter(monkeypatch)
    monkeypatch.setenv("DTE_DELTA_VERIFY_SNAPSHOT", "1")
    verify_calls = []

    class _Tracker:
        def __init__(self):
            self._snapshot = {"w": torch.tensor([1.0, 2.0, 3.0])}
            self._snapshot_names = {"w"}
    ...
    adapter = object.__new__(mod.AwexMegatronAdapter)
    adapter._delta_tracker = _Tracker()
    adapter._delta_detector = SimpleNamespace(name="snapshot")
    adapter._ensure_delta_components = lambda: None

    def _verify(params_list, masks, version):
        verify_calls.append((params_list, masks, version))
        return True

    adapter._delta_verify_masks_against_snapshot = _verify

    masks, use_delta = adapter._delta_prepare_masks_for_separation(
        {"w": torch.tensor([1.0, 4.0, 3.0])},
        version=2,
    )
    ...
    torch.testing.assert_close(
        masks["w"], torch.tensor([1], dtype=torch.int32), rtol=0, atol=0
    )
    torch.testing.assert_close(
        adapter._delta_tracker._snapshot["w"],
        torch.tensor([1.0, 2.0, 3.0]),
        rtol=0,
        atol=0,
    )
    assert len(verify_calls) == 1
    params_list, verified_masks, verified_version = verify_calls[0]
    assert [name for name, _ in params_list] == ["w"]
    assert verified_masks is masks
    assert verified_version == 2


def test_megatron_separation_snapshot_verify_failure_full_syncs(monkeypatch):
    """Snapshot separation uses the same snapshot-verify fallback as adamw."""
    mod = common._load_megatron_adapter(monkeypatch)
    monkeypatch.setenv("DTE_DELTA_VERIFY_SNAPSHOT", "1")
    seed_args = []
    ...
    class _Tracker:
        ...
        def seed(self, params_list, version, store_snapshot=False):
            seed_args.append((params_list, version, store_snapshot))

    adapter = object.__new__(mod.AwexMegatronAdapter)
    adapter._delta_tracker = _Tracker()
    adapter._delta_detector = SimpleNamespace(name="snapshot")
    adapter._ensure_delta_components = lambda: None
    adapter._delta_verify_masks_against_snapshot = lambda *args: False
    ...
    params_list, version, store_snapshot = seed_args[0]
    assert [name for name, _ in params_list] == ["w"]
    assert version == 2
    assert store_snapshot is True


def test_megatron_separation_snapshot_detector_missing_baseline_full_syncs(
    monkeypatch,
):
    """Snapshot separation falls back to full sync when no baseline exists."""
    mod = common._load_megatron_adapter(monkeypatch)
    seed_args = []
...
def test_megatron_separation_snapshot_delta_updates_baseline_after_success(
    monkeypatch,
):
    """A successful snapshot delta advances both version and snapshot values."""
    mod = common._load_megatron_adapter(monkeypatch)
    events = []

    class _Tracker:
        def __init__(self):
            self._snapshot = {"w": torch.tensor([0.0, 0.0, 0.0])}
            self._snapshot_names = {"w"}
    ...
    monkeypatch.setattr(mod, "separation_post_apply_verify_enabled", lambda: False)
    ...
    adapter._transfer_rank = 8
    adapter._world_size = 16
    adapter._delta_tracker = tracker
    adapter._delta_detector = SimpleNamespace(name="snapshot")
    adapter.get_local_shard_parameters = lambda: {"w": torch.tensor([0.0, 5.0, 0.0])}
    adapter._ensure_delta_components = lambda: None
    adapter._delta_capture_synced_state = lambda params: {}

    def _send(params, masks, version):
        del params
        events.append(("send", version))
        torch.testing.assert_close(
            masks["w"], torch.tensor([1], dtype=torch.int32), rtol=0, atol=0
        )

    adapter._execute_separation_delta_send = _send
    adapter._delta_mark_synced = lambda version, state: events.append(
        ("synced", version)
    )
    ...
    torch.testing.assert_close(
        tracker._snapshot["w"], torch.tensor([0.0, 5.0, 0.0]), rtol=0, atol=0
    )
...
def test_megatron_post_apply_mismatch_does_not_advance_delta_state(monkeypatch):
    """A receiver fingerprint mismatch must fail before anchor state advances."""
    mod = common._load_megatron_adapter(monkeypatch)
    events = []
    ...
    def _fail_verify(*args, **kwargs):
        del args, kwargs
        events.append(("verify", "mismatch"))
        raise RuntimeError("post-apply weight mismatch")

    monkeypatch.setattr(mod, "verify_separation_post_apply", _fail_verify)
    adapter = object.__new__(mod.AwexMegatronAdapter)
    adapter._transfer_plan = object()
    adapter._weights_update_group = "nccl"
    adapter._weights_update_group_gloo = "gloo"
    ...
    adapter._execute_separation_delta_send = lambda *args: None
    adapter._delta_commit_separation_verification_snapshot = (
        lambda *args, **kwargs: events.append(("snapshot", kwargs))
    )
    adapter._delta_mark_synced = lambda *args: events.append(("synced", args))

    with pytest.raises(RuntimeError, match="post-apply weight mismatch"):
        adapter._execute_separation_weight_update(version=2)

    assert events == [("barrier", "gloo"), ("verify", "mismatch")]
...
def test_streaming_generator_waits_async_batch_before_yield(monkeypatch):
    ...
    inv = dd.build_detector(
        "inversion", SimpleNamespace(_offloaded_optimizer_states={})
    )
