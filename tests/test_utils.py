...
@pytest.mark.parametrize("sync_mbs", [False, True])
def test_forward_transport_padding_is_removed_from_outputs(padded, sync_mbs):
    data = {
    ...
    mb_list = split_padded_tensor_dict_into_mb_list(
        data,
        MicroBatchSpec(n_mbs=3, max_tokens_per_mb=16),
        allow_transport_padding=True,
        seq_align_to=16,
        padded=padded,
        sync_mbs=sync_mbs,
...
def test_synchronized_training_schedule_preserves_extra_local_microbatches(
    ...
    def _split_into_local_microbatches(data, mb_spec, sync_mbs):
        assert sync_mbs is False
        microbatches = [
