...
def test_factories_build_dte_objects(dc, monkeypatch):
    """The lazy factories should build DTE objects when DTE is available."""
...
def test_full_then_delta_roundtrip(dc, monkeypatch):
    """A dense base followed by sparse delta reconstructs byte-identically."""
    pytest.importorskip("dte")
    ...
    for key, value in w1.items():
        assert torch.equal(full1[key], value), f"full-sync mismatch on {key}"

    w2 = {key: value.clone() for key, value in w1.items()}
    ...
    assert "embed.weight" not in names
    assert "layer.bias" not in names
    full2, masks2 = engine.reconstruct(_over_the_wire(names, tensors), 2)
    assert masks2 is not None
    for key, value in w2.items():
        assert torch.equal(full2[key], value), f"delta mismatch on {key}"


def test_header_constant_matches_dte(dc):
    """The mirrored payload header name must not drift from DTE."""
    pytest.importorskip("dte")
...
def test_payload_carries_delta(dc):
    """Header-only detection must stay dependency-free."""
...
def _load_sglang_adapter(monkeypatch):
    ...
    separation_verify_mod = types.ModuleType(
        "areal.v2.weight_update.awex.separation_verify"
    )
    separation_verify_mod.separation_post_apply_verify_enabled = lambda: False
    separation_verify_mod.verify_separation_post_apply = lambda *args, **kwargs: None
    monkeypatch.setitem(
        sys.modules,
        "areal.v2.weight_update.awex.separation_verify",
        separation_verify_mod,
    )
...
def _load_megatron_adapter(monkeypatch):
    ...
    utils_dte_mod = types.ModuleType("areal.utils.dte")
    utils_dte_mod.dte_verification_snapshot_commit_action = (
        lambda *args, **kwargs: "none"
    )
    monkeypatch.setitem(sys.modules, "areal.utils.dte", utils_dte_mod)
