...
```bash
export AREAL_ADMIN_API_KEY="$(python3 -c 'import secrets; print(secrets.token_hex(32))')"
...
python3 examples/math/gsm8k_rl.py \
    ...
    +rollout.agent.admin_api_key="$AREAL_ADMIN_API_KEY" \
...
- `rollout.setup_timeout=900` leaves enough time for a cold SGLang startup on a new
  machine.
- `rollout.agent.admin_api_key` uses a newly generated key for the local rollout service
