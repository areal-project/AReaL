...
如果希望丢弃样本数不足的分组，可以设置 `min_valid_group_size`：
...
`1`，即只要分组中至少有一个有效结果，就会保留这个部分分组；将它设为 `gconfig.n_samples`（或与 `group_size`
相同的值）可以要求只保留完整分组。如果有效结果数低于该阈值，`GroupedRolloutWorkflow` 会返回 `None`，从而丢弃整个分组。该值必须在
`[1, group_size]` 范围内。
