...
并发的逐轮评分被取消时（包括评分子任务自行取消），`run()` 会取消并等待其余评分任务结束，然后再向外传播取消。
被中断的评分器不会提交部分结果，此次运行也不会发布指标。
...
v1 代理在 `initialize` RPC 中使用同一个异步工厂，全部初始化成功后才发布配置好的客户端。
重复相同的初始化请求会复用已完成的引擎初始化，以及就绪后已有的运行器；代理初始化失败后可以重试，无需重新初始化引擎。
初始化参数不同时会拒绝请求，而不是替换仍在使用的资源。
控制器的 `destroy` RPC 会先停止接收新的 PRM 导出评分，等待已开始的评分结束，再关闭运行器，最后销毁引擎。
未收到 destroy RPC 时，ASGI 生命周期结束也会执行同样的清理。评分器的普通清理异常不会跳过引擎销毁。
仍在等待会话完成的导出请求不会阻塞 destroy RPC 对 PRM 评分的等待；如果它随后需要 PRM 评分，会返回 HTTP 503，而不是导出未经评分的数据。
这不会改变 ASGI 服务器在关闭时对未完成 HTTP 请求的处理。
引擎销毁失败时，异常会向外传播，并保留引擎供后续 destroy 或关闭流程重试；已经完成的评分器清理不会重复执行。
正常运行时，评分仍可并发执行。取消行为遵循上述运行器语义，被中断的清理钩子不会自动重试。
