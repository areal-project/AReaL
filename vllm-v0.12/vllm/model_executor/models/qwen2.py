...
from .chunked_rope_mixin import ChunkedRopeMixin
...
class Qwen2Model(ChunkedRopeMixin, nn.Module):
    def __init__(
        self,
        *,
        ...
        # nn.Module must be initialised first because ChunkedRopeMixin
        # also inherits from nn.Module; Python's MRO handles this correctly
        # via cooperative super().__init__().
        super().__init__()
        ...
        # Chunked-RoPE: detect from config and cache token IDs
        self._init_chunked_rope(config)
...
class Qwen2ForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3):
    ...
    def forward(
        ...
        # Rewrite positions to encode chunk metadata when chunked RoPE is
        # active. Must be done here, *outside* the @support_torch_compile
        # boundary of Qwen2Model.forward, so the eager position-rewriting
        # logic is never traced by torch.compile.
        positions = self.model._maybe_rewrite_positions(input_ids, positions)
